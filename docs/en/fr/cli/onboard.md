...
- Avec `--install-daemon`, si le mode jeton nécessite un jeton et que le SecretRef de jeton configuré n'est pas résolu, l'intégration échoue fermée avec des conseils de correction.
...
- Sauf si vous passez `--skip-health`, l'intégration attend une gateway locale accessible avant de se terminer avec succès.
...
- Comportement de la portée DM d'intégration locale : [CLI Onboarding Reference](/start/wizard-cli-reference#outputs-and-internals).
