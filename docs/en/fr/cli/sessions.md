...
- Note de portée : `openclaw sessions cleanup` maintient uniquement les magasins de sessions/transcriptions. Il ne supprime pas les journaux d'exécution cron (`cron/runs/<jobId>.jsonl`), qui sont gérés par `cron.runLog.maxBytes` et `cron.runLog.keepLines` dans la [Configuration Cron](/automation/cron-jobs#configuration) et expliqués dans [Maintenance Cron](/automation/cron-jobs#maintenance).
