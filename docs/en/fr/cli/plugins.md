...
Gérer les plugins/extensions Gateway (chargés en processus).
...
la liste blanche des plugins, et les entrées `plugins.load.paths` liées le cas échéant.
Pour les plugins de mémoire actifs, l'emplacement de mémoire se réinitialise à `memory-core`.
