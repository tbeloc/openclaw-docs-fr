...
Pour les configurations intentionnelles d'utilisateurs partagés, les conseils d'audit sont de mettre en sandbox toutes les sessions, de garder l'accès au système de fichiers limité à l'espace de travail, et de garder les identités personnelles/privées ou les credentials hors de ce runtime.
...
Les paramètres préfixés par `dangerous`/`dangerously` sont des remplacements explicites de l'opérateur break-glass ; en activer un n'est pas, en soi, un rapport de vulnérabilité de sécurité.
Pour l'inventaire complet des paramètres dangereux, consultez la section « Résumé des drapeaux non sécurisés ou dangereux » dans [Security](/fr/gateway/security).
...
- ne change pas les choix d'exposition de liaison/auth/réseau de gateway
