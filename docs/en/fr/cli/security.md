...
Pour les configurations intentionnelles multi-utilisateurs, les conseils d'audit consistent à mettre en sandbox toutes les sessions, à garder l'accès au système de fichiers limité à l'espace de travail, et à garder les identités personnelles/privées ou les credentials hors de ce runtime.
Il avertit également lorsque de petits modèles (`<=300B`) sont utilisés sans mise en sandbox et avec des outils web/navigateur activés.
...
Il avertit lorsque les conteneurs Docker du navigateur sandbox existants ont des étiquettes de hachage manquantes/obsolètes (par exemple, les conteneurs pré-migration manquant `openclaw.browserConfigEpoch`) et recommande `openclaw sandbox recreate --browser --all`.
