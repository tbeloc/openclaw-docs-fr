...
Si vous avez installé via **npm/pnpm** (installation globale, sans métadonnées git), les mises à jour se font via le flux du gestionnaire de paquets dans [Updating](/install/updating).
...
- `--tag <dist-tag|version>`: remplacer le dist-tag npm ou la version pour cette mise à jour uniquement.
...
Afficher le canal de mise à jour actif + tag/branche/SHA git (pour les extractions de source), plus la disponibilité des mises à jour.
...
- `stable`: extraire le dernier tag non-beta, puis construire + doctor.
- `beta`: extraire le dernier tag `-beta`, puis construire + doctor.
...
7. Construit + construit l'interface utilisateur Control.
