...
Ouvrir l'interface utilisateur terminal connectée à la Gateway.
...
- Lorsqu'elle est lancée depuis l'intérieur d'un répertoire d'espace de travail d'agent configuré, TUI sélectionne automatiquement cet agent pour la clé de session par défaut (sauf si `--session` est explicitement `agent:<id>:...`).
