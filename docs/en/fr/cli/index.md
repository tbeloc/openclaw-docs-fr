...
Configuration du hook Gmail Pub/Sub + exécuteur. Voir [/automation/gmail-pubsub](/automation/gmail-pubsub).
...
Assistant DNS de découverte de zone étendue (CoreDNS + Tailscale). Voir [/gateway/discovery](/gateway/discovery).
...
- L'aperçu inclut l'état de la passerelle + le service d'hôte de nœud lorsqu'il est disponible.
...
Réinitialiser la configuration/l'état local (garde l'interface de ligne de commande installée).
...
Désinstaller le service de passerelle + les données locales (l'interface de ligne de commande reste).
...
- `gateway status` imprime le chemin de configuration que l'interface de ligne de commande utilise par rapport à la configuration que le service utilise probablement (env du service), plus l'URL cible de sonde résolue.
...
Lorsque vous passez `--url`, l'interface de ligne de commande n'applique pas automatiquement la configuration ou les identifiants d'environnement.
