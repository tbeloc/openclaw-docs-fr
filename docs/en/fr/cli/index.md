...
- [`daemon`](/fr/cli/daemon) (alias hérité pour les commandes de service gateway)
- [`clawbot`](/fr/cli/clawbot) (alias d'espace de noms hérité)
...
- `openclaw security audit --fix` — renforcer les paramètres par défaut sûrs et chmod state/config.
...
La plupart des modifications de plugins nécessitent un redémarrage de la gateway. Voir [/plugin](/fr/tools/plugin).
...
- `--deep`: analyser les services système pour les installations gateway supplémentaires.
...
Voir [`acp`](/fr/cli/acp) pour les options complètes et les exemples.
...
- `openclaw status --usage` (imprime la répartition complète du fournisseur)
...
- `gateway status` sonde le RPC de la passerelle par défaut en utilisant le port/la configuration résolue du service (remplacer par `--url/--token/--password`).
...
- `gateway status` imprime le chemin de configuration que la CLI utilise par rapport à la configuration que le service utilise probablement (env du service), plus l'URL cible de sonde résolue.
...
- `node` résout l'authentification de la passerelle à partir de l'env/config (pas de drapeaux `--token`/`--password`) : `OPENCLAW_GATEWAY_TOKEN` / `OPENCLAW_GATEWAY_PASSWORD`, puis `gateway.auth.*`. En mode local, l'hôte de nœud ignore intentionnellement `gateway.remote.*` ; en `gateway.mode=remote`, `gateway.remote.*` participe selon les règles de précédence à distance.
- Les variables d'env héritées `CLAWDBOT_GATEWAY_*` sont intentionnellement ignorées pour la résolution d'authentification du node-host.

## Nœuds

`nodes` communique avec la passerelle et cible les nœuds appairés. Voir [/nodes](/fr/nodes).
...
CLI de contrôle du navigateur (Chrome/Brave/Edge/Chromium dédié). Voir [`openclaw browser`](/fr/cli/browser) et l'[outil Navigateur](/fr/tools/browser).
