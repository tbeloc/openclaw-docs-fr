...
Liste tous les hooks découverts à partir des répertoires workspace, managed et bundled.

**Options :**

- `--eligible`: Afficher uniquement les hooks éligibles (conditions remplies)
- `--json`: Sortie au format JSON
- `-v, --verbose`: Afficher les informations détaillées incluant les conditions manquantes
...
- `--pin`: Enregistrez les installations npm comme `name@version` exacte résolue dans `hooks.internal.installs`
