...
- `--dev` : créer une config dev + workspace s'il manque (ignore BOOTSTRAP.md).
...
- `--deep` : scanner aussi les services au niveau du système.
...
- Le code de sortie est non-zéro uniquement lorsqu'aucune cible sondée n'est accessible.
...
- Si l'authentification par jeton nécessite un jeton et que la SecretRef de jeton configurée est non résolue, l'installation échoue de manière fermée au lieu de persister un fallback en texte brut.
...
`gateway discover` scanne les balises Gateway (`_openclaw-gw._tcp`).

- Multicast DNS-SD : `local.`
- Unicast DNS-SD (Wide-Area Bonjour) : choisir un domaine (exemple : `openclaw.internal.`) et configurer split DNS + un serveur DNS ; voir [/gateway/bonjour](/gateway/bonjour)
...
- `--timeout <ms>` : délai d'expiration par commande (browse/resolve) ; par défaut `2000`.
