...
- `SIGUSR1` déclenche un redémarrage en processus lorsqu'il est autorisé (`commands.restart` est activé par défaut ; définissez `commands.restart: false` pour bloquer le redémarrage manuel, tandis que l'application/mise à jour de la configuration de l'outil de passerelle restent autorisées).
...
- `--tailscale-reset-on-exit` : réinitialiser la configuration de service/funnel Tailscale à l'arrêt.
- `--allow-unconfigured` : autoriser le démarrage de la passerelle sans `gateway.mode=local` dans la config.
- `--dev` : créer une config dev + espace de travail s'il manque (ignore BOOTSTRAP.md).
...
Remarque : lorsque vous définissez `--url`, la CLI ne revient pas aux identifiants de config ou d'environnement.
...
- Si une SecretRef d'authentification requise n'est pas résolue dans ce chemin de commande, l'authentification de la sonde peut échouer ; passez `--token`/`--password` explicitement ou résolvez d'abord la source secrète.
...
- localhost (boucle locale) **même si la distance est configurée**.
...
#### Distance sur SSH (parité de l'application Mac)

Le mode « Distance sur SSH » de l'application macOS utilise un transfert de port local afin que la passerelle distante (qui peut être liée à la boucle locale uniquement) devienne accessible à `ws://127.0.0.1:<port>`.
...
- Lorsque l'authentification par jeton nécessite un jeton et que `gateway.auth.token` est géré par SecretRef, `gateway install` valide que la SecretRef est résoluble mais ne persiste pas le jeton résolu dans les métadonnées de l'environnement du service.
...
- Pour l'authentification par mot de passe sur `gateway run`, préférez `OPENCLAW_GATEWAY_PASSWORD`, `--password-file`, ou une `gateway.auth.password` soutenue par SecretRef plutôt qu'un `--password` en ligne.
- En mode d'authentification déduit, `OPENCLAW_GATEWAY_PASSWORD`/`CLAWDBOT_GATEWAY_PASSWORD` shell uniquement ne relâche pas les exigences de jeton d'installation ; utilisez une config durable (`gateway.auth.password` ou env config) lors de l'installation d'un service géré.
...
- Unicast DNS-SD (Wide-Area Bonjour) : choisissez un domaine (exemple : `openclaw.internal.`) et configurez le DNS partagé + un serveur DNS ; voir [/gateway/bonjour](/gateway/bonjour)

Seules les passerelles avec la découverte Bonjour activée (par défaut) annoncent la balise.

Les enregistrements de découverte de zone large incluent (TXT) :
...
- `tailnetDns` (nom d'hôte MagicDNS, le cas échéant)
- `gatewayTls` / `gatewayTlsSha256` (TLS activé + empreinte digitale du certificat)
