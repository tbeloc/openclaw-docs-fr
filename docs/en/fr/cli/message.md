...
- En cas d'absence du cache, OpenClaw tentera une recherche de répertoire en direct si le fournisseur le supporte.
...
- `send`
  ...
  - Telegram uniquement : `--thread-id` (identifiant du sujet du forum)
...
- `react`
  ...
  - Remarque : `--remove` nécessite `--emoji` (omettez `--emoji` pour effacer vos propres réactions si supporté ; voir /tools/reactions)
...
- `timeout` : `--guild-id`, `--user-id` (optionnel `--duration-min` ou `--until` ; omettez les deux pour effacer le délai d'attente)
