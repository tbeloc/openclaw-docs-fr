...
- `--raw <command>` : exécute une chaîne shell (`/bin/sh -lc` ou `cmd.exe /c`).
  En mode liste blanche sur les hôtes de nœud Windows, les exécutions de wrapper shell `cmd.exe /c` nécessitent une approbation
  (l'entrée de liste blanche seule ne permet pas automatiquement la forme wrapper).
