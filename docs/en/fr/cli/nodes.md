...
- `--invoke-timeout <ms>` : délai d'invocation du nœud (par défaut `15000`).
...
- `--env <key=val>` : remplacement env (répétable). Remarque : les hôtes de nœud ignorent les remplacements `PATH` (et `tools.exec.pathPrepend` n'est pas appliqué aux hôtes de nœud).
- `--command-timeout <ms>` : délai d'expiration de la commande.
- `--invoke-timeout <ms>` : délai d'invocation du nœud (par défaut `30000`).
