...
Gérez les demandes d'appairage d'appareils et les tokens limités aux appareils.
...
Effectuez une rotation du token d'un appareil pour un rôle spécifique (en mettant à jour optionnellement les portées).
...
Révoquez un token d'appareil pour un rôle spécifique.
...
- Si la portée d'appairage n'est pas disponible sur la boucle locale (et qu'aucune `--url` explicite n'est passée), list/approve peut utiliser un secours d'appairage local.
...
1. Confirmez la source actuelle du token de la passerelle :
...
3. Effectuez une rotation du token opérateur pour l'appareil affecté :
...
5. Réessayez la connexion client avec le token/mot de passe partagé actuel.
