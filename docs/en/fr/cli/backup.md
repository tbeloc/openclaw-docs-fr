...
Créez une archive de sauvegarde locale pour l'état, la configuration, les identifiants, les sessions et éventuellement les espaces de travail d'OpenClaw.
...
- Les chemins de sortie à l'intérieur des arborescences d'état/espace de travail sources sont rejetés pour éviter l'auto-inclusion.
...
- Le répertoire des identifiants OAuth
...
Si vous voulez toujours une sauvegarde partielle dans cette situation, réexécutez :
...
Les grands espaces de travail sont généralement le principal moteur de la taille de l'archive. Si vous voulez une sauvegarde plus petite ou plus rapide, utilisez `--no-include-workspace`.
