...
Invite interactive pour configurer les identifiants, les appareils et les paramètres par défaut de l'agent.

Note : La section **Model** inclut désormais une sélection multiple pour la liste d'autorisation `agents.defaults.models` (ce qui s'affiche dans `/model` et le sélecteur de modèle).
...
- Référence de configuration de la passerelle : [Configuration](/fr/gateway/configuration)
