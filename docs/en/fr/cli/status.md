---
summary: "Référence CLI pour `openclaw status` (diagnostics, sondes, snapshots d'utilisation)"
read_when:
...
- L'aperçu inclut l'état d'installation/exécution de la passerelle + service hôte de nœud lorsqu'il est disponible.
...
- Les surfaces de statut en lecture seule (`status`, `status --json`, `status --all`) résolvent les SecretRefs supportées pour leurs chemins de configuration ciblés lorsque c'est possible.
