...
Transport hérité : [Bridge protocol](/gateway/bridge-protocol) (TCP JSONL ; déprécié/supprimé pour les nodes actuels).
...
- `canvas present` accepte les URLs ou les chemins de fichiers locaux (`--target`), plus optionnellement `--x/--y/--width/--height` pour le positionnement.
...
## Commandes de données personnelles et d'appareil Android
