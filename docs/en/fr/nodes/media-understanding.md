---
summary: "Compréhension des images/audio/vidéo entrantes (optionnelle) avec fournisseur + secours CLI"
read_when:
...
- Supporter les **API de fournisseur** et les **secours CLI**.
- Permettre plusieurs modèles avec secours ordonné (erreur/taille/délai d'attente).
...
- `prompt` par défaut à simple "Describe the {media}." plus la guidance `maxChars` (image/vidéo uniquement).
...
1. **CLIs locaux** (audio uniquement; s'ils sont installés)
...
Remarque : La détection binaire est au mieux un effort sur macOS/Linux/Windows; assurez-vous que le CLI est sur `PATH` (nous développons `~`), ou définissez un modèle CLI explicite avec un chemin de commande complet.
...
honore les variables d'environnement proxy sortant standard pour les appels HTTP du fournisseur :
...
Si la valeur du proxy est mal formée, OpenClaw enregistre un avertissement et bascule vers
la récupération directe.

## Capacités (optionnel)

Si vous définissez `capabilities`, l'entrée s'exécute uniquement pour ces types de médias. Pour les listes partagées,
OpenClaw peut déduire les valeurs par défaut :
...
| -------- | ------------------------------------------------ | --------------------------------------------------------- |
| Image    | OpenAI / Anthropic / Google / autres via `pi-ai` | Tout modèle capable d'image dans le registre fonctionne.  |
...
## Guidance de sélection de modèle
...
- Gardez au moins un secours par capacité pour la disponibilité (modèle de qualité + modèle plus rapide/moins cher).
- Les secours CLI (`whisper-cli`, `whisper`, `gemini`) sont utiles quand les API de fournisseur ne sont pas disponibles.
...
- La compréhension est **au mieux un effort**. Les erreurs ne bloquent pas les réponses.
- Les pièces jointes sont toujours transmises aux modèles même quand la compréhension est désactivée.
- Utilisez `scope` pour limiter où la compréhension s'exécute (par ex. uniquement les DM).

## Docs connexes

- [Configuration](/gateway/configuration)
- [Support des images et médias](/nodes/images)
