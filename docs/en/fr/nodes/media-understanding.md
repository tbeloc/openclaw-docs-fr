...
- `tools.media.models`: liste de modèles partagés (utiliser `capabilities` pour contrôler).
...
1. **CLI locaux** (audio uniquement; s'ils sont installés)
...
- Pour les agents activés par des outils traitant des entrées non fiables, évitez les modèles média plus anciens/plus faibles.
