---
summary: "Capture caméra (nœuds iOS/Android + app macOS) pour utilisation par agent : photos (jpg) et courts clips vidéo (mp4)"
read_when:
...
OpenClaw supporte la **capture caméra** pour les workflows d'agent :
...
```bash
openclaw nodes camera list --node <id>            # lister les ids de caméra
