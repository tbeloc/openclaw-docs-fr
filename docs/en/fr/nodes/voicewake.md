...
- macOS et iOS conservent des **bascules locales Voice Wake activé/désactivé** (l'expérience utilisateur locale et les permissions diffèrent).
...
- Tous les nœuds connectés (iOS/Android), et aussi lors de la connexion du nœud en tant que poussée initiale « état actuel ».
...
- La modification des mots de réveil dans les paramètres appelle `voicewake.set` (sur la WS de la Passerelle) et maintient également la détection locale des mots de réveil réactive.
