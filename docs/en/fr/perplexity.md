...
OpenClaw supporte Perplexity Search API en tant que fournisseur `web_search`.
Il retourne des résultats structurés avec les champs `title`, `url` et `snippet`.

Pour la compatibilité, OpenClaw supporte également les configurations héritées Perplexity Sonar/OpenRouter.
Si vous utilisez `OPENROUTER_API_KEY`, une clé `sk-or-...` dans `tools.web.search.perplexity.apiKey`, ou si vous définissez `tools.web.search.perplexity.baseUrl` / `model`, le fournisseur bascule vers le chemin chat-completions et retourne des réponses synthétisées par IA avec citations au lieu des résultats structurés de Search API.
...
Si `provider: "perplexity"` est configuré et que la SecretRef de clé Perplexity n'est pas résolue sans secours d'environnement, le démarrage/rechargement échoue rapidement.

## Paramètres de l'outil

Ces paramètres s'appliquent au chemin natif Perplexity Search API.
...
| `max_tokens`          | Budget de contenu total (défaut : 25000, max : 1000000) |
...
Les filtres spécifiques à Search API tels que `country`, `language`, `date_after`, `date_before`, `domain_filter`, `max_tokens` et `max_tokens_per_page` retournent des erreurs explicites.
...
- Perplexity Search API retourne des résultats de recherche web structurés (`title`, `url`, `snippet`)
...
Voir [Documentation Perplexity Search API](https://docs.perplexity.ai/docs/search/quickstart) pour plus de détails.
```
