...
read_when:
  ...
  - Gérer les schémas de config de plugin ou le gatekeeping de chargement de plugin
...
- Compatibilité rétroactive au chargement (les clés héritées ne se migrent pas automatiquement).
...
## Gatekeeping de commande (quand la config est invalide)
...
- `src/plugins/*` : ajouter le registre de schéma + gatekeeping.
- Gatekeeping de commande CLI dans `src/cli`.
