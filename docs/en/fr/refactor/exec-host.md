...
- Garder les valeurs par défaut **sûres** : pas d'exécution cross-host sauf si explicitement activée.
- Diviser l'exécution en un **service runner sans interface** avec UI optionnelle (app macOS) via IPC local.
...
- Cross-plateforme : socket Unix + authentification par token (parité macOS/Linux/Windows).
...
- **Authentification du socket :** socket Unix + token (cross-plateforme) ; scinder plus tard si nécessaire.
- **État du nœud hôte :** `~/.openclaw/node.json` (id du nœud + token d'appairage).
...
- TTL court (ex. 10s) + charge utile max + limite de débit.
...
- Limiter stdout+stderr combinés à **200k** ; conserver la **queue de 20k** pour les événements.
...
- Tests unitaires : précédence de résolution de politique (paramètre d'outil → override d'agent → global).
