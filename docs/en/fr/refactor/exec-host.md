...
- Fournir une **politique par agent**, liste d'autorisation, mode ask et liaison de nœud.
- Supporter les **modes ask** qui fonctionnent _avec_ ou _sans_ listes d'autorisation.
- Cross-platform : socket Unix + authentification par token (parité macOS/Linux/Windows).
...
- **Authentification du socket :** socket Unix + token (cross-platform) ; scinder plus tard si nécessaire.
...
- Fallback ask quand aucune UI n'est disponible.
...
- TTL court (par ex. 10s) + payload max + limite de débit.
...
- Le `handleBridgeEvent` de la gateway mappe ces derniers dans `enqueueSystemEvent`.
...
- App Mac : éditeur de liste d'autorisation, sélecteur par agent, UI de politique ask.
...
- Tests d'événement Bridge : routage événement nœud → événement système.
