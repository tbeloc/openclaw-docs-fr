...
description: Suivi des notes de refactorisation, décisions, tests et éléments ouverts pour la mise en miroir des sessions sortantes.
summary: "Notes de refactorisation pour la mise en miroir des envois sortants dans les sessions de canal cible"
read_when:
  - Travail sur le comportement de mise en miroir des transcriptions/sessions sortantes
  - Débogage de la dérivation de sessionKey pour les chemins d'envoi/outil de message
---
...
- En cours.
- Routage des canaux principaux + plugins mis à jour pour la mise en miroir sortante.
- L'envoi de passerelle dérive maintenant la session cible lorsque sessionKey est omis.

## Contexte

Les envois sortants étaient mis en miroir dans la session d'agent _actuelle_ (clé de session d'outil) plutôt que dans la session de canal cible. Le routage entrant utilise les clés de session de canal/pair, donc les réponses sortantes se retrouvaient dans la mauvaise session et les cibles de premier contact manquaient souvent d'entrées de session.
