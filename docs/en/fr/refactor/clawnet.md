...
Bonjour Peter — excellente direction ; cela déverrouille une UX plus simple + une sécurité plus forte.
...
- TLS annonce l'empreinte dans la découverte TXT.
- Code :
...
- ACP → Gateway WS.
- Le contrôle du navigateur utilise son propre serveur de contrôle HTTP.
...
- Entrées de présence de Gateway à partir des clients WS.
...
- **Local uniquement** : appairage automatique quand le client se connecte via loopback/socket Unix.
- **Défi via SSH** : gateway émet un nonce ; le client prouve SSH en le récupérant.
- **Fenêtre de présence physique** : après une approbation locale sur l'UI de l'hôte de la gateway, autoriser l'appairage automatique pour une courte fenêtre (par ex. 10 minutes).

Toujours enregistrer + enregistrer les approbations automatiques.
...
L'approbation est **hébergée par la gateway**, l'UI livrée aux clients opérateurs.
...
4. La décision d'approbation est envoyée à la gateway : `approval.resolve`.
...
- La première résolution gagne ; gateway rejette les résolutions ultérieures comme déjà réglées.
...
- Rôle/allowlist appliqué à la limite de la gateway.
- Aucun client n'obtient l'API "complète" sans la portée d'opérateur.
...
- Auditer les modifications avec les horodatages.

# Audit + limitation de débit

- Enregistrer : demandes d'appairage, approbations/refus, émission/rotation/révocation de tokens.
...
3. UX de rotation + révocation de token
   - Rotation automatique au changement de rôle.
   - UI pour révoquer par deviceId + rôle.

4. Découverte
   - Étendre le TXT Bonjour actuel pour inclure l'empreinte WS TLS + les indices de rôle.
...
5. Approbation inter-réseau
   ...
   - La première réponse gagne ; gateway applique l'atomicité.
...
- Proposition : un protocole WS avec rôles + portées explicites, appairage + épinglage TLS unifié, approbations hébergées par la gateway, IDs d'appareil stables + slugs mignons.
