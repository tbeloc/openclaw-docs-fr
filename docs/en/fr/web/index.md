---
summary: "Surfaces web de la passerelle : Interface de contrôle, modes de liaison et sécurité"
read_when:
...
La passerelle sert une petite **interface de contrôle de navigateur** (Vite + Lit) depuis le même port que la WebSocket de la passerelle :
...
L'interface de contrôle est **activée par défaut** quand les ressources sont présentes (`dist/control-ui`).
...
- Pour les déploiements d'interface de contrôle non-loopback, définissez `gateway.controlUi.allowedOrigins`
...
- Avec Serve, les en-têtes d'identité Tailscale peuvent satisfaire l'authentification de l'interface de contrôle/WebSocket
  ...
  [Tailscale](/gateway/tailscale) et [Sécurité](/gateway/security). Ce flux sans token suppose que l'hôte de la passerelle est de confiance.
