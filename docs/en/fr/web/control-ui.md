...
- Les connexions locales (`127.0.0.1`) sont approuvées automatiquement.
...
L'interface de contrôle peut se localiser automatiquement au premier chargement en fonction de la locale de votre navigateur, et vous pouvez la remplacer ultérieurement à partir du sélecteur de langue dans la carte Access.
...
- Sessions : liste + remplacements par session thinking/fast/verbose/reasoning (`sessions.list`, `sessions.patch`)
...
- Approbations Exec : modifier les listes blanches de gateway ou de nœud + demander la politique pour `exec host=gateway/node` (`exec.approvals.*`)
...
- Les écritures de config incluent une protection de hash de base pour éviter de écraser les modifications concurrentes
...
- Debug : snapshots statut/santé/modèles + journal des événements + appels RPC manuels (`status`, `health`, `models.list`)
- Logs : suivi en direct des journaux de fichiers de gateway avec filtre/export (`logs.tail`)
- Mise à jour : exécuter une mise à jour de package/git + redémarrer (`update.run`) avec un rapport de redémarrage
...
- Les contrôles d'édition avancée incluent la suppression après exécution, l'effacement du remplacement d'agent, les options cron exact/stagger,
  les remplacements de modèle/thinking d'agent, et les bascules de livraison au mieux.
...
- Définir `cron.webhookToken` pour envoyer un jeton bearer dédié, s'il est omis le webhook est envoyé sans en-tête d'authentification.
...
- Le renvoi avec la même `idempotencyKey` retourne `{ status: "in_flight" }` pendant l'exécution, et `{ status: "ok" }` après la fin.
...
- `chat.inject` ajoute une note d'assistant à la transcription de session et diffuse un événement `chat` pour les mises à jour de l'interface utilisateur uniquement (pas d'exécution d'agent, pas de livraison de canal).
- Arrêt :
...
- Rétention partielle d'avortement :
  - Lorsqu'une exécution est avortée, le texte assistant partiel peut toujours être affiché dans l'interface utilisateur
  - La Gateway persiste le texte assistant partiel avorté dans l'historique de transcription lorsqu'il existe une sortie mise en mémoire tampon
  - Les entrées persistées incluent les métadonnées d'avortement afin que les consommateurs de transcription puissent distinguer les partiels d'avortement de la sortie de fin normale
...
Collez le jeton dans les paramètres de l'interface utilisateur (envoyé comme `connect.params.auth.token`).
...
La Gateway sert les fichiers statiques à partir de `dist/control-ui`. Construisez-les avec :
...
```bash
pnpm ui:dev # installe automatiquement les dépendances de l'interface utilisateur au premier lancement
```

Pointez ensuite l'interface utilisateur sur votre URL Gateway WS (par ex. `ws://127.0.0.1:18789`).

## Débogage/test : serveur de développement + Gateway distante

L'interface de contrôle est des fichiers statiques ; la cible WebSocket est configurable et peut être
différente de l'origine HTTP. C'est pratique lorsque vous voulez le serveur Vite localement mais que la Gateway s'exécute ailleurs.
...
- Lorsque `gatewayUrl` est défini, l'interface utilisateur ne revient pas aux identifiants de config ou d'environnement.
...
- `gateway.controlUi.dangerouslyAllowHostHeaderOriginFallback=true` active
  le mode de fallback d'origine d'en-tête Host, mais c'est un mode de sécurité dangereux.
