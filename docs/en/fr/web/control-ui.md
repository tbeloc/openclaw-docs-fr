...
Control UI peut se localiser automatiquement au premier chargement en fonction de la locale de votre navigateur, et vous pouvez la modifier ultérieurement à partir du sélecteur de langue dans la carte Access.
...
- Approbations Exec : modifier les listes blanches de gateway ou de nœud + demander une politique pour `exec host=gateway/node` (`exec.approvals.*`)
...
- Rétention partielle d'avortement :
  ...
  - La Gateway persiste le texte assistant partiel avorté dans l'historique de transcription lorsqu'il existe une sortie mise en buffer
...
Puis pointez l'interface utilisateur vers votre URL Gateway WS (par ex. `ws://127.0.0.1:18789`).
...
Détails de configuration d'accès distant : [Accès distant](/fr/gateway/remote).
