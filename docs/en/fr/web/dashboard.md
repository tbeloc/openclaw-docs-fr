...
## Bases des jetons (local vs distant)
...
- Pour les étapes de réparation de dérive de jeton, suivez [Liste de contrôle de récupération de dérive de jeton](/cli/devices#token-drift-recovery-checklist).
- Récupérez ou fournissez le jeton depuis l'hôte de la passerelle :
  ...
  - Config gérée par SecretRef : résolvez le fournisseur de secret externe ou exportez `OPENCLAW_GATEWAY_TOKEN` dans ce shell, puis réexécutez `openclaw dashboard`
