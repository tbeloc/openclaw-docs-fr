...
- Pied de page : état de connexion + agent + session + modèle + think/fast/verbose/reasoning + compteurs de jetons + deliver.
...
- Paramètres : basculez deliver, expansion de la sortie d'outil et visibilité de la réflexion.
...
Les autres commandes slash de Gateway (par exemple, `/context`) sont transmises à la Gateway et affichées en tant que sortie système. Voir [Commandes slash](/tools/slash-commands).
...
- Pour forcer la palette sombre d'origine, définissez `OPENCLAW_THEME=dark`.
...
- `--session <key>` : clé de session (par défaut : `main`, ou `global` quand la portée est global)
...
- Aucun agent dans le sélecteur : vérifiez `openclaw agents list` et votre config de routage.
- Sélecteur de session vide : vous pourriez être dans la portée global ou n'avoir aucune session encore.
