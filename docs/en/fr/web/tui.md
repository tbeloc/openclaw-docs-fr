...
Les autres commandes slash de Gateway (par exemple, `/context`) sont transmises à la Gateway et affichées en tant que sortie système. Voir [Commandes slash](/fr/tools/slash-commands).
...
- Sélecteur de session vide : vous pourriez être dans une portée globale ou n'avoir aucune session pour le moment.
