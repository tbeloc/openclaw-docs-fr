...
- Authentification : `HUGGINGFACE_HUB_TOKEN` ou `HF_TOKEN` (token à granularité fine avec **Faire des appels aux Inference Providers**)
...
3. Dans le menu déroulant **Default Hugging Face model**, choisissez le modèle que vous souhaitez (la liste est chargée depuis l'API Inference lorsque vous avez un token valide ; sinon une liste intégrée est affichée). Votre choix est enregistré comme modèle par défaut.
...
(Optionnel : envoyez `Authorization: Bearer $HUGGINGFACE_HUB_TOKEN` ou `$HF_TOKEN` pour la liste complète ; certains points de terminaison retournent un sous-ensemble sans authentification.) La réponse est de style OpenAI `{ "object": "list", "data": [ { "id": "Qwen/Qwen3-8B", "owned_by": "Qwen", ... }, ... ] }`.
...
Les références de modèles utilisent la forme `huggingface/<org>/<model>` (IDs de style Hub). La liste ci-dessous provient de **GET** `https://router.huggingface.co/v1/models` ; votre catalogue peut en inclure davantage.
