...
OpenClaw peut également **découvrir automatiquement** les modèles disponibles à partir de vLLM lorsque vous optez pour `VLLM_API_KEY` (n'importe quelle valeur fonctionne si votre serveur n'applique pas l'authentification) et que vous ne définissez pas d'entrée `models.providers.vllm` explicite.
...
2. Optez pour (n'importe quelle valeur fonctionne si aucune authentification n'est configurée) :
...
- Votre serveur nécessite une clé API réelle (ou vous voulez contrôler les en-têtes).
