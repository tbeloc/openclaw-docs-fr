...
**Venice** est notre configuration Venice highlight pour l'inférence privacy-first avec accès optionnel anonymisé aux modèles propriétaires.

Venice AI fournit une inférence IA axée sur la confidentialité avec support des modèles non censurés et accès aux principaux modèles propriétaires via leur proxy anonymisé. Toute inférence est privée par défaut — pas d'entraînement sur vos données, pas de journalisation.
...
- Points de terminaison OpenAI-compatibles `/v1`.
...
| -------------- | --------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| **Private**    | Entièrement privé. Les invites/réponses ne sont **jamais stockées ou journalisées**. Éphémère.                                                       | Llama, Qwen, DeepSeek, Kimi, MiniMax, Venice Uncensored, etc. |
| **Anonymized** | Proxié via Venice avec métadonnées supprimées. Le fournisseur sous-jacent (OpenAI, Anthropic, Google, xAI) voit les demandes anonymisées. | Claude, GPT, Gemini, Grok                                     |
...
- **Pas de limites de débit strictes** : L'accélération à usage équitable peut s'appliquer pour une utilisation extrême
...
- **Option haute capacité** : `venice/claude-opus-4-6` pour le chemin Venice anonymisé le plus fort.
...
Le point de terminaison `/models` est public (pas d'authentification nécessaire pour lister), mais l'inférence nécessite une clé API valide.
...
| **Fonctionnalités** | La plupart des fonctionnalités supportées       | Fonctionnalités complètes       |
