...
- Coloration de niveau (info/warn/error)
...
- `compact` : sortie plus compacte (meilleure pour les longues sessions).
- `json` : JSON par ligne (pour les processeurs de journaux).

### Rédaction

Les résumés d'outils peuvent rédiger les jetons sensibles avant qu'ils n'atteignent la console :
...
- **OpenTelemetry (OTel)** : le modèle de données + SDKs pour les traces, métriques et journaux.
...
- **Traces** : spans pour l'utilisation de modèles + traitement des webhooks/messages.
...
- Les journaux de drapeaux vont au fichier journal standard (identique à `logging.file`).
...
- Les métriques incluent l'utilisation de jetons, le coût, la taille du contexte, la durée d'exécution et les compteurs/histogrammes de flux de messages (webhooks, mise en file d'attente, état de session, profondeur/attente de file d'attente).
- Les traces/métriques peuvent être basculées avec `traces` / `metrics` (par défaut : activé). Les traces incluent les spans d'utilisation de modèles plus les spans de traitement des webhooks/messages lorsqu'ils sont activés.
...
- Si le point de terminaison contient déjà `/v1/logs`, il est utilisé tel quel
