---
summary: "Cron jobs + wakeups for le planificateur Gateway"
read_when:
...
# Cron jobs (planificateur Gateway)

> **Cron vs Heartbeat?** Voir [Cron vs Heartbeat](/fr/automation/cron-vs-heartbeat) pour des conseils sur quand utiliser chacun.

Cron est le planificateur intégré du Gateway. Il persiste les jobs, réveille l'agent
au bon moment, et peut optionnellement livrer la sortie vers un chat.
...
- Deux styles d'exécution:
  - **Session principale**: enqueuer un événement système, puis exécuter au prochain heartbeat.
...
Créer un rappel unique, vérifier qu'il existe, et l'exécuter immédiatement:
...
Planifier un job isolé récurrent avec livraison:
...
Pour les formes JSON canoniques et les exemples, voir [Schéma JSON pour les appels d'outils](/fr/automation/cron-jobs#json-schema-for-tool-calls).

## Où les jobs cron sont stockés

Les jobs cron sont persistés sur l'hôte Gateway à `~/.openclaw/cron/jobs.json` par défaut.
...
Pensez à un job cron comme: **quand** exécuter + **quoi** faire.
...
- En pratique, le runtime intégré s'exécute avec `bootstrapContextMode: "lightweight"`, ce qui garde intentionnellement le contexte d'amorçage cron vide.
...
- `cron.run` (force ou due), `cron.runs`
  Pour les événements système immédiats sans job, utilisez [`openclaw system event`](/fr/cli/system).
...
- Les jobs ponctuels (`at`) réessaient les erreurs transitoires (limite de débit, surcharge, réseau, erreur_serveur) jusqu'à 3 fois avec backoff ; les erreurs permanentes désactivent immédiatement. Voir [Politique de retry](/fr/automation/cron-jobs#retry-policy).
