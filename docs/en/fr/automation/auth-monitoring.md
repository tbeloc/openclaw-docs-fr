...
read_when:
  ...
  - Automatisation des vérifications d'actualisation OAuth de Claude Code / Codex
title: "Surveillance de l'authentification"
---

# Surveillance de l'authentification

OpenClaw expose l'état de santé de l'expiration OAuth via `openclaw models status`. Utilisez cela pour
...
- `scripts/claude-auth-status.sh` utilise maintenant `openclaw models status --json` comme
  source de vérité (avec repli sur les lectures directes de fichiers si la CLI n'est pas disponible),
