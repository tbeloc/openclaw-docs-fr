...
- **Continuité conversationnelle** : Les heartbeats partagent la même session, l'agent se souvient donc des conversations récentes et peut faire un suivi naturel.
...
Voir [Tâches cron](/fr/automation/cron-jobs) pour la référence CLI complète.
...
A-t-il besoin d'un modèle différent ou d'un niveau de réflexion ?
...
- Le rappel/événement apparaître dans le contexte de la session principale
- L'agent le gérer lors du prochain heartbeat avec contexte complet
...
- [Tâches cron](/fr/automation/cron-jobs) - référence CLI et API cron complète
