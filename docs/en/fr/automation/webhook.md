...
- `hooks.allowedSessionKeyPrefixes` restreint optionnellement les valeurs explicites `sessionKey` des charges utiles de requête et des mappages.
...
- `openclaw webhooks gmail setup` écrit la configuration `hooks.gmail` pour `openclaw webhooks gmail run`.
  Voir [Gmail Pub/Sub](/fr/automation/gmail-pubsub) pour le flux Gmail watch complet.
