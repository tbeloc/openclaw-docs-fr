...
- **Webhooks** : webhooks HTTP externes qui permettent à d'autres systèmes de déclencher du travail dans OpenClaw. Voir [Webhook Hooks](/fr/automation/webhook) ou utiliser `openclaw webhooks` pour les commandes d'aide Gmail.

Les hooks peuvent également être regroupés à l'intérieur de plugins ; voir [Plugins](/fr/tools/plugin#plugin-hooks).
...
Chaque entrée `openclaw.hooks` doit rester à l'intérieur du répertoire du package après la résolution des symlinks ;
...
- **`tool_result_persist`** : transformer les résultats des outils avant qu'ils ne soient écrits dans la transcription de session. Doit être synchrone ; retournez la charge utile de résultat d'outil mise à jour ou `undefined` pour la conserver telle quelle. Voir [Boucle d'agent](/fr/concepts/agent-loop).
...
# Redémarrer votre processus de passerelle (redémarrage de l'application de la barre de menu sur macOS, ou redémarrage de votre processus de développement)
