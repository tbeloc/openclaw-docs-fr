...
<Note>
Si l'installation réussit mais que `openclaw` n'est pas trouvé dans un nouveau terminal, consultez [Dépannage Node.js](/fr/install/node#troubleshooting).
</Note>
...
<Steps>
  <Step title="Installer le runtime Node local">
    Télécharge une tarball Node supportée épinglée (actuellement par défaut `22.22.0`) vers `<prefix>/tools/node-v<version>` et vérifie SHA-256.
  </Step>
  <Step title="Assurer Git">
