...
- En mode distant, le répertoire d'état se trouve sur l'**hôte gateway**, donc exécutez également les étapes 1-4 là-bas.
...
Le label par défaut est `ai.openclaw.gateway` (ou `ai.openclaw.<profile>` ; les anciens `com.openclaw.*` peuvent toujours exister) :
...
Si vous exécutez à partir d'un checkout de repo (`git clone` + `openclaw ...` / `bun run openclaw ...`) :

1. Désinstallez le service gateway **avant** de supprimer le repo (utilisez le chemin facile ci-dessus ou la suppression manuelle du service).
2. Supprimez le répertoire du repo.
