...
**Note de sécurité :** La config par défaut expose une URL publique. Pour un déploiement renforcé sans IP publique, voir [Déploiement privé](#private-deployment-hardened) ou utiliser `fly.private.toml`.
...
- **Préférez les variables d'env au fichier config** pour toutes les clés API et jetons. Cela garde les secrets hors de `openclaw.json` où ils pourraient être accidentellement exposés ou enregistrés.
...
Si vous utilisez une variable d'env, pas besoin d'ajouter le jeton à la config. La gateway lit `DISCORD_BOT_TOKEN` automatiquement.
...
**Note :** 512 Mo est trop petit. 1 Go peut fonctionner mais peut faire un OOM sous charge ou avec un logging verbeux. **2 Go est recommandé.**
...
Si vous perdez les identifiants ou sessions après un redémarrage, le répertoire d'état écrit dans le système de fichiers du conteneur.
...
**Note :** Après `fly deploy`, la commande de machine peut réinitialiser à ce qui est dans `fly.toml`. Si vous avez fait des changements manuels, réappliquez-les après le déploiement.
...
- Vous ne faites que des appels **sortants** (pas de webhooks entrants)
...
1. **Tunnel ngrok** - Exécuter ngrok à l'intérieur du conteneur ou comme sidecar
...
Le tunnel ngrok s'exécute à l'intérieur du conteneur et fournit une URL webhook publique sans exposer l'app Fly elle-même. Définissez `webhookSecurity.allowedHosts` au nom d'hôte du tunnel public pour que les en-têtes d'hôte transférés soient acceptés.
...
| Accès interface de contrôle | Navigateur | Proxy/VPN  |
