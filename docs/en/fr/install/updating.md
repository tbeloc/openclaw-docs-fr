...
OpenClaw évolue rapidement (pré "1.0"). Traitez les mises à jour comme une infrastructure : mise à jour → vérifications → redémarrage (ou utilisez `openclaw update`, qui redémarre) → vérification.

## Recommandé : réexécuter l'installateur du site web (mise à jour sur place)

Le **chemin de mise à jour préféré** est de réexécuter l'installateur depuis le site web. Il
détecte les installations existantes, met à jour sur place et exécute `openclaw doctor` si
nécessaire.
...
- Créez un snapshot de votre personnalisation :
...
Pour basculer entre les canaux de mise à jour (installations git + npm) :
...
Utilisez `--tag <dist-tag|version>` pour une installation ponctuelle de tag/version.
...
- `stable` : quand une nouvelle version est détectée, OpenClaw attend `stableDelayHours` puis applique un jitter déterministe par installation dans `stableJitterHours` (déploiement échelonné).
...
- Si votre Gateway s'exécute en tant que service, `openclaw gateway restart` est préféré à la suppression des PIDs.
...
- Récupère + rebase par rapport à l'upstream configuré (canal dev).
...
- Si vous exécutez directement à partir de TypeScript (`pnpm openclaw ...`), une reconstruction est généralement inutile, mais **les migrations de config s'appliquent toujours** → exécutez doctor.
...
- Migrer les clés de config dépréciées / emplacements de fichiers de config hérités.
...
- Sur Linux, assurer la persistance utilisateur systemd (pour que la Gateway survive à la déconnexion).
...
- macOS launchd (LaunchAgent fourni avec l'app) : `launchctl kickstart -k gui/$UID/ai.openclaw.gateway` (utilisez `ai.openclaw.<profile>` ; `com.openclaw.*` hérité fonctionne toujours)
...
- Exécutez `openclaw doctor` à nouveau et lisez attentivement la sortie (elle vous dit souvent la solution).
