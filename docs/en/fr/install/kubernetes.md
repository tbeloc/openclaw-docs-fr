...
OpenClaw est un conteneur unique avec quelques fichiers de configuration. La personnalisation intéressante se trouve dans le contenu des agents (fichiers markdown, compétences, remplacements de configuration), pas dans la création de modèles d'infrastructure. Kustomize gère les superpositions sans la surcharge d'un graphique Helm. Si votre déploiement devient plus complexe, un graphique Helm peut être ajouté en couche au-dessus de ces manifestes.
...
./scripts/k8s/create-kind.sh --delete  # démanteler
```
...
- Gardez l'authentification de la passerelle activée et utilisez un point d'entrée approprié avec TLS terminé
