...
Si la construction échoue avec `Killed` ou `exit code 137` lors de `pnpm install --frozen-lockfile`, la VM n'a pas assez de mémoire.
...
| ------------------- | --------------------------------- | ------------------------ | -------------------------------- |
| Configuration de la passerelle | `/home/node/.openclaw/`           | Montage de volume hôte   | Inclut `openclaw.json`, tokens |
| Profils d'authentification du modèle | `/home/node/.openclaw/`           | Montage de volume hôte   | Tokens OAuth, clés API           |
