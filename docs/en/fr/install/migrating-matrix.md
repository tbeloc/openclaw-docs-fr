...
read_when:
  ...
  - Migration de l'historique Matrix chiffré et de l'état des appareils
title: "Migration Matrix"
---

# Migration Matrix

Cette page couvre les mises à niveau du plugin `matrix` public précédent vers l'implémentation actuelle.
...
Avant que toute étape de migration Matrix actionnable ne mute l'état sur disque, OpenClaw crée ou réutilise un snapshot de récupération ciblé.
...
- les installations du gestionnaire de paquets mettent à jour le paquet, exécutent une passe doctor non-interactive, puis s'appuient sur le redémarrage par défaut de la passerelle pour que le démarrage puisse terminer la migration Matrix
...
- l'extraction d'une clé de déchiffrement de sauvegarde de clé de salle Matrix précédemment enregistrée du ancien magasin de crypto rust, lorsque cette clé existe localement
...
- Si Matrix n'a que l'état de migration d'avertissement uniquement, par exemple parce que `userId` ou `accessToken` est toujours manquant, OpenClaw ne crée pas encore le snapshot car aucune mutation Matrix n'est actionnable.
...
Le plugin Matrix public précédent n'a **pas** créé automatiquement de sauvegardes de clé de salle Matrix. Il a persisté l'état de crypto local et demandé la vérification de l'appareil, mais il n'a pas garanti que vos clés de salle ont été sauvegardées sur le serveur d'accueil.
...
- les installations de chemin de plugin Matrix personnalisé sont signalées à la fois par le démarrage de la passerelle et par `openclaw doctor`
...
1. Mettez à jour OpenClaw et le plugin Matrix normalement.
   Préférez le simple `openclaw update` sans `--no-restart` pour que le démarrage puisse terminer la migration Matrix immédiatement.
2. Exécutez :
   ...
   Si Matrix a un travail de migration actionnable, doctor créera ou réutilisera le snapshot pré-migration en premier et imprimera le chemin de l'archive.
...
5. Si OpenClaw vous dit qu'une clé de récupération est nécessaire, exécutez :
...
7. Si vous abandonnez intentionnellement l'ancien historique irrécupérable et souhaitez une nouvelle ligne de base de sauvegarde pour les messages futurs, exécutez :
...
1. Le démarrage ou `openclaw doctor --fix` crée ou réutilise le snapshot pré-migration si la migration chiffrée est actionnable.
2. Le démarrage ou `openclaw doctor --fix` inspecte l'ancien magasin de crypto Matrix via l'installation du plugin Matrix actif.
...
- Signification : le nouvel emplacement délimité par compte a déjà un magasin de synchronisation ou de chiffrement, donc OpenClaw ne l'a pas écrasé automatiquement.
...
- Signification : OpenClaw a trouvé un ancien magasin Matrix chiffré, mais il n'y a pas de configuration Matrix actuelle à y attacher.
- À faire : configurez `channels.matrix`, puis réexécutez `openclaw doctor --fix` ou redémarrez la passerelle.
...
- À faire : démarrez la passerelle une fois avec une connexion Matrix fonctionnelle, ou réexécutez `openclaw doctor --fix` après que les identifiants en cache soient disponibles.
...
- Signification : OpenClaw a détecté l'ancien état Matrix, mais la migration est toujours bloquée sur les données d'identité ou d'identifiants manquants.
...
- À faire : réinstallez ou réparez le plugin Matrix (`openclaw plugins install @openclaw/matrix`, ou `openclaw plugins install ./extensions/matrix` pour un checkout de repo), puis réexécutez `openclaw doctor --fix` ou redémarrez la passerelle.
...
- À faire : inspectez les permissions du système de fichiers ou les conflits, conservez l'ancien état intact, et réessayez après avoir corrigé l'erreur.
...
- À faire : exécutez `openclaw matrix verify backup restore --recovery-key "<your-recovery-key>"`.
...
- À faire : réexécutez `openclaw doctor --fix`. S'il se répète, conservez le répertoire d'état ancien intact et récupérez en utilisant un autre client Matrix vérifié plus `openclaw matrix verify backup restore --recovery-key "<your-recovery-key>"`.
...
- À faire : exécutez `openclaw matrix verify backup status`, puis réessayez avec `openclaw matrix verify backup restore --recovery-key "<your-recovery-key>"` si nécessaire.
...
- À faire : exécutez `openclaw matrix verify backup restore`, ou passez `--recovery-key` si nécessaire.

`Store a recovery key with 'openclaw matrix verify device <key>', then run 'openclaw matrix verify backup restore'.`

- Signification : cet appareil n'a pas actuellement la clé de récupération stockée.
- À faire : vérifiez d'abord l'appareil avec votre clé de récupération, puis restaurez la sauvegarde.

`Backup key mismatch on this device. Re-run 'openclaw matrix verify device <key>' with the matching recovery key.`

- Signification : la clé stockée ne correspond pas à la sauvegarde Matrix active.
- À faire : réexécutez `openclaw matrix verify device "<your-recovery-key>"` avec la clé correcte.

Si vous acceptez de perdre l'ancien historique chiffré irrécupérable, vous pouvez à la place réinitialiser la ligne de base de sauvegarde actuelle avec `openclaw matrix verify backup reset --yes`.

`Backup trust chain is not verified on this device. Re-run 'openclaw matrix verify device <key>'.`

- Signification : la sauvegarde existe, mais cet appareil ne fait pas suffisamment confiance à la chaîne de signature croisée pour le moment.
- À faire : réexécutez `openclaw matrix verify device "<your-recovery-key>"`.
...
- À faire : réexécutez la commande avec votre clé de récupération.
...
- À faire : confirmez que vous avez utilisé la clé correcte et que la signature croisée est disponible sur le compte, puis réessayez.
...
- Signification : cet appareil ne peut pas restaurer à partir du stockage secret jusqu'à ce que la vérification de l'appareil soit terminée.
- À faire : exécutez d'abord `openclaw matrix verify device "<your-recovery-key>"`.
...
- À faire : réinstallez avec `openclaw plugins install @openclaw/matrix`, ou si vous exécutez à partir d'un checkout de repo, `openclaw plugins install ./extensions/matrix`.
...
Si la sauvegarde se restaure avec succès mais que certaines anciennes salles manquent toujours d'historique, ces clés manquantes n'ont probablement jamais été sauvegardées par le plugin précédent.

## Si vous voulez recommencer à zéro pour les messages futurs

Si vous acceptez de perdre l'historique chiffré ancien irrécupérable et que vous ne voulez qu'une sauvegarde de base propre à l'avenir, exécutez ces commandes dans l'ordre :
