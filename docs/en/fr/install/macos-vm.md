...
## Configuration par défaut recommandée (pour la plupart des utilisateurs)

- **Petit VPS Linux** pour une Gateway toujours active et un coût réduit. Voir [Hébergement VPS](/vps).
- **Matériel dédié** (Mac mini ou serveur Linux) si vous voulez un contrôle total et une **adresse IP résidentielle** pour l'automatisation de navigateur. De nombreux sites bloquent les adresses IP des data centers, donc la navigation locale fonctionne souvent mieux.
...
5. Connectez-vous via SSH, installez OpenClaw, configurez les canaux
6. Terminé
...
## 5) Connectez-vous via SSH à la VM
...
Pour une véritable disponibilité 24/7, envisagez un Mac mini dédié ou un petit VPS. Voir [Hébergement VPS](/vps).
...
| Problème                 | Solution                                                                                    |
| ----------------------- | ------------------------------------------------------------------------------------------- |
| Impossible de se connecter via SSH à la VM | Vérifiez que « Connexion à distance » est activée dans les Réglages Système de la VM       |
