...
read_when:
  ...
  - Vous voulez comparer les options de VM macOS locale vs hébergée
title: "VMs macOS"
---

# OpenClaw sur VMs macOS (Sandboxing)

## Recommandation par défaut (la plupart des utilisateurs)

- **Petit VPS Linux** pour une Gateway toujours active et un coût faible. Voir [Hébergement VPS](/vps).
- **Matériel dédié** (Mac mini ou boîte Linux) si vous voulez le contrôle total et une **adresse IP résidentielle** pour l'automatisation de navigateur. De nombreux sites bloquent les IPs de data center, donc la navigation locale fonctionne souvent mieux.
- **Hybride :** gardez la Gateway sur un VPS bon marché, et connectez votre Mac en tant que **nœud** quand vous avez besoin d'automatisation de navigateur/UI. Voir [Nœuds](/nodes) et [Gateway distante](/gateway/remote).

Utilisez une VM macOS quand vous avez spécifiquement besoin de capacités macOS uniquement (iMessage/BlueBubbles) ou voulez une isolation stricte de votre Mac quotidien.
...
- Aucun coût matériel supplémentaire ou cloud
...
## 8) Exécutez la VM sans interface
...
## Enregistrez une image dorée

Avant de personnaliser davantage, prenez un snapshot de votre état propre :
...
- Désactivant la mise en veille dans Réglages Système → Économies d'énergie
...
| IP de VM non affichée        | Attendez que la VM démarre complètement, exécutez `lume get openclaw` à nouveau             |
...
- [Hébergement VPS](/vps)
- [Nœuds](/nodes)
- [Gateway distante](/gateway/remote)
