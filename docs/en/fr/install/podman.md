...
`setup-podman.sh` met maintenant en scène le tar d'image dans un répertoire temporaire privé et imprime le répertoire de base choisi lors de la configuration. Pour les exécutions sans root, il accepte `TMPDIR` uniquement quand cette base est sûre à utiliser ; sinon, il revient à `/var/tmp`, puis `/tmp`. Le tar sauvegardé reste propriétaire uniquement et est diffusé dans le `podman load` de l'utilisateur cible, donc les répertoires temporaires privés de l'appelant ne bloquent pas la configuration.
