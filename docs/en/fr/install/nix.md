...
aussi activer le mode Nix via les defaults :
...
Si nécessaire, vous pouvez aussi définir `OPENCLAW_HOME` pour contrôler le répertoire home de base utilisé pour la résolution des chemins internes.
...
- L'interface affiche une bannière de mode Nix en lecture seule si présente
...
(ID du bundle, version/build, SHA Git, clés Sparkle). Cela garde le plist déterministe pour l'empaquetage SwiftPM
...
- [Wizard](/fr/start/wizard) — configuration CLI non-Nix
