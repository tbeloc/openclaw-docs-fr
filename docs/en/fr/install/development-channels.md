---
summary: "Canaux stables, bêta et dev : sémantique, commutation et étiquetage"
read_when:
...
Lorsque vous **explicitement** commutez les canaux avec `--channel`, OpenClaw aligne également
la méthode d'installation :

- `dev` assure un checkout git (par défaut `~/openclaw`, à remplacer par `OPENCLAW_GIT_DIR`),
...
Lorsque vous commutez les canaux avec `openclaw update`, OpenClaw synchronise également les sources de plugins :
...
- Les balises héritées `vYYYY.M.D-<patch>` sont toujours reconnues comme stables (non-bêta).
...
Les builds bêta et dev peuvent **ne pas** inclure de version d'application macOS. C'est normal :

- La balise git et le dist-tag npm peuvent toujours être publiés.
- Signalez « pas de build macOS pour cette bêta » dans les notes de version ou le changelog.
