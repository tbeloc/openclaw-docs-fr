...
Nous expédions les builds vers **beta**, les testons, puis **promouvons un build validé vers `latest`**
sans modifier le numéro de version — les dist-tags sont la source de vérité pour les installations npm.
...
- `dev` assure une extraction git (par défaut `~/openclaw`, à remplacer par `OPENCLAW_GIT_DIR`),
...
- `dev` préfère les plugins groupés de l'extraction git.
- `stable` et `beta` restaurent les packages de plugins installés via npm.

## Meilleures pratiques d'étiquetage

- Étiquetez les versions que vous voulez que les extractions git atteignent (`vYYYY.M.D` pour stable, `vYYYY.M.D-beta.N` pour beta).
- `vYYYY.M.D.beta.N` est également reconnu pour la compatibilité, mais préférez `-beta.N`.
- Les balises héritées `vYYYY.M.D-<patch>` sont toujours reconnues comme stable (non-beta).
