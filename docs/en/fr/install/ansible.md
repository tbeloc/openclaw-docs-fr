...
Remarque : La passerelle s'exécute **directement sur l'hôte** (pas dans Docker), mais les sandboxes d'agents utilisent Docker pour l'isolation. Voir [Sandboxing](/fr/gateway/sandboxing) pour plus de détails.
...
Docker est installé pour les **sandboxes d'agents** (exécution d'outils isolée), pas pour exécuter la passerelle elle-même. La passerelle se lie à localhost uniquement et est accessible via Tailscale VPN.
...
- Assurez-vous de pouvoir accéder via Tailscale VPN d'abord
