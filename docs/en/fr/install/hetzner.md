...
services:
  openclaw-gateway:
    ...
    ports:
      # Recommandé : garder la passerelle en boucle locale uniquement sur le VPS ; accédez via tunnel SSH.
...
## 7) Étapes du runtime Docker VM partagé

Utilisez le guide du runtime partagé pour le flux d'hôte Docker courant :
