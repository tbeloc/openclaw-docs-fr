...
# Workspace Memory v2 (hors ligne) : notes de recherche

Cible : espace de travail de style Clawd (`agents.defaults.workspace`, par défaut `~/.openclaw/workspace`) où la « mémoire » est stockée sous forme d'un fichier Markdown par jour (`memory/YYYY-MM-DD.md`) plus un petit ensemble de fichiers stables (par exemple `memory.md`, `SOUL.md`).

Ce document propose une **architecture mémoire hors ligne d'abord** qui garde Markdown comme source de vérité canonique et vérifiable, mais ajoute un **rappel structuré** (recherche, résumés d'entités, mises à jour de confiance) via un index dérivé.
...
- capture sans friction (« il suffit de l'écrire »)
...
- réponses centrées sur les entités (« parlez-moi d'Alice / The Castle / warelay ») sans relire plusieurs fichiers
...
- **Hors ligne** : fonctionne sans réseau ; peut s'exécuter sur un ordinateur portable/Castle ; aucune dépendance cloud.
...
- **Convivial pour les agents** : rend facile le « rappel dans les budgets de jetons » (retourner de petits paquets de faits).
...
- **FTS5** SQLite pour le rappel lexical (rapide, minuscule, hors ligne)
...
Le format de retour doit être convivial pour les agents et citer les sources :
...
- met à jour `bank/entities/*.md` à partir de faits récents (résumés d'entités)
...
- chaque opinion a :
  ...
  - confiance `c ∈ [0,1]`
  - last_updated
  - liens de preuves (IDs de faits soutenant + contredisant)
...
- réutiliser à partir d'autres contextes (scripts locaux, future app de bureau, etc.)
...
- commencez avec SQLite FTS + (optionnel) embeddings simples ; vous obtiendrez la plupart des gains UX immédiatement.
- considérez les solutions de classe SuCo/HNSW/ScaNN uniquement une fois :
...
- quel est le **meilleur** modèle d'embedding hors ligne pour « mémoire d'assistant personnel » sur vos machines (ordinateur portable + bureau) ?
  - si vous avez déjà Ollama : intégrez avec un modèle local ; sinon livrez un petit modèle d'embedding dans la chaîne d'outils.
...
- Concepts Letta / MemGPT : « blocs de mémoire centrale » + « mémoire d'archivage » + mémoire d'auto-édition pilotée par outils.
