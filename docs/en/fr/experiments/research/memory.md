...
Ce document propose une architecture mémoire **hors ligne d'abord** qui garde Markdown comme source de vérité canonique et révisable, mais ajoute une **récupération structurée** (recherche, résumés d'entités, mises à jour de confiance) via un index dérivé.
...
- **Hors ligne** : fonctionne sans réseau ; peut s'exécuter sur un ordinateur portable/Château ; aucune dépendance au cloud.
- **Explicable** : les éléments récupérés doivent être attribuables (fichier + localisation) et séparables de l'inférence.
- **Peu de cérémonie** : la journalisation quotidienne reste Markdown, aucun travail de schéma lourd.
...
Si « S-Collide » fait référence à **SuCo (Subspace Collision)** : c'est une approche de récupération ANN qui cible des compromis forts rappel/latence en utilisant des collisions apprises/structurées dans des sous-espaces (article : arXiv 2411.14754, 2024).
