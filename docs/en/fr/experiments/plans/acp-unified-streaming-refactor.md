---
summary: "Plan de refonte holy grail pour un pipeline de streaming d'exécution unifié sur main, subagent et ACP"
owner: "onutc"
status: "draft"
...
- Rejeter les événements de runtime mal formés rapidement et exposer les diagnostics structurés.

### 2) Processeur de flux partagé

- Remplacer la logique de coalescence/projecteur spécifique au runtime par un processeur unique.
- Le processeur gère la mise en buffer des deltas texte, le flush inactif, le fractionnement max-chunk et le flush de fin.
- Déplacer la résolution de configuration ACP/main/subagent dans un helper unique pour éviter la dérive.

### 3) Projection de canal partagée

- Garder les adaptateurs de canal simples : accepter les blocs finalisés et envoyer.
- Déplacer les quirks de chunking spécifiques à Discord vers le projecteur de canal uniquement.
...
- Aucune expansion de fonctionnalités spécifiques au canal en dehors des correctifs de compatibilité de projection.
...
- La relecture de crash/redémarrage n'envoie aucun chunk dupliqué pour le même ID de livraison.
