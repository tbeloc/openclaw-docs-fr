...
read_when:
  - Refactorisation du routage et des liaisons de session agnostiques des canaux
...
- livraison dupliquée du canal principal et du fil sous des exécutions concurrentes
...
- actualiser l'utilisation des jetons lors de la réutilisation du gestionnaire de liaisons de fil existant
- enregistrer l'activité sortante pour les envois Discord basés sur webhook
- arrêter le repli implicite du canal principal lorsqu'une destination de fil liée est sélectionnée pour l'achèvement en mode session
...
- le nouveau chemin de base affecte uniquement le routage d'achèvement de session lié où activé
...
- si l'achèvement en mode session se résout à une destination liée active, la livraison doit cibler cette destination
...
2. Router les livraisons d'achèvement Discord en mode lié via le routeur.
...
- les envois de webhook mettent à jour les horodatages d'activité du canal
- deux sessions liées actives dans le même canal demandeur ne dupliquent pas vers le canal principal
- l'achèvement pour l'exécution en mode session lié se résout uniquement à la destination du fil
...
- aucune livraison d'achèvement dupliquée du canal principal et du fil dans les exécutions liées en mode session
