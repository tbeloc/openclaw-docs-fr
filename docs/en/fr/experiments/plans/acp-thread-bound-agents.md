...
- [Plan de refactorisation du streaming unifié du runtime](/experiments/plans/acp-unified-streaming-refactor)
...
- Le backend du runtime ACP est enfichable
  ...
  - le runtime effectue le transport ACP, la mise en file d'attente, l'annulation, la reconnexion
...
Traiter ACP comme un plan de contrôle de première classe dans OpenClaw, avec des adaptateurs de runtime enfichables.
...
- sémantique de débind du cycle de vie sur archive, suppression, unfocus, reset et suppression
...
- sémantique de débind du cycle de vie et détachement du runtime liés au reset/suppression de session
...
Ajouter un contrat de runtime ACP core afin que le code d'acheminement ne dépende pas des détails CLI et puisse basculer les backends sans modifier la logique de dispatch :
...
- le core résout le runtime via le registre et échoue avec une erreur d'opérateur explicite quand aucun backend de runtime ACP n'est disponible
...
- si la cible de liaison est une session ACP, acheminer vers la branche de runtime ACP au lieu de `getReplyFromConfig`
...
- backoff de redémarrage borné pour les crashes du runtime
...
- `src/discord/monitor/reply-delivery.test.ts` (comportement de destination de livraison ACP liée)
- `src/discord/monitor/message-handler.preflight*.test.ts` (continuité du routage de clé de session ACP liée)
...
- normaliser les événements acpx ndjson en événements runtime ACP
...
- publier le runbook pour l'indisponibilité du backend, la corruption DB et la correction des liaisons obsolètes
...
- Confusion de configuration entre la sous-agent et les portes ACP
  - Atténuation : clés ACP explicites et retour de commande incluant la source de politique effective
...
- les liaisons obsolètes sont détectées et surfacées explicitement (avec nettoyage automatique sûr optionnel)
- les métriques et diagnostics du plan de contrôle sont disponibles pour les opérateurs
- la nouvelle couverture unitaire, d'intégration et e2e réussit

## Addendum : refactorisations ciblées pour l'implémentation actuelle (statut)

Ce sont des suites non bloquantes pour maintenir le chemin ACP maintenable après l'arrivée de l'ensemble de fonctionnalités actuel.
...
- le dispatch, les gestionnaires de cycle de vie des commandes ACP et le chemin de spawn ACP consomment désormais la logique de politique partagée
