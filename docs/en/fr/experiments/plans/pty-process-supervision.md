...
- Le module Supervisor est en place sous `src/process/supervisor/*`.
- L'exec runtime et le CLI runner sont déjà routés via le spawn et l'attente du superviseur.
...
4. Défauts de watchdog à source unique
...
- Ajout d'un utilitaire process-tree partagé dans `src/process/kill-tree.ts`.
...
1. `runExecProcess` dans `src/agents/bash-tools.exec-runtime.ts` gère toujours plusieurs responsabilités et peut être divisé en helpers ciblés dans une suite.
...
- diviser `runExecProcess` en helpers ciblés sans dérive de comportement
...
- `src/agents/bash-tools.shared.ts` chemin helper kill direct supprimé.
...
- L'adaptateur PTY garantit toujours le règlement de l'attente sur kill forcé et la suppression du listener.
...
4. Les défauts de watchdog sont à source unique.
