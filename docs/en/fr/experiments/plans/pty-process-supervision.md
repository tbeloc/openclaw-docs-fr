---
summary: "Plan de production pour une supervision de processus interactive fiable (PTY + non-PTY) avec propriété explicite, cycle de vie unifié et nettoyage déterministe"
read_when:
...
- L'exécution exec et le runner CLI sont déjà routés via spawn et wait du superviseur.
...
- `src/agents/bash-tools.process.ts` revient maintenant à la terminaison de processus réelle au niveau du système d'exploitation en cas d'absence de recherche du superviseur.
- `src/process/supervisor/adapters/child.ts` utilise maintenant la sémantique de terminaison process-tree pour les chemins d'annulation/délai d'expiration par défaut.
- Ajout d'utilitaire process-tree partagé dans `src/process/kill-tree.ts`.

8. Couverture de cas limite du contrat PTY ajoutée
...
- Ajout de `src/process/supervisor/adapters/child.test.ts` pour le comportement de suppression process-tree dans l'annulation de l'adaptateur enfant.
...
- L'annulation/délai d'expiration enfant utilise maintenant la sémantique de suppression process-tree pour le chemin de suppression par défaut.
...
- annulation process-tree pour le chemin de suppression par défaut de l'adaptateur enfant
- tests de régression pour le secours de suppression et le chemin de suppression de l'adaptateur enfant
- tests de cas limite de commande PTY sous `ptyCommand` explicite
...
- L'adaptateur PTY garantit toujours le règlement d'attente lors de la suppression forcée et de l'élimination de l'écouteur.
...
- stratification de processus plus propre
...
- annulation process-tree pour les chemins de suppression par défaut des exécutions enfants
- défauts de surveillance unifiés
- limite de redémarrage en mémoire explicite (pas de réconciliation d'orphelins entre redémarrages dans cette passe)
