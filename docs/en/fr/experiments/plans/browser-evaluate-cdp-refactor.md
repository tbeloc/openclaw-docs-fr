...
La PR #13498 ajoute un filet de sécurité pragmatique (évaluation bornée, propagation d'abandon et récupération au mieux).
Ce document décrit un refactorisation plus large qui rend `act:evaluate` intrinsèquement
...
- Le ciblage d'éléments pour l'évaluation est pris en charge sans tout basculer de Playwright.
...
- Ajouter l'isolation de processus (processus worker/thread) pour l'évaluation. Si nous voyons toujours des états
...
- Une file d'attente bloquée signifie que les opérations de clic/saisie/attente ultérieures sur l'onglet peuvent sembler se bloquer.
...
Ajouter une implémentation d'évaluation basée sur CDP qui ne partage pas la file d'attente des commandes par page de Playwright.
La propriété clé est que le transport d'évaluation est une connexion WebSocket séparée
...
- Cela exécute toujours du JavaScript dans la page, donc la terminaison peut avoir des effets secondaires. Le gain
  est qu'elle ne coince pas la file d'attente Playwright, et elle est annulable au niveau du transport
  en tuant la session CDP.

### 3. Récit de Ref (Ciblage d'Éléments Sans Réécriture Complète)

La partie difficile est le ciblage d'éléments. CDP a besoin d'une poignée DOM ou d'un `backendDOMNodeId`, tandis que
aujourd'hui la plupart des actions de navigateur utilisent des localisateurs Playwright basés sur des refs des snapshots.

Approche recommandée : conserver les refs existantes, mais joindre un id CDP résolvable optionnel.
...
1. Générer la carte de ref de rôle existante comme aujourd'hui (rôle, nom, nième).
...
3. Fusionner l'id dans les informations de ref stockées pour l'onglet actuel.

Si le mappage échoue pour une ref, laisser `backendDOMNodeId` indéfini. Cela rend la fonctionnalité
au mieux et sûre à déployer.

#### 3.3 Comportement d'Évaluation Avec Ref
...
- Étendre la forme de requête pour accepter `backendDOMNodeId` directement pour les appelants avancés (et
  pour le débogage), tout en gardant `ref` comme interface principale.

### 4. Garder un Chemin de Récupération de Dernier Recours
...
- Un budget de délai d'expiration/abandon unique de bout en bout utilisé de manière cohérente par les appelants et les gestionnaires.
- Métadonnées de ref qui peuvent optionnellement porter `backendDOMNodeId` pour l'évaluation d'élément.
- `act:evaluate` préfère le moteur CDP quand c'est possible et revient à Playwright quand ce n'est pas le cas.
...
3. Implémenter `src/browser/cdp-evaluate.ts` :
   ...
   - en cas de délai d'expiration/abandon : meilleur effort `Runtime.terminateExecution` puis fermer le socket
4. Étendre les métadonnées de ref de rôle stockées pour inclure optionnellement `backendDOMNodeId` :
   - garder le comportement existant `{ role, name, nth }` pour les actions Playwright
   - ajouter `backendDOMNodeId?: number` pour le ciblage d'élément CDP
5. Remplir `backendDOMNodeId` lors de la création du snapshot (meilleur effort) :
...
7. Garder le chemin de récupération "dernier recours" existant comme repli, pas le chemin par défaut.
8. Ajouter des tests :
   - l'évaluation bloquée intentionnellement expire dans le budget et le prochain clic/saisie réussit
   - l'abandon annule l'évaluation (déconnexion du client ou délai d'expiration) et déverrouille les actions ultérieures
   - les défaillances de mappage reviennent proprement à Playwright
9. Ajouter l'observabilité :
   - compteurs de durée d'évaluation et de délai d'expiration
   - utilisation de terminateExecution
   - taux de repli (CDP -> Playwright) et raisons

### Critères d'Acceptation

- Une `act:evaluate` intentionnellement bloquée retourne dans le budget de l'appelant et ne coince pas l'onglet
...
- Tests unitaires :
  - logique de correspondance `(role, name, nth)` entre les refs de rôle et les nœuds d'arbre AX.
  - comportement de l'assistant de budget (marge de manœuvre, mathématiques du temps restant).
- Tests d'intégration :
  - l'évaluation CDP avec délai d'expiration retourne dans le budget et ne bloque pas l'action suivante.
...
- `Runtime.terminateExecution` a des effets secondaires :
  - Atténuation : utiliser uniquement en cas de délai d'expiration/abandon et documenter le comportement dans les erreurs.
