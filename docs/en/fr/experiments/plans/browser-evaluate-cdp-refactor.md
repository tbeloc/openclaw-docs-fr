...
- Le ciblage d'éléments pour evaluate est pris en charge sans désactiver tout Playwright.
...
- Cela exécute toujours du JavaScript dans la page, donc la terminaison peut avoir des effets secondaires. Le gain
  est qu'elle ne coince pas la file d'attente Playwright, et elle est annulable au niveau du transport en tuant la session CDP.
...
- Un moteur d'évaluation CDP qui s'exécute en dehors de la file d'attente des commandes par page Playwright.
...
3. Implémenter `src/browser/cdp-evaluate.ts` :
   ...
   - en cas de timeout/abandon : meilleur effort `Runtime.terminateExecution` puis fermer le socket
...
8. Ajouter des tests :
   ...
   - l'abandon annule l'évaluation (déconnexion client ou timeout) et déverrouille les actions ultérieures
...
- Tests d'intégration :
  - l'évaluation CDP timeout retourne dans le budget et ne bloque pas l'action suivante.
...
- Le nouveau moteur devrait-il être configurable comme `playwright`, `cdp` ou `auto` ?
- Voulons-nous exposer un nouveau format "nodeRef" pour les utilisateurs avancés, ou garder `ref` uniquement ?
- Comment les snapshots de cadre et les snapshots délimités par sélecteur devraient-ils participer au mappage AX ?
