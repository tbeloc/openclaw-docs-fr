...
evaluate bloqué ou longue durée peut bloquer la file d'attente des commandes de la page et rendre chaque action ultérieure
sur cet onglet "bloquée".

La PR #13498 ajoute un filet de sécurité pragmatique (evaluate borné, propagation d'abandon et récupération
au mieux). Ce document décrit un refactorisation plus large qui rend `act:evaluate` intrinsèquement
isolé de Playwright afin qu'un evaluate bloqué ne puisse pas coincer les opérations Playwright normales.

## Objectifs

- `act:evaluate` ne peut pas bloquer définitivement les actions de navigateur ultérieures sur le même onglet.
- Les timeouts sont une source unique de vérité de bout en bout afin qu'un appelant puisse compter sur un budget.
- L'abandon et le timeout sont traités de la même manière dans la distribution HTTP et en processus.
- Le ciblage d'éléments pour evaluate est supporté sans tout basculer de Playwright.
...
- Le timeout de la requête externe, la logique du gestionnaire de route et le budget d'exécution à l'intérieur de la page
...
Ajouter une implémentation d'evaluate basée sur CDP qui ne partage pas la file d'attente des commandes par page de Playwright. La propriété clé est que le transport d'evaluate est une connexion WebSocket séparée
...
Cela garde toutes les actions basées sur Playwright existantes fonctionnelles et permet à l'evaluate CDP d'accepter
...
#### 3.3 Comportement d'Evaluate avec référence

Dans `act:evaluate` :

- Si `ref` est présent et a `backendDOMNodeId`, exécuter l'evaluate d'élément via CDP.
...
Même avec l'evaluate CDP, il y a d'autres façons de coincer un onglet ou une connexion. Garder les
...
- Un moteur d'evaluate basé sur CDP qui s'exécute en dehors de la file d'attente des commandes par page de Playwright.
- Un budget de timeout/abandon de bout en bout unique utilisé de manière cohérente par les appelants et les gestionnaires.
- Métadonnées de référence qui peuvent optionnellement porter `backendDOMNodeId` pour l'evaluate d'élément.
- `act:evaluate` préfère le moteur CDP quand possible et revient à Playwright quand ce n'est pas le cas.
- Tests qui prouvent qu'un evaluate bloqué ne coince pas les actions ultérieures.
...
6. Mettre à jour le routage `act:evaluate` :
   ...
   - si `ref` se résout en `backendDOMNodeId` : utiliser l'evaluate d'élément CDP
   - sinon : revenir à l'evaluate Playwright (toujours borné et annulable)
7. Garder le chemin de récupération "en dernier recours" existant comme repli, pas le chemin par défaut.
8. Ajouter des tests :
   - l'evaluate bloquée intentionnellement expire dans le budget et le prochain clic/saisie réussit
   - l'abandon annule l'evaluate (déconnexion du client ou timeout) et déverrouille les actions ultérieures
   - les défaillances de mappage reviennent proprement à Playwright
9. Ajouter l'observabilité :
   - compteurs de durée et de timeout d'evaluate
   - utilisation de terminateExecution
   - taux de repli (CDP -> Playwright) et raisons

### Critères d'acceptation

- Un `act:evaluate` intentionnellement bloqué retourne dans le budget de l'appelant et ne coince pas l'onglet
...
- Si `ref` peut être mappé à `backendDOMNodeId`, l'evaluate d'élément utilise CDP ; sinon le
  chemin de repli est toujours borné et récupérable.

## Plan de test

- Tests unitaires :
  - logique de correspondance `(role, name, nth)` entre les références de rôle et les nœuds d'arbre AX.
  - comportement de l'assistant budget (marge de manœuvre, mathématiques du temps restant).
- Tests d'intégration :
  - l'evaluate CDP timeout retourne dans le budget et ne bloque pas l'action suivante.
...
- Le mappage est imparfait :
  - Atténuation : mappage au mieux, repli à l'evaluate Playwright et ajout d'outils de débogage.
