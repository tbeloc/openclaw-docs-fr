...
2. Une file d'attente d'exécution Discord stocke les tâches sérialisées avec une clé basée sur la même limite de classement que celle utilisée aujourd'hui.
...
C'est la solution à long terme pour les exécutions Discord en file d'attente qui expirent à `channels.discord.eventQueue.listenerTimeout` alors que l'exécution de l'agent elle-même progresse toujours.
...
- Une couverture de régression de délai d'expiration existe pour le chemin du worker Discord.
...
- Ne pas ajouter de récupération de crash durable dans la première passe sauf si nécessaire pour un atterrissage sûr.
...
- charger les médias et toute métadonnée de canal supplémentaire nécessaire pour l'exécution
...
- `new`
- `reset`
- toute commande de contrôle de session future

La file d'attente du worker doit exposer suffisamment d'état d'exécution pour que les commandes ciblent l'exécution active ou en file d'attente.
...
Cela devrait être un suivi séparé sauf si la récupération de redémarrage est requise pour un atterrissage.
...
3. Réduire les tâches en file d'attente à des données pures spécifiques à Discord :
   ...
   - décisions de contrôle d'accès et de liaison
...
- `guildHistories`
- `discordRestFetch`
- autres poignées mutables réservées à l'exécution

ne devraient plus vivre sur chaque tâche en file d'attente et vivre à la place sur le worker lui-même ou derrière des adaptateurs possédés par le worker.
...
3. différents canaux s'exécutent toujours en concurrence
...
- Risque : la livraison de réponse perd le contexte du fil ou de la réponse
  Atténuation : rendre l'identité de livraison de première classe dans `DiscordInboundJob`

- Risque : envois en double lors de tentatives ou de redémarrages de file d'attente
