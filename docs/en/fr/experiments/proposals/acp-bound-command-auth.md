...
Aujourd'hui, nous avons des vérifications spécifiques aux commandes (par exemple `/new` et `/reset`) qui doivent fonctionner dans les canaux/sujets liés à ACP même lorsque les listes blanches sont vides. Cela résout la douleur UX immédiate, mais les exceptions basées sur le nom de commande ne s'adaptent pas bien.
...
Introduire un assistant qui évalue l'authentification de commande en utilisant :
...
- S'adapte aux commandes futures sans ajouter plus de conditions basées sur le nom de commande.
