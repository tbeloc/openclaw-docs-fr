...
<Tabs>
  <Tab title="Démarrage rapide (paramètres par défaut)">
    ...
    - Isolation DM par défaut : l'intégration CLI locale écrit `session.dmScope: "per-channel-peer"` lorsqu'il n'est pas défini. Détails : [Référence d'intégration CLI](/start/wizard-cli-reference#outputs-and-internals)
