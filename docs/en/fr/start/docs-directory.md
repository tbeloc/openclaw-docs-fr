---
summary: "Liens organisés vers les documents OpenClaw les plus utilisés."
read_when:
  - You want quick access to key docs pages
title: "Répertoire des documents"
---

<Note>
Cette page est un index organisé. Si vous êtes nouveau, commencez par [Getting Started](/start/getting-started).
Pour une carte complète de la documentation, consultez [Docs hubs](/start/hubs).
</Note>

## Commencer ici

- [Docs hubs (toutes les pages liées)](/start/hubs)
- [Aide](/help)
- [Configuration](/gateway/configuration)
- [Exemples de configuration](/gateway/configuration-examples)
- [Commandes slash](/tools/slash-commands)
- [Routage multi-agent](/concepts/multi-agent)
- [Mise à jour et restauration](/install/updating)
- [Appairage (DM et nœuds)](/channels/pairing)
- [Mode Nix](/install/nix)
- [Configuration de l'assistant OpenClaw](/start/openclaw)
- [Compétences](/tools/skills)
- [Configuration des compétences](/tools/skills-config)
- [Modèles d'espace de travail](/reference/templates/AGENTS)
- [Adaptateurs RPC](/reference/rpc)
- [Guide opérationnel de la passerelle](/gateway)
- [Nœuds (iOS et Android)](/nodes)
- [Surfaces web (Interface de contrôle)](/web)
- [Découverte et transports](/gateway/discovery)
- [Accès à distance](/gateway/remote)

## Fournisseurs et UX

- [WebChat](/web/webchat)
- [Interface de contrôle (navigateur)](/web/control-ui)
- [Telegram](/channels/telegram)
- [Discord](/channels/discord)
- [Mattermost (plugin)](/channels/mattermost)
- [BlueBubbles (iMessage)](/channels/bluebubbles)
- [iMessage (hérité)](/channels/imessage)
- [Groupes](/channels/groups)
- [Messages de groupe WhatsApp](/channels/group-messages)
- [Images médias](/nodes/images)
- [Audio médias](/nodes/audio)

## Applications complémentaires

- [Application macOS](/platforms/macos)
- [Application iOS](/platforms/ios)
- [Application Android](/platforms/android)
- [Windows (WSL2)](/platforms/windows)
- [Application Linux](/platforms/linux)

## Opérations et sécurité

- [Sessions](/concepts/session)
- [Tâches cron](/automation/cron-jobs)
- [Webhooks](/automation/webhook)
- [Crochets Gmail (Pub/Sub)](/automation/gmail-pubsub)
- [Sécurité](/gateway/security)
- [Dépannage](/gateway/troubleshooting)
