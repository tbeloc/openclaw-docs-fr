...
Pour plus de détails sur l'assistant, voir [Assistant d'intégration](/fr/start/wizard).
</Note>
...
- Node `>=22`
- `pnpm`
- Docker (optionnel ; uniquement pour la configuration conteneurisée/e2e — voir [Docker](/fr/install/docker))
...
Utilisez ceci lors du débogage d'authentification ou pour décider ce qu'il faut sauvegarder :
...
- **Jeton bot Discord** : config/env ou SecretRef (fournisseurs env/fichier/exec)
...
Pour les serveurs toujours actifs ou multi-utilisateurs, envisagez un service **système** au lieu d'un service utilisateur (pas besoin de persistance). Voir [Runbook Gateway](/fr/gateway) pour les notes systemd.

## Documentation connexe

- [Runbook Gateway](/fr/gateway) (drapeaux, supervision, ports)
...
- [App macOS](/fr/platforms/macos) (cycle de vie de la gateway)
