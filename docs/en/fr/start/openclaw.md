...
- Les pulsations cardiaques sont maintenant définies par défaut toutes les 30 minutes. Désactivez jusqu'à ce que vous fassiez confiance à la configuration en définissant `agents.defaults.heartbeat.every: "0m"`.

## Prérequis

- OpenClaw installé et intégré — consultez [Démarrage](/fr/start/getting-started) si vous ne l'avez pas encore fait
...
Si vous livrez déjà vos propres fichiers d'espace de travail à partir d'un dépôt, vous pouvez désactiver entièrement la création de fichiers de démarrage :
...
- pulsations cardiaques (une fois que vous en êtes sûr)
...
## Pulsations cardiaques (mode proactif)

Par défaut, OpenClaw exécute une pulsation cardiaque toutes les 30 minutes avec l'invite :
...
- Si `HEARTBEAT.md` existe mais est effectivement vide (uniquement des lignes vides et des en-têtes markdown comme `# Heading`), OpenClaw ignore l'exécution de la pulsation cardiaque pour économiser les appels API.
- Si le fichier est manquant, la pulsation cardiaque s'exécute quand même et le modèle décide quoi faire.
- Si l'agent répond avec `HEARTBEAT_OK` (optionnellement avec un court remplissage ; voir `agents.defaults.heartbeat.ackMaxChars`), OpenClaw supprime la livraison sortante pour cette pulsation cardiaque.
- Par défaut, la livraison de pulsation cardiaque aux cibles de style DM `user:<id>` est autorisée. Définissez `agents.defaults.heartbeat.directPolicy: "block"` pour supprimer la livraison de cible directe tout en gardant les exécutions de pulsation cardiaque actives.
- Les pulsations cardiaques exécutent des tours d'agent complets — les intervalles plus courts consomment plus de tokens.
...
- Opérations de passerelle : [Gateway runbook](/fr/gateway)
- Cron + réveils : [Cron jobs](/fr/automation/cron-jobs)
- Compagnon de barre de menu macOS : [OpenClaw macOS app](/fr/platforms/macos)
...
- Statut Linux : [Linux app](/fr/platforms/linux)
