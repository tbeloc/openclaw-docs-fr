...
Ce document décrit le flux d'intégration au premier lancement **actuel**. L'objectif est une expérience fluide au « jour 0 » : choisir où la Gateway s'exécute, connecter l'authentification, exécuter l'assistant et laisser l'agent s'initialiser lui-même.
Pour un aperçu général des chemins d'intégration, voir [Aperçu de l'intégration](/start/onboarding-overview).
...
- Les configurations partagées/multi-utilisateurs nécessitent un verrouillage (limites de confiance divisées, accès aux outils minimal, et suivre [Sécurité](/gateway/security)).
...
- Si les hooks/webhooks ou d'autres flux de contenu non fiables sont activés, utilisez un niveau de modèle moderne fort et maintenez une politique d'outils/sandboxing stricte.
...
- Utilisez un **token** pour l'accès multi-machines ou les liaisons non-boucle locale.
