---
summary: "Référence complète pour le flux d'intégration CLI, configuration auth/modèle, sorties et mécanismes internes"
read_when:
...
<Steps>
  <Step title="Détection de configuration existante">
    ...
    - CLI `--reset` par défaut à `config+creds+sessions` ; utilisez `--reset-scope full` pour supprimer aussi l'espace de travail.
  ...
  <Step title="Passerelle">
    ...
    - En mode mot de passe, l'intégration interactive supporte aussi le stockage en texte brut ou SecretRef.
  ...
  <Step title="Canaux">
    ...
    - [BlueBubbles](/channels/bluebubbles): recommandé pour iMessage ; URL serveur + mot de passe + webhook
    - [iMessage](/channels/imessage): chemin CLI `imsg` hérité + accès BD
...
Si les ressources de l'interface de contrôle sont manquantes, l'assistant tente de les construire ; le recours est `pnpm ui:build` (installe automatiquement les dépendances UI).
</Note>
...
- URL de passerelle distante (`ws://...`)
- Jeton si l'authentification de passerelle distante est requise (recommandé)

<Note>
- Si la passerelle est boucle-locale uniquement, utilisez le tunneling SSH ou un tailnet.
...
<AccordionGroup>
  <Accordion title="Clé API Anthropic">
    Utilise `ANTHROPIC_API_KEY` si présente ou demande une clé, puis l'enregistre pour utilisation par le démon.
  </Accordion>
  <Accordion title="OAuth Anthropic (Claude Code CLI)">
    ...
    Sur macOS, choisissez "Always Allow" afin que les démarrages launchd ne se bloquent pas.
  ...
  <Accordion title="Abonnement OpenAI Code (OAuth)">
    ...
    Définit `agents.defaults.model` à `openai-codex/gpt-5.4` quand le modèle n'est pas défini ou `openai/*`.
  ...
  <Accordion title="Clé API OpenAI">
    ...
    Définit `agents.defaults.model` à `openai/gpt-5.1-codex` quand le modèle n'est pas défini, `openai/*`, ou `openai-codex/*`.
  ...
  <Accordion title="Ollama (Modèles ouverts Cloud et locaux)">
    Demande l'URL de base (par défaut `http://127.0.0.1:11434`), puis offre le mode Cloud + Local ou Local.
...
Chemins de profil et d'identifiant :
...
Mode de stockage d'identifiant :
...
- `tools.profile` (l'intégration locale utilise par défaut `"coding"` quand non défini ; les valeurs explicites existantes sont conservées)
- `gateway.*` (mode, bind, auth, tailscale)
- `session.dmScope` (l'intégration locale utilise par défaut `per-channel-peer` quand non défini ; les valeurs explicites existantes sont conservées)
...
<Note>
Certains canaux sont fournis sous forme de plugins. Quand sélectionnés lors de l'intégration, l'assistant
