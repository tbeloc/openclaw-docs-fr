...
- **Session store** (`sessions.json`) et ce qu'il suit
...
En `mode: "warn"`, OpenClaw signale les évictions potentielles mais ne modifie pas le store/les fichiers.
...
- `cron.sessionRetention` (par défaut `24h`) nettoie les anciennes sessions d'exécution cron isolées du store de session (`false` désactive).
...
Une `sessionKey` identifie _quel bucket de conversation_ vous êtes (routage + isolation).
...
Les règles canoniques sont documentées à [/concepts/session](/fr/concepts/session).
...
- **Garde de bifurcation de parent de thread** (`session.parentForkMaxTokens`, par défaut `100000`) ignore la bifurcation de transcription parent quand la session parent est déjà trop grande ; le nouveau thread démarre à zéro. Définissez `0` pour désactiver.

Détail d'implémentation : la décision se produit dans `initSessionState()` dans `src/auto-reply/reply/session.ts`.

---

## Schéma du session store (`sessions.json`)

Le type de valeur du store est `SessionEntry` dans `src/config/sessions.ts`.
...
- `updatedAt` : timestamp de dernière activité
...
- `compactionCount` : combien de fois la compaction automatique s'est complétée pour cette clé de session
- `memoryFlushAt` : timestamp du dernier vidage de mémoire pré-compaction
- `memoryFlushCompactionCount` : comptage de compaction quand le dernier vidage s'est exécuté

Le store est sûr à éditer, mais la Gateway est l'autorité : elle peut réécrire ou réhydrater les entrées au fur et à mesure que les sessions s'exécutent.
...
- `branch_summary` : résumé persisté lors de la navigation d'une branche d'arbre
...
2. **Compteurs du session store** : statistiques roulantes écrites dans `sessions.json` (utilisées pour /status et les tableaux de bord)
...
- `contextTokens` dans le store est une valeur d'estimation/rapport d'exécution ; ne la traitez pas comme une garantie stricte.

Pour plus, voir [/token-use](/fr/reference/token-use).
...
- S'il est déjà plus élevé, OpenClaw le laisse seul.

Pourquoi : laisser assez d'espace pour la « maintenance » multi-tour (comme les écritures en mémoire) avant que la compaction ne devienne inévitable.
...
- Mode verbeux : `🧹 Auto-compaction complete` + comptage de compaction
...
- L'assistant commence sa sortie avec `NO_REPLY` pour indiquer « ne pas livrer de réponse à l'utilisateur ».
- OpenClaw supprime/masque cela dans la couche de livraison.

À partir de `2026.1.10`, OpenClaw supprime également le **streaming de brouillon/saisie** quand un chunk partiel commence par `NO_REPLY`, donc les opérations silencieuses ne fuient pas de sortie partielle à mi-tour.

---

## Pré-compaction « memory flush » (implémenté)
...
OpenClaw utilise l'approche **pré-seuil flush** :
...
- Voir [Memory](/fr/concepts/memory) pour la disposition des fichiers d'espace de travail et les modèles d'écriture.

Pi expose également un hook `session_before_compact` dans l'API d'extension, mais la logique de vidage d'OpenClaw se trouve côté Gateway aujourd'hui.
...
- Spam de compaction ? Vérifiez :
  ...
  - encombrement des résultats d'outils : activez/ajustez l'élagage de session
