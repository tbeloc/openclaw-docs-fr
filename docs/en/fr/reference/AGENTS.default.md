...
- Lisez `MEMORY.md` si présent ; ne revenez à `memory.md` en minuscules que si `MEMORY.md` est absent.
...
- `memory.md` en minuscules est un repli hérité uniquement ; ne conservez pas les deux fichiers racine intentionnellement.
- Au démarrage de la session, lisez aujourd'hui + hier + `MEMORY.md` si présent, sinon `memory.md`.
...
- Les chats directs s'effondrent dans la session `main` de l'agent par défaut ; les groupes restent isolés comme `agent:<agentId>:<channel>:group:<id>` (salons/canaux : `agent:<agentId>:<channel>:channel:<id>`) ; les battements de cœur maintiennent les tâches de fond actives.
...
- **Gemini CLI** — Modèles Google Gemini depuis le terminal pour les Q&R rapides.
