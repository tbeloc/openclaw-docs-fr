...
Voir la ventilation complète dans [Invite système](/concepts/system-prompt).
...
Pour une ventilation pratique (par fichier injecté, outils, compétences et taille de l'invite système), utilisez `/context list` ou `/context detail`. Voir [Contexte](/concepts/context).
...
optionnellement exécuter **l'élagage cache-ttl** : il élague la session une fois que la TTL du cache
a expiré, puis réinitialise la fenêtre du cache afin que les demandes ultérieures puissent réutiliser
le contexte fraîchement mis en cache au lieu de remettre en cache l'historique complet. Cela maintient les coûts d'écriture du cache plus bas quand une session devient inactive au-delà de la TTL.
...
Le battement cardiaque peut garder le cache **chaud** sur les lacunes inactives. Si votre TTL du cache de modèle
est `1h`, définir l'intervalle de battement cardiaque juste en dessous (par exemple, `55m`) peut éviter
...
La fenêtre de contexte 1M d'Anthropic est actuellement contrôlée par bêta. OpenClaw peut injecter la
