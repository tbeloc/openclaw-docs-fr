...
Pour les images, OpenClaw réduit les payloads d'images de transcription/outil avant les appels au fournisseur.
Utilisez `agents.defaults.imageMaxDimensionPx` (par défaut : `1200`) pour ajuster ceci :

- Les valeurs plus basses réduisent généralement l'utilisation des tokens de vision et la taille du payload.
- Les valeurs plus élevées préservent plus de détails visuels pour les captures d'écran OCR/UI-lourdes.

Pour une ventilation pratique (par fichier injecté, outils, compétences et taille du prompt système), utilisez `/context list` ou `/context detail`. Voir [Context](/concepts/context).
...
La mise en cache du prompt du fournisseur s'applique uniquement dans la fenêtre TTL du cache. OpenClaw peut
...
détails du comportement dans [Session pruning](/concepts/session-pruning).

Le Heartbeat peut garder le cache **chaud** sur les lacunes d'inactivité. Si votre TTL du cache du modèle
est `1h`, définir l'intervalle du heartbeat juste en dessous (par exemple, `55m`) peut éviter
de remettre en cache le prompt complet, réduisant les coûts d'écriture du cache.
...
Pour un guide complet bouton par bouton, voir [Prompt Caching](/reference/prompt-caching).

Pour la tarification de l'API Anthropic, les lectures du cache sont considérablement moins chères que les tokens d'entrée, tandis que les écritures du cache sont facturées à un multiplicateur plus élevé. Consultez la tarification de la mise en cache du prompt d'Anthropic pour les tarifs et multiplicateurs TTL les plus récents :
...
ou abonnement avec Extra Usage activé). Sinon, Anthropic répond avec `HTTP 429: rate_limit_error: Extra usage is required for long context requests`.
...
- Baissez `agents.defaults.imageMaxDimensionPx` pour les sessions lourdes en captures d'écran.
...
Voir [Skills](/tools/skills) pour la formule exacte de surcharge de la liste des compétences.
