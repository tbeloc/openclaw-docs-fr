...
- `cacheWrite` élevé sur la plupart des tours : vérifiez les entrées d'invite système volatiles et vérifiez que la clé du modèle/fournisseur prend en charge vos paramètres de cache.
- Aucun effet de `cacheRetention` : confirmez que la clé du modèle correspond à `agents.defaults.models["provider/model"]`.
- Demandes Bedrock Nova/Mistral avec paramètres de cache : forçage d'exécution attendu à `none`.
