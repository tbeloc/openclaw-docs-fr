...
- `OPENCLAW_CACHE_TRACE_MESSAGES=0|1` bascule la capture de charge utile de message complète.
- `OPENCLAW_CACHE_TRACE_PROMPT=0|1` bascule la capture de texte d'invite.
- `OPENCLAW_CACHE_TRACE_SYSTEM=0|1` bascule la capture d'invite système.
...
- `cacheWrite` élevé sur la plupart des tours : vérifiez les entrées d'invites système volatiles et vérifiez que le modèle/fournisseur supporte vos paramètres de cache.
...
- [Référence de configuration de la passerelle](/gateway/configuration-reference)
