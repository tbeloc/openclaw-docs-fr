...
summary: "Paramètres de mise en cache des invites, ordre de fusion, comportement du fournisseur et modèles de réglage"
read_when:
...
La mise en cache des invites signifie que le fournisseur de modèle peut réutiliser les préfixes d'invites inchangés (généralement les instructions système/développeur et autres contextes stables) entre les tours au lieu de les retraiter à chaque fois. La première demande correspondante écrit les jetons de cache (`cacheWrite`), et les demandes correspondantes ultérieures peuvent les relire (`cacheRead`).
...
Cette page couvre tous les paramètres liés au cache qui affectent la réutilisation des invites et le coût des jetons.
...
Définissez la rétention du cache sur les paramètres du modèle :
...
Consultez [Session Pruning](/concepts/session-pruning) pour le comportement complet.
...
Le battement de cœur par agent est supporté à `agents.list[].heartbeat`.

## Comportement du fournisseur

### Anthropic (API directe)

- `cacheRetention` est supporté.
...
- Les références de modèles Anthropic Claude (`amazon-bedrock/*anthropic.claude*`) supportent le passage explicite de `cacheRetention`.
...
Si le fournisseur ne supporte pas ce mode de cache, `cacheRetention` n'a aucun effet.

## Modèles de réglage

### Trafic mixte (recommandé par défaut)

Maintenez une ligne de base longue durée sur votre agent principal, désactivez la mise en cache sur les agents notificateurs en rafales :
...
- `cacheWrite` élevé sur la plupart des tours : vérifiez les entrées d'invite système volatiles et vérifiez que le modèle/fournisseur supporte vos paramètres de cache.
- Aucun effet de `cacheRetention` : confirmez que la clé du modèle correspond à `agents.defaults.models["provider/model"]`.
- Demandes Bedrock Nova/Mistral avec paramètres de cache : forçage à l'exécution attendu à `none`.
