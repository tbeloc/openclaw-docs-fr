...
Quand l'opérateur dit « release », faites immédiatement ce contrôle préalable (sans questions supplémentaires sauf si bloqué) :
...
- Balise de correction de secours : `vYYYY.M.D-N`
  - À utiliser uniquement comme balise de récupération en dernier recours quand une publication immuable a brûlé la balise stable d'origine et que vous ne pouvez pas la réutiliser.
  - La version du package npm reste `YYYY.M.D` ; le suffixe `-N` est uniquement pour la balise git et la version GitHub.
  - Préférez les bêtas pour l'itération normale de pré-publication, puis coupez une balise stable propre une fois prêt.
...
- Traitez les balises de correction comme une échappatoire de secours uniquement. Les nouvelles publications doivent toujours utiliser `vYYYY.M.D` pour stable et `vYYYY.M.D-beta.N` pour bêta.
...
- [ ] `OPENCLAW_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke` (test de fumée d'installation Docker, chemin rapide ; requis avant la publication)
  - Si la publication npm immédiatement précédente est connue comme cassée, définissez `OPENCLAW_INSTALL_SMOKE_PREVIOUS=<last-good-version>` ou `OPENCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1` pour l'étape de préinstallation.
- [ ] (Optionnel) Test de fumée d'installation complet (ajoute la couverture non-root + CLI) : `pnpm test:install:smoke`
- [ ] (Optionnel) Installation E2E (Docker, exécute `curl -fsSL https://openclaw.ai/install.sh | bash`, intègre, puis exécute des appels d'outils réels) :
...
- [ ] Suivez [macOS release](/platforms/mac/release) pour les commandes exactes et les variables env requises.
  ...
  - Si vous notarisez, utilisez le profil de chaîne de clés `openclaw-notary` créé à partir des variables env de l'API App Store Connect (voir [macOS release](/platforms/mac/release)).
...
- **La balise a besoin d'une récupération après un correctif tardif** : si la balise stable d'origine est liée à une version GitHub immuable, créez une balise de correction de secours comme `vX.Y.Z-1` au lieu d'essayer de forcer la mise à jour de `vX.Y.Z`.
  ...
  - Utilisez ceci uniquement en dernier recours. Pour l'itération normale, préférez les balises bêta, puis coupez une publication stable propre.
...
- [ ] Créez/actualisez la version GitHub pour `vX.Y.Z` avec le **titre `openclaw X.Y.Z`** (pas seulement la balise) ; le corps doit inclure la section de **changelog complète** pour cette version (Points forts + Modifications + Correctifs), en ligne (pas de liens nus), et **ne doit pas répéter le titre dans le corps**.
