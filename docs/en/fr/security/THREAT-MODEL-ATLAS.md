...
Ceci est un document vivant maintenu par la communauté OpenClaw. Consultez [CONTRIBUTING-THREAT-MODEL.md](/fr/security/CONTRIBUTING-THREAT-MODEL) pour les directives de contribution :
...
| **Description**         | L'attaquant analyse les points de terminaison OpenClaw exposés       |
...
| **Description**         | L'attaquant élabore des commandes qui contournent la liste blanche d'approbation |
| **Vecteur d'attaque**   | Obfuscation de commande, exploitation d'alias, manipulation de chemin |
| **Composants affectés** | exec-approvals.ts, liste blanche de commande                |
| **Atténuations actuelles** | Mode liste blanche + demande                                |
| **Risque résiduel**     | Élevé - Pas d'assainissement de commande                    |
| **Recommandations**     | Implémenter la normalisation de commande, élargir la liste noire |
...
| **Risque résiduel**     | Critique - Pas d'isolation, révision limitée                             |
| **Recommandations**     | Intégration VirusTotal (en cours), isolation de compétence, révision communautaire |
...
| ----------------------- | ---------------------------------------------------------------------- |
| **ID ATLAS**            | AML.T0043 - Créer des données adversariales                             |
| **Description**         | L'attaquant élabore le contenu de compétence pour contourner les motifs de modération |
...
| ----------------------- | --------------------------------------------------------- |
| **ID ATLAS**            | AML.T0043 - Créer des données adversariales                |
| **Description**         | L'attaquant élabore le contenu qui s'échappe du contexte d'encapsulation XML |
...
| **Vecteur d'attaque**   | Requêtes "De quoi avons-nous parlé ?", sondage de contexte |
...
| **Vecteur d'attaque**   | Injection de prompt causant l'agent à POST les données vers le serveur de l'attaquant |
...
| ----------------------- | ---------------------------------------------------------------- |
| **ID ATLAS**            | AML.T0009 - Collecte                                              |
| **Description**         | L'attaquant cause l'agent à envoyer des messages contenant des données sensibles |
| **Vecteur d'attaque**   | Injection de prompt causant l'agent à envoyer un message à l'attaquant |
...
#### T-EXFIL-003: Récolte de credentials

| Attribut                | Valeur                                                   |
| ----------------------- | ------------------------------------------------------- |
| **ID ATLAS**            | AML.T0009 - Collecte                                     |
| **Description**         | Une compétence malveillante récolte les credentials du contexte d'agent |
...
| **Recommandations**     | Isolation de compétence, isolation de credentials        |
...
| **Vecteur d'attaque**   | Inondation de message automatisée, appels d'outil coûteux |
...
| ----------------------- | ------------------------------------------------------- |
| **ID ATLAS**            | AML.T0031 - Éroder l'intégrité du modèle IA             |
| **Description**         | L'attaquant cause l'agent à envoyer du contenu nuisible/offensant |
...
| Validation du type de fichier | `isTextFile()`              | Moyen - Fichiers texte uniquement, mais peuvent toujours être malveillants |
...
(Empoisonner le contenu URL) → (L'agent récupère et suit les instructions) → (Les données sont envoyées à l'attaquant)
```
...
| ID    | Recommandation                                        | Adresse     |
| ----- | ----------------------------------------------------- | ------------- |
| R-008 | Ajouter la vérification du canal cryptographique où possible | T-ACCESS-002  |
