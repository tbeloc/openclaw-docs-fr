...
Vous avez repéré un vecteur d'attaque ou un risque que nous n'avons pas couverts ? Ouvrez un problème sur [openclaw/trust](https://github.com/openclaw/trust/issues) et décrivez-le avec vos propres mots. Vous n'avez pas besoin de connaître les frameworks ou de remplir tous les champs - décrivez simplement le scénario.
...
Ce modèle de menace est construit sur [MITRE ATLAS](https://atlas.mitre.org/) (Adversarial Threat Landscape for AI Systems), un framework conçu spécifiquement pour les menaces IA/ML comme l'injection de prompt, l'abus d'outils et l'exploitation d'agents. Vous n'avez pas besoin de connaître ATLAS pour contribuer - nous mappons les soumissions au framework lors de l'examen.
