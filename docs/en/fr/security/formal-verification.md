...
**Objectif (étoile polaire) :** fournir un argument vérifié par machine selon lequel OpenClaw applique sa politique de sécurité prévue (autorisation, isolation de session, gating d'outils et sécurité contre les erreurs de configuration), selon des hypothèses explicites.
...
**Ce que ce n'est pas (encore) :** une preuve que « OpenClaw est sécurisé sous tous les aspects » ou que l'implémentation TypeScript complète est correcte.
...
Aujourd'hui, les résultats sont reproduits en clonant le dépôt des modèles localement et en exécutant TLC (voir ci-dessous). Une itération future pourrait offrir :
...
### Exposition de la passerelle et erreur de configuration de passerelle ouverte

**Affirmation :** la liaison au-delà de loopback sans authentification peut rendre la compromission à distance possible / augmente l'exposition ; le token/mot de passe bloque les attaquants non authentifiés (selon les hypothèses du modèle).
...
**Affirmation :** un magasin d'appairage doit appliquer `MaxPending` et l'idempotence même sous les entrelacages (c'est-à-dire que « vérifier-puis-écrire » doit être atomique / verrouillé ; l'actualisation ne devrait pas créer de doublons).

Ce que cela signifie :

- Sous des demandes concurrentes, vous ne pouvez pas dépasser `MaxPending` pour un canal.
- Les demandes/actualisations répétées pour le même `(channel, sender)` ne doivent pas créer de lignes en attente en direct dupliquées.
...
- identityLinks ne doit fusionner que dans les groupes explicitement liés, pas entre les pairs non liés.
