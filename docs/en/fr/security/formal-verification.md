...
**Objectif (nord magnétique) :** fournir un argument vérifié par machine selon lequel OpenClaw applique sa politique de sécurité prévue (autorisation, isolation de session, gating d'outils et sécurité contre les erreurs de configuration), selon des hypothèses explicites.
...
**Affirmation :** la liaison au-delà de loopback sans authentification peut rendre le compromis à distance possible / augmente l'exposition ; le token/mot de passe bloque les attaquants non authentifiés (selon les hypothèses du modèle).
