...
Edge TTS utilise le service TTS neuronal en ligne de Microsoft Edge via la
...
comme étant au mieux. Si vous avez besoin de limites garanties et d'assistance, utilisez OpenAI ou ElevenLabs.
...
Le schéma complet est dans [Configuration de la passerelle](/gateway/configuration).
...
Réponse -> TTS activé ?
  non  -> envoyer texte
  oui  -> contient des médias / MEDIA: / court ?
          oui  -> envoyer texte
          non  -> longueur > limite ?
...
- `/tts audio` génère une réponse audio unique (ne bascule pas le TTS).
