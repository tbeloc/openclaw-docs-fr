...
répondre aux messages directs et aux messages de groupe. Les réponses de groupe nécessitent une mention @ par défaut et peuvent
...
Le ship propriétaire est **automatiquement autorisé partout** — les invitations de message direct sont auto-acceptées et
...
- Les demandes de message direct des ships qui ne sont pas dans la liste blanche
- Les mentions dans les canaux sans autorisation
- Les demandes d'invitation de groupe

## Paramètres d'acceptation automatique

Auto-accepter les invitations de message direct (pour les ships dans dmAllowlist) :
...
| Threads         | ✅ Supporté (auto-réponses dans le thread)   |
...
- **Messages directs ignorés** : l'expéditeur n'est pas dans `dmAllowlist` et aucun `ownerShip` configuré pour le flux d'approbation.
...
- `channels.tlon.ship` : nom du ship Urbit du bot (par exemple `~sampel-palnet`).
- `channels.tlon.url` : URL du ship (par exemple `https://sampel-palnet.tlon.network`).
...
- `channels.tlon.autoAcceptDmInvites` : auto-accepter les messages directs des ships de la liste blanche.
...
- Réponses de thread : si le message entrant est dans un thread, OpenClaw répond dans le thread.
