...
Choisissez **Feishu** et collez votre ID d'application + secret d'application.
...
> Remarque : Feishu ne supporte pas encore les menus de commande natifs, donc les commandes doivent être envoyées sous forme de texte.
...
1. Réinitialisez le secret de l'application dans la plateforme ouverte Feishu
2. Mettez à jour le secret de l'application dans votre configuration
...
- `textChunkLimit`: taille des chunks de texte sortants (par défaut : 2000 caractères)
- `mediaMaxMb`: limite de téléchargement/téléversement de médias (par défaut : 30 Mo)

### Streaming

Feishu supporte les réponses en streaming via des cartes interactives. Lorsqu'activé, le bot met à jour une carte au fur et à mesure qu'il génère du texte.
...
Utilisez `bindings` pour router les DM ou groupes Feishu vers différents agents.
...
| `channels.feishu.textChunkLimit`                  | Taille des chunks de message                   | `2000`           |
| `channels.feishu.mediaMaxMb`                      | Limite de taille des médias                    | `30`             |
| `channels.feishu.streaming`                       | Activer la sortie de carte en streaming        | `true`           |
| `channels.feishu.blockStreaming`                  | Activer le streaming de blocs                  | `true`           |
...
- ✅ Fichiers
- ✅ Audio
- ⚠️ Texte enrichi (support partiel)
