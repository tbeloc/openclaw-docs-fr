...
> - **L'accès au groupe** est contrôlé par `*.groupPolicy` + listes blanches (`*.groups`, `*.groupAllowFrom`).
...
Oui — cela fonctionne bien si votre trafic « personnel » est des **DMs** et votre trafic « public » est des **groupes**.
...
> Si vous avez besoin d'espaces de travail/personas vraiment séparés (« personnel » et « public » ne doivent jamais se mélanger), utilisez un deuxième agent + bindings. Voir [Multi-Agent Routing](/concepts/multi-agent).
...
- Clés de configuration et valeurs par défaut : [Gateway configuration](/gateway/configuration#agentsdefaultssandbox)
...
- Détails des bind mounts : [Sandboxing](/gateway/sandboxing#custom-bind-mounts)

## Étiquettes d'affichage

- Les étiquettes UI utilisent `displayName` quand disponible, formatées comme `<channel>:<token>`.
...
| ------------- | ------------------------------------------------------------ |
| `"open"`      | Les groupes contournent les listes blanches ; la mention gating s'applique toujours. |
...
- `groupPolicy` est séparé de la mention gating (qui nécessite des @mentions).
- WhatsApp/Telegram/Signal/iMessage/Microsoft Teams/Zalo : utilisez `groupAllowFrom` (fallback : `allowFrom` explicite).
- Les approbations d'appairage DM (`*-allowFrom` store entries) s'appliquent uniquement à l'accès DM ; l'autorisation de l'expéditeur du groupe reste explicite aux listes blanches du groupe.
...
- Sécurité à l'exécution : quand un bloc de fournisseur est complètement absent (`channels.<provider>` absent), la politique de groupe revient à un mode fail-closed (généralement `allowlist`) au lieu d'hériter de `channels.defaults.groupPolicy`.
...
Répondre à un message bot compte comme une mention implicite (quand le canal supporte les métadonnées de réponse). Cela s'applique à Telegram, WhatsApp, Slack, Discord et Microsoft Teams.
...
- Les surfaces qui fournissent des mentions explicites passent toujours ; les modèles sont un fallback.
- Remplacement par agent : `agents.list[].groupChat.mentionPatterns` (utile quand plusieurs agents partagent un groupe).
- La mention gating n'est appliquée que quand la détection de mention est possible (mentions natives ou `mentionPatterns` configurés).
- Les valeurs par défaut Discord vivent dans `channels.discord.guilds."*"` (remplaçable par guild/canal).
- L'historique du contexte du groupe est enveloppé uniformément sur les canaux et est **pending-only** (messages ignorés en raison de la mention gating); utilisez `messages.groupChat.historyLimit` pour la valeur par défaut globale et `channels.<channel>.historyLimit` (ou `channels.<channel>.accounts.*.historyLimit`) pour les remplacements. Définissez `0` pour désactiver.
...
- `toolsBySender` : remplacements par expéditeur dans le groupe.
  ...
  `id:<senderId>`, `e164:<phone>`, `username:<handle>`, `name:<displayName>`, et le wildcard `"*"`.
