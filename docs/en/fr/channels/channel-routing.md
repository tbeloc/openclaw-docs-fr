...
Pour empêcher que `lastRoute` de la session soit écrasé par des DM non-propriétaires,
...
- L'expéditeur DM entrant ne correspond pas à ce propriétaire épinglé.
...
Les magasins de sessions se trouvent sous le répertoire d'état (défaut `~/.openclaw`) :
...
La découverte de session Gateway et ACP analyse également les magasins d'agents sauvegardés sur disque sous la
...
- Le contexte cité est ajouté à `Body` comme un bloc `[Replying to ...]`.
