...
Support multi-compte : utilisez `channels.signal.accounts` avec configuration par compte et `name` optionnel. Voir [`gateway/configuration`](/fr/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) pour le modèle partagé.
...
- L'appairage est l'échange de jetons par défaut pour les DM Signal. Détails : [Pairing](/fr/channels/pairing)
...
- Noms d'utilisateur : `username:<name>` (s'il est supporté par votre compte Signal).
...
- Messages de groupe ignorés : le contrôle de porte de l'expéditeur/mention du groupe bloque la livraison.
...
- La vérification SMS n'est nécessaire que pour les flux d'enregistrement ou de récupération, mais perdre le contrôle du numéro/compte peut compliquer la réinscription.
