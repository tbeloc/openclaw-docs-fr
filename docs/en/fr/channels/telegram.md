...
<AccordionGroup>
  <Accordion title="Aperçu du flux en direct (éditions de messages)">
    ...
    La diffusion en continu d'aperçu est distincte de la diffusion en continu de bloc. Lorsque la diffusion en continu de bloc est explicitement activée pour Telegram, OpenClaw ignore le flux d'aperçu pour éviter la double diffusion.
...
```
  ...
  <Accordion title="Sujets de forum et comportement des threads">
    ...
    - les réponses et la saisie ciblent le thread de sujet
    ...
    Héritage de sujet : les entrées de sujet héritent des paramètres de groupe sauf s'ils sont remplacés (`requireMention`, `allowFrom`, `skills`, `systemPrompt`, `enabled`, `groupPolicy`).
    ...
    **Liaison de sujet ACP persistante** : Les sujets de forum peuvent épingler les sessions de harnais ACP via les liaisons ACP typées de niveau supérieur :
    ...
    Ceci est actuellement limité aux sujets de forum dans les groupes et supergroups.
...
<AccordionGroup>
  ...
  <Accordion title="Instabilité du polling ou du réseau">
    ...
    - Sur les hôtes VPS avec sortie/TLS directe instable, acheminez les appels de l'API Telegram via `channels.telegram.proxy` :
...
- Précédence multi-compte :
  - Quand deux ou plusieurs ID de compte sont configurés, définissez `channels.telegram.defaultAccount` (ou incluez `channels.telegram.accounts.default`) pour rendre l'acheminement par défaut explicite.
...
- `channels.telegram.groups` : valeurs par défaut par groupe + liste blanche (utilisez `"*"` pour les valeurs par défaut globales).
  ...
  - `channels.telegram.groups.<id>.topics.<threadId>.agentId` : acheminer ce sujet vers un agent spécifique (remplace l'acheminement au niveau du groupe et de la liaison).
...
- `bindings[]` de haut niveau avec `type: "acp"` et ID de sujet canonique `chatId:topic:topicId` dans `match.peer.id` : champs de liaison de sujet ACP persistants (voir [Agents ACP](/fr/tools/acp-agents#channel-specific-settings)).
...
- `channels.telegram.accounts.<account>.execApprovals` : remplacement par compte pour l'acheminement d'approbation d'exécution Telegram et l'autorisation d'approbateur.
...
- `channels.telegram.mediaMaxMb` : limite de média Telegram entrante/sortante (MB, par défaut : 100).
...
- `channels.telegram.network.autoSelectFamily` : remplacer Node autoSelectFamily (true=activer, false=désactiver). Activé par défaut sur Node 22+, WSL2 étant désactivé par défaut.
...
- `channels.telegram.reactionNotifications` : `off | own | all` — contrôler quelles réactions déclenchent des événements système (par défaut : `own` quand non défini).
...
- [Acheminement des canaux](/fr/channels/channel-routing)
- [Acheminement multi-agent](/fr/concepts/multi-agent)
