...
- Pour ne permettre **aucun canal**, définissez `channels.msteams.groupPolicy: "disabled"`.
...
| ------------------------ | ---------------------------------------------- | ----------------------------------------------- |
| **Messages directs**     | FileConsentCard → utilisateur accepte → bot télécharge | Fonctionne directement                          |
...
Les bots n'ont pas de lecteur OneDrive personnel (le point de terminaison Graph API `/me/drive` ne fonctionne pas pour les identités d'application). Pour envoyer des fichiers dans les chats de groupe/canaux, le bot télécharge vers un **site SharePoint** et crée un lien de partage.

### Configuration

1. **Ajouter les permissions Graph API** dans Entra ID (Azure AD) → Inscription d'application :
...
- ID d'équipe = segment de chemin après `/team/` (décodé URL, par ex. `19:Bk4j...@thread.tacv2`)
...
1. Utilisez les canaux standard pour les interactions de bot
...
- **Les images ne s'affichent pas dans les canaux :** Permissions Graph manquantes ou consentement administrateur manquant. Réinstallez l'application Teams et quittez/rouvrez complètement Teams.
