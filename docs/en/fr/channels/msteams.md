---
summary: "Statut du support des bots Microsoft Teams, capacités et configuration"
read_when:
...
Statut : texte + pièces jointes DM sont supportés ; l'envoi de fichiers en groupe/canal nécessite `sharePointSiteId` + permissions Graph (voir [Envoi de fichiers dans les chats de groupe](#envoi-de-fichiers-dans-les-chats-de-groupe)). Les sondages sont envoyés via Adaptive Cards.
...
2. Définissez **Point de terminaison de messagerie** sur votre URL webhook :
   ...
   - Dev local : Utilisez un tunnel (voir [Développement local](#développement-local-tunneling) ci-dessous)
...
## Exemple de manifeste Teams (censuré)
...
1. Dans **Enregistrement d'application** Entra ID (Azure AD), ajoutez les permissions d'**application** Microsoft Graph :
...
- `channels.msteams.allowFrom` : liste d'autorisation DM (ID d'objet AAD recommandés). L'assistant résout les noms en ID lors de la configuration lorsque l'accès Graph est disponible.
...
- `channels.msteams.chunkMode` : `length` (par défaut) ou `newline` pour diviser sur les lignes vides (limites de paragraphe) avant la segmentation par longueur.
...
- `channels.msteams.teams.<teamId>.toolsBySender` : remplacements de politique d'outil par défaut par expéditeur par équipe (caractère générique `"*"` pris en charge).
...
- `channels.msteams.teams.<teamId>.channels.<conversationId>.toolsBySender` : remplacements de politique d'outil par expéditeur par canal (caractère générique `"*"` pris en charge).
- Les clés `toolsBySender` doivent utiliser des préfixes explicites :
  `id:`, `e164:`, `username:`, `name:` (les clés non préfixées héritées correspondent toujours à `id:` uniquement).
...
| ------------------------ | --------------------------------------------------------- | ----------------------- |
| **Posts** (classique)    | Les messages apparaissent sous forme de cartes avec des réponses filetées en dessous | `thread` (par défaut)   |
| **Threads** (style Slack) | Les messages s'écoulent linéairement, plus comme Slack    | `top-level`             |

**Le problème :** L'API Teams n'expose pas le style d'interface utilisateur qu'un canal utilise. Si vous utilisez le mauvais `replyStyle` :

- `thread` dans un canal de style Threads → les réponses apparaissent imbriquées maladroitement
- `top-level` dans un canal de style Posts → les réponses apparaissent comme des posts de niveau supérieur séparés au lieu d'être dans le thread
...
- **Canaux/groupes :** Les pièces jointes se trouvent dans le stockage M365 (SharePoint/OneDrive). La charge utile du webhook inclut uniquement un stub HTML, pas les octets de fichier réels. **Les permissions API Graph sont requises** pour télécharger les pièces jointes de canal.
...
Par défaut, OpenClaw télécharge uniquement les médias à partir des noms d'hôte Microsoft/Teams. Remplacez par `channels.msteams.mediaAllowHosts` (utilisez `["*"]` pour autoriser n'importe quel hôte).
...
| Chat de groupe + fichier + pas de `sharePointSiteId` | Tenter le téléchargement OneDrive (peut échouer), envoyer le texte uniquement |
...
- Les sondages ne publient pas encore automatiquement les résumés des résultats (inspectez le fichier de stockage si nécessaire).
...
- Les messages proactifs ne sont possibles **qu'après** qu'un utilisateur a interagi, car nous stockons les références de conversation à ce moment.
- Voir `/gateway/configuration` pour `dmPolicy` et la restriction de liste d'autorisation.

## ID d'équipe et de canal (Piège courant)

Le paramètre de requête `groupId` dans les URL Teams **N'EST PAS** l'ID d'équipe utilisé pour la configuration. Extrayez les ID du chemin d'accès de l'URL à la place :
...
- ID d'équipe = segment de chemin après `/team/` (décodé en URL, par exemple `19:Bk4j...@thread.tacv2`)
...
- **"Icon file cannot be empty" :** Le manifeste référence des fichiers d'icône qui font 0 octet. Créez des icônes PNG valides (32x32 pour `outline.png`, 192x192 pour `color.png`).
- **"webApplicationInfo.Id already in use" :** L'application est toujours installée dans une autre équipe/chat. Trouvez et désinstallez-la d'abord, ou attendez 5-10 minutes pour la propagation.
...
- **Sideload échouant :** Essayez "Upload an app to your org's app catalog" au lieu de "Upload a custom app" - cela contourne souvent les restrictions de sideload.
