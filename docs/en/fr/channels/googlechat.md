...
Avec cette configuration, toute demande à `your-domain.com/` sera ignorée ou retournera un 404, tandis que `your-domain.com/googlechat` est sûrement acheminé vers OpenClaw.
...
- E-mail brut `name@example.com` est mutable et utilisé uniquement pour la correspondance directe de la liste d'autorisation lorsque `channels.googlechat.dangerouslyAllowNameMatching: true`.
...
- `serviceAccountRef` est également pris en charge (env/file SecretRef), y compris les références par compte sous `channels.googlechat.accounts.<id>.serviceAccountRef`.
...
Détails de la référence des secrets : [Gestion des secrets](/fr/gateway/secrets).
