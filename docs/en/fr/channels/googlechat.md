...
5. Créez une application Google Chat dans la [Configuration Google Chat de la console Google Cloud](https://console.cloud.google.com/apis/api/chat.googleapis.com/hangouts-chat) :
   ...
   - Sous **Visibilité**, cochez **Rendre cette application Chat disponible pour des personnes et des groupes spécifiques dans &lt;Votre domaine&gt;**.
...
9. Démarrez la passerelle. Google Chat enverra des POST à votre chemin webhook.
...
3. **Exposez uniquement le chemin webhook publiquement :**
...
Avec cette configuration, toute demande à `your-domain.com/` sera ignorée ou retournera 404, tandis que `your-domain.com/googlechat` est routée en toute sécurité vers OpenClaw.

### Option C : Tunnel Cloudflare

Configurez les règles d'entrée de votre tunnel pour router uniquement le chemin webhook :
...
1. Google Chat envoie des POST webhook à la passerelle. Chaque demande inclut un en-tête `Authorization: Bearer <token>`.
   ...
   - Les demandes de module complémentaire Google Workspace qui portent `authorizationEventObject.systemIdToken` dans le corps sont prises en charge via un budget de pré-authentification du corps plus strict.
...
- Le chemin webhook par défaut est `/googlechat` si `webhookPath` n'est pas défini.
- `dangerouslyAllowNameMatching` réactive la correspondance des principaux d'e-mail mutable pour les listes d'autorisation (mode de compatibilité de secours).
...
- Si aucun message n'arrive, confirmez l'URL webhook de l'application Chat + les abonnements aux événements.
- Si la limitation de mention bloque les réponses, définissez `botUser` sur le nom de ressource utilisateur de l'application et vérifiez `requireMention`.
- Utilisez `openclaw logs --follow` lors de l'envoi d'un message de test pour voir si les demandes atteignent la passerelle.
