---
summary: "Comportement et configuration pour la gestion des messages de groupe WhatsApp (les mentionPatterns sont partagés entre les surfaces)"
read_when:
...
Objectif : permettre à Clawd de rester dans les groupes WhatsApp, de se réveiller uniquement lorsqu'il est mentionné, et de garder ce fil de discussion séparé de la session DM personnelle.
...
- Modes d'activation : `mention` (par défaut) ou `always`. `mention` nécessite une mention (vraies mentions WhatsApp @-mentions via `mentionedJids`, motifs regex, ou le numéro E.164 du bot n'importe où dans le texte). `always` réveille l'agent à chaque message mais il ne devrait répondre que lorsqu'il peut ajouter une valeur significative ; sinon il retourne le token silencieux `NO_REPLY`. Les valeurs par défaut peuvent être définies dans la config (`channels.whatsapp.groups`) et remplacées par groupe via `/activation`. Lorsque `channels.whatsapp.groups` est défini, il agit également comme une liste blanche de groupes (incluez `"*"` pour autoriser tous).
...
- Les expressions régulières sont insensibles à la casse ; elles couvrent une mention par nom d'affichage comme `@openclaw` et le numéro brut avec ou sans `+`/espaces.
- WhatsApp envoie toujours les mentions canoniques via `mentionedJids` lorsque quelqu'un appuie sur le contact, donc le fallback numérique est rarement nécessaire mais est un filet de sécurité utile.
...
Seul le numéro du propriétaire (de `channels.whatsapp.allowFrom`, ou le numéro E.164 du bot lorsqu'il n'est pas défini) peut le modifier. Envoyez `/status` comme message autonome dans le groupe pour voir le mode d'activation actuel.
...
- Les battements de cœur sont intentionnellement ignorés pour les groupes afin d'éviter les diffusions bruyantes.
- La suppression d'écho utilise la chaîne de lot combinée ; si vous envoyez deux fois le même texte sans mentions, seul le premier recevra une réponse.
- Les entrées du magasin de sessions apparaîtront comme `agent:<agentId>:whatsapp:group:<jid>` dans le magasin de sessions (`~/.openclaw/agents/<agentId>/sessions/sessions.json` par défaut) ; une entrée manquante signifie simplement que le groupe n'a pas encore déclenché une exécution.
