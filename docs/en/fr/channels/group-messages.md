...
- WhatsApp envoie toujours les mentions canoniques via `mentionedJids` quand quelqu'un appuie sur le contact, donc le fallback de numéro est rarement nécessaire mais c'est un filet de sécurité utile.
...
Seul le numéro du propriétaire (de `channels.whatsapp.allowFrom`, ou le numéro E.164 du bot quand non défini) peut changer cela. Envoyez `/status` comme message autonome dans le groupe pour voir le mode d'activation actuel.
...
4. Les directives au niveau de la session (`/verbose on`, `/think high`, `/new` ou `/reset`, `/compact`) s'appliquent uniquement à la session de ce groupe ; envoyez-les comme messages autonomes pour qu'elles s'enregistrent. Votre session DM personnelle reste indépendante.
...
- La suppression d'écho utilise la chaîne de lot combinée ; si vous envoyez du texte identique deux fois sans mentions, seul le premier recevra une réponse.
