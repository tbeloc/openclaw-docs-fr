...
Statut : prêt pour la production via WhatsApp Web (Baileys). La passerelle possède la ou les session(s) liée(s).
...
<Note>
OpenClaw recommande d'exécuter WhatsApp sur un numéro séparé si possible. (Les métadonnées du canal et le flux d'intégration sont optimisés pour cette configuration, mais les configurations avec numéro personnel sont également prises en charge.)
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Secours avec numéro personnel">
    ...
    À l'exécution, les protections d'auto-chat utilisent le numéro personnel lié et `allowFrom`.
...
<Tabs>
  <Tab title="Politique DM">
    ...
    - les appairages sont persistés dans le magasin de blocage du canal et fusionnés avec le `allowFrom` configuré
    - si aucune liste de blocage n'est configurée, le numéro personnel lié est autorisé par défaut
  ...
  <Tab title="Politique de groupe + listes de blocage">
    ...
    1. **Liste de blocage d'appartenance au groupe** (`channels.whatsapp.groups`)
...
Lorsque le numéro personnel lié est également présent dans `allowFrom`, les protections d'auto-chat WhatsApp s'activent :
...
<AccordionGroup>
  ...
  <Accordion title="Espaces réservés de média et extraction de localisation/contact">
...
<AccordionGroup>
  <Accordion title="Fragmentation de texte">
    - limite de fragment par défaut : `channels.whatsapp.textChunkLimit = 4000`
    - `channels.whatsapp.chunkMode = "length" | "newline"`
    - le mode `newline` préfère les limites de paragraphe (lignes vides), puis revient à la fragmentation sûre en longueur
  </Accordion>
  ...
  <Accordion title="Comportement des médias sortants">
    ...
    - la lecture de GIF animé est prise en charge via `gifPlayback: true` sur les envois vidéo
    - les légendes sont appliquées au premier élément multimédia lors de l'envoi de charges utiles de réponse multi-médias
    - la source de média peut être HTTP(S), `file://`, ou des chemins locaux
  </Accordion>

  <Accordion title="Limites de taille de média et comportement de secours">
    - limite de sauvegarde de média entrant : `channels.whatsapp.mediaMaxMb` (par défaut `50`)
    - limite d'envoi de média sortant : `channels.whatsapp.mediaMaxMb` (par défaut `50`)
    - les remplacements par compte utilisent `channels.whatsapp.accounts.<accountId>.mediaMaxMb`
    - les images sont auto-optimisées (redimensionnement/balayage de qualité) pour s'adapter aux limites
...
- les échecs sont enregistrés mais ne bloquent pas la livraison de réponse normale
...
- Le support des outils d'agent inclut l'action de réaction WhatsApp (`react`).
- Portes d'action :
...
<AccordionGroup>
  <Accordion title="Non lié (QR requis)">
    Symptôme : rapports d'état de canal non liés.
  ...
  <Accordion title="Lié mais déconnecté / boucle de reconnexion">
    ...
    Si nécessaire, relancez la liaison avec `channels login`.

  </Accordion>

  <Accordion title="Aucun écouteur actif lors de l'envoi">
    Les envois sortants échouent rapidement quand aucun écouteur de passerelle actif n'existe pour le compte cible.
  ...
  <Accordion title="Messages de groupe ignorés de manière inattendue">
    ...
    - clés en double dans `openclaw.json` (JSON5) : les entrées ultérieures remplacent les entrées antérieures, donc conservez une seule `groupPolicy` par portée
