...
5. L'accès aux DM est par défaut en appairage ; approuvez le code d'appairage au premier contact.
...
- Ceci s'applique à la fois aux groupes en liste blanche et au mode groupe ouvert.
...
- La limite d'historique de groupe est par défaut `messages.groupChat.historyLimit` (secours `50`). Vous pouvez remplacer par compte avec `channels.zalouser.historyLimit`.
...
- L'action de réaction de message `react` est supportée pour `zalouser` dans les actions de canal.
  ...
  - Sémantique des réactions : [Réactions](/tools/reactions)
- Pour les messages entrants qui incluent les métadonnées d'événement, OpenClaw envoie les accusés de livraison et de lecture (meilleur effort).
