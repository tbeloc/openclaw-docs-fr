...
Même si un canal est autorisé (via `groupPolicy` + `groups`) et l'expéditeur est autorisé, OpenClaw par défaut **contrôle par mention** dans les contextes de groupe.
...
Pour plus d'informations sur l'accès au groupe par rapport au contrôle par mention (et comment ils interagissent), voir : [/channels/groups](/channels/groups).
