...
Les Broadcast Groups permettent à plusieurs agents de traiter et de répondre au même message simultanément. Cela vous permet de créer des équipes d'agents spécialisés qui travaillent ensemble dans un seul groupe WhatsApp ou DM — en utilisant un seul numéro de téléphone.

Portée actuelle : **WhatsApp uniquement** (canal web).

Les broadcast groups sont évalués après les listes blanches de canaux et les règles d'activation de groupe. Dans les groupes WhatsApp, cela signifie que les broadcasts se produisent quand OpenClaw répondrait normalement (par exemple : sur mention, selon vos paramètres de groupe).
...
Ajoutez une section `broadcast` au niveau supérieur (à côté de `bindings`). Les clés sont les identifiants de pairs WhatsApp :
...
2. **Vérification du broadcast** : Le système vérifie si l'ID de pair est dans `broadcast`
3. **Si dans la liste de broadcast** :
...
4. **Si pas dans la liste de broadcast** :
   - Le routage normal s'applique (première liaison correspondante)

Remarque : les broadcast groups ne contournent pas les listes blanches de canaux ou les règles d'activation de groupe (mentions/commandes/etc). Ils changent seulement _quels agents s'exécutent_ quand un message est éligible pour le traitement.

### Isolation des sessions

Chaque agent dans un broadcast group maintient complètement séparé :
...
- **Tampon de contexte de groupe** (messages récents du groupe utilisés pour le contexte) est partagé par pair, donc tous les agents de broadcast voient le même contexte quand ils sont déclenchés
...
- Limiter les broadcast groups à 5-10 agents
- Utiliser des modèles plus rapides pour les agents plus simples

### 5. Gérez les défaillances avec grâce
...
Les broadcast groups fonctionnent actuellement avec :
...
Les broadcast groups fonctionnent aux côtés du routage existant :
...
- `GROUP_B` : agent1 ET agent2 répondent (broadcast)
...
1. Les IDs d'agent existent dans `agents.list`
...
**Correction :** Ajoutez à la configuration de broadcast ou supprimez de bindings.
...
- `[peerId]` : JID de groupe WhatsApp, numéro E.164, ou autre ID de pair
  - Valeur : Tableau d'IDs d'agent qui doivent traiter les messages
