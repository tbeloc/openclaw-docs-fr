...
read_when:
  - Configuration de l'intégration de chat Twitch pour OpenClaw
title: "Twitch"
---
...
4. Configurez le jeton :
   ...
   - Si les deux sont définis, la config a la priorité (fallback env pour le compte par défaut uniquement).
...
Si les deux env et config sont définis, la config a la priorité.
...
Préférez `allowFrom` pour une liste blanche stricte. Utilisez `allowedRoles` à la place si vous voulez un accès basé sur les rôles.
...
Si vous voulez un accès basé sur les rôles, laissez `allowFrom` non défini et configurez `allowedRoles` à la place :
