...
4. L'accès aux messages directs est appairage par défaut ; approuver le code d'appairage au premier contact.
...
- Mode webhook : définir `channels.zalo.webhookUrl` et `channels.zalo.webhookSecret`.
  ...
  - Le HTTP de la passerelle gère les requêtes webhook à `channels.zalo.webhookPath` (par défaut le chemin de l'URL webhook).
...
- **Stickers** : Enregistrés mais pas entièrement traités (pas de réponse de l'agent).
