...
1. Installer le plugin Zalo :
   ...
   - Ou choisir **Zalo** lors de l'intégration et confirmer l'invite d'installation
2. Définir le token :
   - Env : `ZALO_BOT_TOKEN=...`
...
### 1) Créer un token de bot (Plateforme Zalo Bot)
...
- L'appairage est l'échange de token par défaut. Détails : [Pairing](/fr/channels/pairing)
...
- Mode webhook : définir `channels.zalo.webhookUrl` et `channels.zalo.webhookSecret`.
  ...
  - Les événements en double (`event_name + message_id`) sont ignorés pour une courte fenêtre de relecture.
...
- `channels.zalo.botToken`: token du bot depuis la plateforme Zalo Bot.
- `channels.zalo.tokenFile`: lire le token depuis un chemin de fichier régulier. Les symlinks sont rejetés.
...
- `channels.zalo.accounts.<id>.tokenFile`: fichier token régulier par compte. Les symlinks sont rejetés.
