...
- N'utilisez pas les noms d'affichage ou les parties locales nues (exemple : `"Alice"` ou `"alice"`). Ils sont ambigus et sont ignorés pour la correspondance de liste d'autorisation. Utilisez les ID complets `@user:server`.
...
- Pour ne permettre **aucun salon**, définissez `channels.matrix.groupPolicy: "disabled"` (ou conservez une liste d'autorisation vide).
...
- `channels.matrix.actions`: contrôle d'outils par action (réactions/messages/épingles/memberInfo/channelInfo).
