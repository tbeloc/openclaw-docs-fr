---
summary: "Canal Nostr DM via messages chiffrés NIP-04"
read_when:
...
| `dmPolicy`   | string   | `pairing`                               | Politique d'accès aux DMs           |
...
Les données de profil sont publiées en tant qu'événement NIP-01 `kind:0`. Vous pouvez les gérer à partir de l'interface de contrôle (Canaux -> Nostr -> Profil) ou les définir directement dans la configuration.
...
| NIP-04 | Supporté  | DMs chiffrés (`kind:4`)                |
| NIP-17 | Planifié  | DMs enveloppés en cadeau               |
