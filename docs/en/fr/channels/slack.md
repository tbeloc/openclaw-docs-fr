...
<Tabs>
  <Tab title="Politique DM">
    `channels.slack.dmPolicy` contrôle l'accès DM (héritage : `channels.slack.dm.policy`) :
  ...
  <Tab title="Mentions et utilisateurs de canal">
    ...
    - modèles regex de mention (`agents.list[].groupChat.mentionPatterns`, fallback `messages.groupChat.mentionPatterns`)
...
<AccordionGroup>
  ...
  <Accordion title="Texte et fichiers sortants">
    ...
    - les envois de fichiers utilisent les APIs de téléchargement Slack et peuvent inclure des réponses de thread (`thread_ts`)
...
- Les mises à jour du statut du thread assistant (pour les indicateurs "est en train de taper..." dans les threads) utilisent `assistant.threads.setStatus` et nécessitent la portée bot `assistant:write`.
...
## Réaction de saisie par défaut

`typingReaction` ajoute une réaction temporaire au message Slack entrant tandis qu'OpenClaw traite une réponse, puis la supprime lorsque l'exécution se termine. C'est un bon recours lorsque la saisie native de l'assistant Slack n'est pas disponible, en particulier dans les DM.
...
- `off` : désactiver la diffusion en continu de l'aperçu.
