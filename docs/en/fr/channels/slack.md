  ...
  <Tab title="Mode HTTP Events API">
    <Steps>
      <Step title="Configurer l'application Slack pour HTTP">
        ...
        - définissez l'URL de webhook pour Event Subscriptions + Interactivity + Slash command sur le même chemin (par défaut `/slack/events`)
...
<Tip>
Pour les lectures d'actions/répertoire, le token utilisateur peut être préféré quand configuré. Pour les écritures, le token bot reste préféré ; les écritures avec token utilisateur ne sont autorisées que quand `userTokenReadOnly: false` et que le token bot n'est pas disponible.
</Tip>
...
<Tabs>
  ...
  <Tab title="Politique de canal">
    ...
    - les entrées de liste d'autorisation de canal et les entrées de liste d'autorisation DM sont résolues au démarrage quand l'accès au token le permet
    - les entrées de nom de canal non résolues sont conservées comme configurées mais ignorées pour le routage par défaut
    - l'autorisation entrante et le routage de canal sont d'abord basés sur l'ID ; la correspondance directe nom d'utilisateur/slug nécessite `channels.slack.dangerouslyAllowNameMatching: true`
  ...
  <Tab title="Mentions et utilisateurs de canal">
    ...
    - format de clé `toolsBySender` : `id:`, `e164:`, `username:`, `name:`, ou wildcard `"*"`
      (les clés non préfixées héritées mappent toujours à `id:` uniquement)
...
- Quand les commandes natives sont activées, enregistrez les commandes slash correspondantes dans Slack (noms `/<command>`), avec une exception :
...
- Les menus d'arguments natifs adaptent maintenant leur stratégie de rendu :
  ...
  - plus de 100 options : sélection externe avec filtrage d'options asynchrone quand les gestionnaires d'options d'interactivité sont disponibles
...
Quand activée, les agents peuvent émettre des directives de réponse spécifiques à Slack :

- `[[slack_buttons: Approve:approve, Reject:reject]]`
- `[[slack_select: Choose a target | Canary:canary, Production:production]]`

Ces directives se compilent en Block Kit Slack et routent les clics ou sélections via le chemin d'événement d'interaction Slack existant.

Notes :

- Ceci est une interface utilisateur spécifique à Slack. Les autres canaux ne traduisent pas les directives Block Kit Slack dans leurs propres systèmes de boutons.
- Les valeurs de rappel interactif sont des tokens opaques générés par OpenClaw, pas des valeurs brutes créées par l'agent.
- Si les blocs interactifs générés dépasseraient les limites de Block Kit Slack, OpenClaw revient à la réponse texte originale au lieu d'envoyer une charge utile de blocs invalide.
...
et routent toujours l'exécution de commande contre la session de conversation cible (`CommandTargetSessionKey`).
...
- Les réponses de thread peuvent créer des suffixes de session de thread (`:thread:<threadTs>`) quand applicable.
...
- `channels.slack.thread.initialHistoryLimit` contrôle combien de messages de thread existants sont récupérés quand une nouvelle session de thread démarre (par défaut `20` ; définissez `0` pour désactiver).
...
Note : `replyToMode="off"` désactive **tout** threading de réponse dans Slack, y compris les tags explicites `[[reply_to_*]]`. Ceci diffère de Telegram, où les tags explicites sont toujours honorés en mode `"off"`. La différence reflète les modèles de threading de plateforme : les threads Slack masquent les messages du canal, tandis que les réponses Telegram restent visibles dans le flux de chat principal.

## Médias, chunking et livraison

<AccordionGroup>
  <Accordion title="Pièces jointes entrantes">
    Les pièces jointes de fichier Slack sont téléchargées à partir d'URLs privées hébergées par Slack (flux de requête authentifiée par token) et écrites dans le magasin de médias quand la récupération réussit et que les limites de taille le permettent.
  ...
  <Accordion title="Texte et fichiers sortants">
    ...
    - le plafond de médias sortants suit `channels.slack.mediaMaxMb` quand configuré ; sinon les envois de canal utilisent les valeurs par défaut de type MIME du pipeline de médias
  </Accordion>
...
```
  ...
  <Accordion title="Commandes natives/slash ne se déclenchent pas">
...
- [Référence de configuration - Slack](/gateway/configuration-reference#slack)
  ...
  - basculement de compatibilité : `dangerouslyAllowNameMatching` (verre brisé ; garder désactivé sauf si nécessaire)
...
- [Routage de canal](/channels/channel-routing)
