...
Lorsqu'un canal est configuré avec la politique DM `pairing`, les expéditeurs inconnus reçoivent un code court et leur message n'est **pas traité** jusqu'à ce que vous l'approuviez.
...
## 2) Appairage d'appareil nœud (nœuds iOS/Android/macOS/sans interface)
...
- L'API `node.pair.*` héritée (CLI : `openclaw nodes pending/approve`) est un
  magasin d'appairage distinct appartenant à la passerelle. Les nœuds WS nécessitent toujours l'appairage d'appareil.
