...
<Steps>
  ...
  <Step title="Étape 0 : Définir votre jeton de bot de manière sécurisée (ne l'envoyez pas en chat)">
    Votre jeton de bot Discord est un secret (comme un mot de passe). Définissez-le sur la machine exécutant OpenClaw avant de envoyer un message à votre agent.
...
OpenClaw supporte les conteneurs de composants Discord v2 pour les messages d'agent. Utilisez l'outil de message avec une charge utile `components`. Les résultats d'interaction sont acheminés vers l'agent en tant que messages entrants normaux et suivent les paramètres `replyToMode` Discord existants.
...
<Tabs>
  <Tab title="Politique DM">
    `channels.discord.dmPolicy` contrôle l'accès DM (héritage : `channels.discord.dm.policy`) :
    ...
    Les IDs numériques nus sont ambigus et rejetés sauf si un type de cible utilisateur/canal explicite est fourni.
  ...
  <Tab title="Politique de guilde">
    ...
    - listes blanches d'expéditeurs optionnelles : `users` (IDs stables recommandés) et `roles` (IDs de rôle uniquement) ; si l'un est configuré, les expéditeurs sont autorisés lorsqu'ils correspondent à `users` OU `roles`
    ...
    - les noms/tags sont supportés pour `users`, mais les IDs sont plus sûrs ; `openclaw security audit` avertit lorsque des entrées nom/tag sont utilisées
...
```
  ...
  <Tab title="Mentions et DMs de groupe">
    ...
    - comportement implicite de réponse au bot dans les cas supportés
    ...
    - liste blanche optionnelle via `dm.groupChannels` (IDs de canal ou slugs)
...
Utilisez `bindings[].match.roles` pour router les membres de guilde Discord vers différents agents par ID de rôle. Les liaisons basées sur les rôles acceptent uniquement les IDs de rôle et sont évaluées après les liaisons pair ou pair-parent et avant les liaisons uniquement de guilde. Si une liaison définit également d'autres champs de correspondance (par exemple `peer` + `guildId` + `roles`), tous les champs configurés doivent correspondre.
...
<AccordionGroup>
  ...
  <Accordion title="Intentions privilégiées">
    ...
    L'intention de présence est optionnelle et uniquement requise si vous souhaitez recevoir les mises à jour de présence. La définition de la présence du bot (`setPresence`) ne nécessite pas d'activer les mises à jour de présence pour les membres.
  ...
  <Accordion title="Copier les IDs">
    ...
    Préférez les IDs numériques dans la configuration OpenClaw pour des audits et des sondes fiables.
...
- `commands.native` par défaut à `"auto"` et est activé pour Discord.
...
Voir [Commandes slash](/tools/slash-commands) pour le catalogue des commandes et le comportement.
...
<AccordionGroup>
  ...
  <Accordion title="Aperçu du flux en direct">
    OpenClaw peut diffuser en continu les réponses brouillon en envoyant un message temporaire et en le modifiant au fur et à mesure que le texte arrive.
...
```
    ...
    Voir [Agents ACP](/tools/acp-agents) pour les détails du comportement de liaison.
...
```
  ...
  <Accordion title="Approbations d'exécution dans Discord">
    ...
    Docs connexes : [Approbations d'exécution](/tools/exec-approvals)
...
```
  ...
  <Accordion title="Exiger une mention fausse mais toujours bloquée">
    Causes courantes :
...
```
  ...
  <Accordion title="Incompatibilités d'audit des permissions">
    `channels status --probe` les vérifications de permissions ne fonctionnent que pour les ID de canal numériques.
