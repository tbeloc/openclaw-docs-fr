---
summary: "Support hérité d'iMessage via imsg (JSON-RPC sur stdio). Les nouvelles configurations doivent utiliser BlueBubbles."
read_when:
...
<Warning>
Pour les nouveaux déploiements d'iMessage, utilisez <a href="/channels/bluebubbles">BlueBubbles</a>.
...
```
  ...
  <Tab title="Mac distant via SSH">
    OpenClaw ne nécessite qu'un `cliPath` compatible avec stdio, vous pouvez donc pointer `cliPath` vers un script wrapper qui se connecte en SSH à un Mac distant et exécute `imsg`.
...
```
    ...
    OpenClaw utilise la vérification stricte des clés d'hôte pour SCP, la clé d'hôte du relais doit donc déjà exister dans `~/.ssh/known_hosts`.
...
<Tabs>
  ...
  <Tab title="Politique de groupe + mentions">
    ...
    - `allowlist` (par défaut si configuré)
    - `open`
    - `disabled`

    Liste blanche des expéditeurs de groupe : `channels.imessage.groupAllowFrom`.

    Secours à l'exécution : si `groupAllowFrom` n'est pas défini, les vérifications des expéditeurs de groupe iMessage se replient sur `allowFrom` si disponible.
    Note à l'exécution : si `channels.imessage` est complètement absent, le secours à l'exécution utilise `groupPolicy="allowlist"` et enregistre un avertissement (même si `channels.defaults.groupPolicy` est défini).

    Gating de mention pour les groupes :

    - iMessage n'a pas de métadonnées de mention natives
    - la détection de mention utilise des motifs regex (`agents.list[].groupChat.mentionPatterns`, secours `messages.groupChat.mentionPatterns`)
    - sans motifs configurés, le gating de mention ne peut pas être appliqué

    Les commandes de contrôle des expéditeurs autorisés peuvent contourner le gating de mention dans les groupes.
...
<AccordionGroup>
  <Accordion title="Utilisateur macOS bot dédié (identité iMessage séparée)">
    ...
    2. Se connecter à Messages avec l'Apple ID du bot dans cet utilisateur.
...
<AccordionGroup>
  <Accordion title="Pièces jointes et médias">
    - l'ingestion de pièces jointes entrantes est optionnelle : `channels.imessage.includeAttachments`
