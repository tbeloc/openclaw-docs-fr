...
<Tabs>
  ...
  <Tab title="Politique de groupe + mentions">
    ...
    Note d'exécution : si `channels.imessage` est complètement absent, le secours à l'exécution utilise `groupPolicy="allowlist"` et enregistre un avertissement (même si `channels.defaults.groupPolicy` est défini).
