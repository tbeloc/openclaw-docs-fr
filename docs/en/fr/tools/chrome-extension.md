...
OpenClaw est livré avec un profil de navigateur intégré nommé `chrome-relay` qui cible le relais d'extension sur le port par défaut.
...
- Outil d'agent : `browser` avec `profile="chrome-relay"`
...
Si vous utilisez un port de gateway personnalisé, le port relais d'extension est automatiquement dérivé :
...
Si votre Gateway s'exécute sur une autre machine, démarrez un hôte nœud sur la machine qui exécute Chrome. La Gateway proxiera les actions du navigateur vers ce nœud ; l'extension + relais restent locaux à la machine du navigateur.
