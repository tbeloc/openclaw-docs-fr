...
Exemple (liste d'autorisation spécifique au provider/model pour un endpoint instable) :
...
les conseils d'utilisation d'outils sont injectés dans les prompts. Certains plugins livrent leurs propres compétences
aux côtés des outils (par exemple, le plugin d'appel vocal).

Outils de plugin optionnels :

- [Lobster](/tools/lobster) : runtime de workflow typé avec approbations reprises (nécessite la CLI Lobster sur l'hôte gateway).
- [LLM Task](/tools/llm-task) : étape LLM JSON uniquement pour la sortie de workflow structurée (validation de schéma optionnelle).
- [Diffs](/tools/diffs) : visionneuse de diff en lecture seule et rendu de fichier PNG ou PDF pour les patches texte avant/après ou unifiés.
...
- Les réponses sont mises en cache (par défaut 15 min).
- Pour les sites gourmands en JS, préférer l'outil navigateur.
- Voir [Outils Web](/tools/web) pour la configuration.
...
- Les commandes caméra/écran nécessitent que l'application de nœud soit au premier plan.
...
Pour le comportement complet, les limites, la configuration et les exemples, voir [Outil PDF](/tools/pdf).
...
Redémarrer ou appliquer des mises à jour au processus Passerelle en cours d'exécution (sur place).

Actions principales :

- `restart` (autorise + envoie `SIGUSR1` pour redémarrage en processus ; `openclaw gateway` redémarrage sur place)
- `config.schema.lookup` (inspecter un chemin de config à la fois sans charger le schéma complet dans le contexte d'invite)
- `config.get`
- `config.apply` (valider + écrire config + redémarrer + réveiller)
- `config.patch` (fusionner mise à jour partielle + redémarrer + réveiller)
- `update.run` (exécuter mise à jour + redémarrer + réveiller)

Remarques :

- `config.schema.lookup` attend un chemin de config ciblé tel que `gateway.auth` ou `agents.list.*.heartbeat`.
...
- `session_status` : `sessionKey?` (défaut courant ; accepte `sessionId`), `model?` (`default` efface le remplacement)
...
- `sessions_spawn` supporte `runtime: "subagent" | "acp"` (par défaut `subagent`). Pour le comportement du runtime ACP, voir [Agents ACP](/tools/acp-agents).
...
- Les générations manuelles en mode achèvement envoient directement en premier, avec secours de file d'attente et nouvelle tentative en cas d'échecs transitoires (`status: "ok"` signifie que l'exécution s'est terminée, pas que l'annonce a été livrée).
...
- Guides de dépannage :
  - Problèmes de démarrage/CDP Linux : [Dépannage du navigateur (Linux)](/tools/browser-linux-troubleshooting)
  - WSL2 Gateway + Chrome CDP distant Windows : [Dépannage WSL2 + Windows + Chrome CDP distant](/tools/browser-wsl2-windows-remote-cdp-troubleshooting)
...
- Utilisez `status/describe` pour assurer les autorisations avant d'invoquer les commandes multimédias.

## Comment les outils sont présentés à l'agent

Les outils sont exposés sur deux canaux parallèles :

1. **Texte d'invite système** : une liste lisible par l'homme + des conseils.
2. **Schéma d'outil** : les définitions de fonction structurées envoyées à l'API du modèle.

Cela signifie que l'agent voit à la fois « quels outils existent » et « comment les appeler ». Si un outil n'apparaît pas dans l'invite système ou le schéma, le modèle ne peut pas l'appeler.
