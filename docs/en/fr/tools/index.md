...
OpenClaw expose des **outils agent de première classe** pour le navigateur, le canvas, les nœuds et cron.
...
Voir [Plugins](/tools/plugin) pour l'installation + configuration, et [Skills](/tools/skills) pour savoir comment
...
- `poll` retourne la nouvelle sortie et l'état de sortie une fois terminé.
...
- `upload` supporte également `inputRef` (ref aria) ou `element` (sélecteur CSS) pour définir directement `<input type="file">`.
...
- Guides de dépannage:
  ...
  - Passerelle WSL2 + Chrome CDP distant Windows: [Dépannage WSL2 + Windows + Chrome CDP distant](/tools/browser-wsl2-windows-remote-cdp-troubleshooting)
