...
OpenClaw peut utiliser **Firecrawl** comme extracteur de secours pour `web_fetch`. C'est un service d'extraction de contenu hébergé qui supporte la contournement des bots et la mise en cache, ce qui aide avec les sites lourds en JS ou les pages qui bloquent les requêtes HTTP simples.
...
Si le proxy est omis, Firecrawl utilise par défaut `auto`. `auto` réessaie avec des proxies furtifs si une tentative basique échoue, ce qui peut utiliser plus de crédits que le scraping basique uniquement.
