...
- Pour les sessions du harnais ACP (Codex, Claude Code, Gemini CLI), utilisez `sessions_spawn` avec `runtime: "acp"` et voir [ACP Agents](/fr/tools/acp-agents).
...
- `mode?` (`run|session`)
  - par défaut `run`
  - si `thread: true` et `mode` omis, par défaut devient `session`
