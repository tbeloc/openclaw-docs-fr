...
Si vous utilisez `OPENROUTER_API_KEY`, une clé `sk-or-...` dans `plugins.entries.perplexity.config.webSearch.apiKey`, ou si vous définissez `plugins.entries.perplexity.config.webSearch.baseUrl` / `model`, le fournisseur bascule vers le chemin chat-completions et retourne des réponses synthétisées par IA avec citations au lieu de résultats structurés de Search API.
...
Pour le chemin de compatibilité héritée Sonar/OpenRouter, seuls `query` et `freshness` sont supportés.
...
- Utilisez le préfixe `-` pour les entrées de liste noire (par ex., `["-reddit.com"]`)

## Notes

- L'API Perplexity Search retourne des résultats de recherche web structurés (`title`, `url`, `snippet`)
- OpenRouter ou `plugins.entries.perplexity.config.webSearch.baseUrl` / `model` explicite bascule Perplexity vers Sonar chat completions pour la compatibilité
