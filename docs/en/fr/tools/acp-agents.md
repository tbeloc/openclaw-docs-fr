...
Si vous demandez à OpenClaw en langage naturel « exécute ceci dans Codex » ou « démarre Claude Code dans un thread », OpenClaw devrait router cette demande vers le runtime ACP (et non vers le runtime natif des sous-agents).
...
- « Exécute ceci comme une session Claude Code ACP unique et résume le résultat. »
...
2. Résoudre la cible du harnais demandée (`agentId`, par exemple `codex`).
...
Utilisez ACP quand vous voulez un runtime de harnais externe. Utilisez les sous-agents quand vous voulez des exécutions déléguées natives à OpenClaw.
...
Le support de la liaison de thread est spécifique à l'adaptateur. Si l'adaptateur de canal actif ne supporte pas les liaisons de thread, OpenClaw retourne un message clair indiquant que c'est non supporté/indisponible.

Drapeaux de fonctionnalité requis pour ACP lié à un thread :

- `acp.enabled=true`
- `acp.dispatch.enabled` est activé par défaut (définir `false` pour mettre en pause la dispatch ACP)
...
Pour les workflows non éphémères, configurez les liaisons ACP persistantes dans les entrées `bindings[]` de haut niveau.
...
- Les liaisons de runtime temporaires (par exemple créées par des flux de focus de thread) s'appliquent toujours où elles sont présentes.
...
- `streamTo` (optionnel) : `"parent"` diffuse les résumés de progression initiale de la session ACP vers la session du demandeur en tant qu'événements système.
...
- Continuer une session de codage que vous avez démarrée de manière interactive dans la CLI, maintenant sans interface via votre agent
- Reprendre le travail qui a été interrompu par un redémarrage de passerelle ou un délai d'inactivité

Notes :

- `resumeSessionId` nécessite `runtime: "acp"` — retourne une erreur s'il est utilisé avec le runtime de sous-agent.
...
- Si l'id de session n'est pas trouvé, la création échoue avec une erreur claire — pas de repli silencieux vers une nouvelle session.
...
- Ne nécessitez pas `streamTo: "parent"` pour la porte basique. Ce chemin dépend des
  capacités du demandeur/session et est une vérification d'intégration séparée.
- Traitez le test `mode: "session"` lié à un thread comme une deuxième passe d'intégration
...
Certains contrôles dépendent des capacités du backend. Si un backend ne supporte pas un contrôle, OpenClaw retourne une erreur claire de contrôle non supporté.
...
| `ACP dispatch is disabled by policy (acp.dispatch.enabled=false)`        | Dispatch à partir des messages de thread normal désactivé.                      | Définissez `acp.dispatch.enabled=true`.                                                                                                                         |
...
| `Sandboxed sessions cannot spawn ACP sessions ...`                       | Le runtime ACP est côté hôte ; la session du demandeur est en sandbox.          | Utilisez `runtime="subagent"` à partir de sessions en sandbox, ou exécutez ACP spawn à partir d'une session non-sandbox.                                       |
| `sessions_spawn sandbox="require" is unsupported for runtime="acp" ...`  | `sandbox="require"` demandé pour le runtime ACP.                                | Utilisez `runtime="subagent"` pour le sandboxing requis, ou utilisez ACP avec `sandbox="inherit"` à partir d'une session non-sandbox.                           |
