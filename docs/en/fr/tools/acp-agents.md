...
Si vous demandez à OpenClaw en langage naturel « exécute ceci dans Codex » ou « démarre Claude Code dans un thread », OpenClaw devrait acheminer cette demande vers le runtime ACP (et non vers le runtime natif du sous-agent).
...
- « Démarrez une session Codex persistante dans un thread ici et gardez-la concentrée. »
- « Exécutez ceci comme une session ACP Claude Code unique et résumez le résultat. »
- « Utilisez Gemini CLI pour cette tâche dans un thread, puis gardez les suites dans le même thread. »

Ce qu'OpenClaw devrait faire :

1. Choisissez `runtime: "acp"`.
2. Résolvez la cible de harnais demandée (`agentId`, par exemple `codex`).
3. Si la liaison de thread est demandée et que le canal actuel la supporte, liez la session ACP au thread.
4. Acheminez les messages de thread de suivi vers la même session ACP jusqu'à ce qu'elle soit défocalisée/fermée/expirée.
...
- Transférez une session Codex de votre ordinateur portable à votre téléphone — dites à votre agent de reprendre où vous l'aviez laissé
- Continuez une session de codage que vous avez démarrée de manière interactive dans la CLI, maintenant sans tête via votre agent
- Reprenez le travail qui a été interrompu par un redémarrage de passerelle ou un délai d'inactivité
