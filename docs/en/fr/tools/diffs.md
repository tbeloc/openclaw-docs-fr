...
Si vous souhaitez garder l'outil `diffs` activé mais désactiver ses conseils d'invite système intégrés, définissez `plugins.entries.diffs.hooks.allowPromptInjection` sur `false` :
...
Définissez les valeurs par défaut du plugin dans `~/.openclaw/openclaw.json` :
...
- Les métadonnées de l'artefact de visionneuse contiennent :
  ...
  - jeton aléatoire (48 caractères hex)
...
- Le nettoyage de secours supprime les dossiers obsolètes de plus de 24 heures quand les métadonnées manquent.
...
- Chemins de visionneuse tokenisés avec validation stricte de l'ID et du jeton.
