...
Lorsqu'il est activé, le plugin ajoute des conseils d'utilisation concis dans l'espace du système-prompt et expose également une compétence détaillée pour les cas où l'agent a besoin d'instructions plus complètes.
...
Si vous souhaitez garder l'outil `diffs` activé mais désactiver ses conseils système-prompt intégrés, définissez `plugins.entries.diffs.hooks.allowPromptInjection` sur `false` :
...
- `ttlSeconds` (`number`) : TTL d'artefact de visionneuse en secondes. Par défaut 1800, max 21600.
- `baseUrl` (`string`) : remplacement d'origine d'URL de visionneuse. Doit être `http` ou `https`, pas de query/hash.
...
- Limite de complexité du patch : max 128 fichiers et 120000 lignes totales.
...
Définissez les défauts à l'échelle du plugin dans `~/.openclaw/openclaw.json` :
...
Les paramètres d'outil explicites remplacent ces défauts.
...
- Le TTL de visionneuse par défaut est 30 minutes quand non spécifié.
- Le TTL de visionneuse maximum accepté est 6 heures.
- Le nettoyage s'exécute opportunément après la création d'artefact.
...
- Si le mode de liaison de passerelle est `custom` et `gateway.customBindHost` est défini, cet hôte est utilisé.

Règles `baseUrl` :

- Doit être `http://` ou `https://`.
- Query et hash sont rejetés.
- Origine plus chemin de base optionnel est autorisé.
...
- Chemins de visionneuse tokenisés avec validation stricte d'ID et de token.
- CSP de réponse de visionneuse :
  - `default-src 'none'`
  - scripts et ressources uniquement depuis self
  - pas de `connect-src` sortant
- Limitation de débit de miss distant quand l'accès distant est activé :
  - 40 défaillances par 60 secondes
  - Verrouillage de 60 secondes (`429 Too Many Requests`)

Durcissement du rendu de fichier :

- Le routage de requête du navigateur de capture d'écran est deny-by-default.
...
- Activez `security.allowRemoteViewer` uniquement quand vous avez l'intention d'un accès de visionneuse externe.
...
- C'est le comportement attendu et n'indique pas une défaillance de visionneuse.
...
- Si votre canal compresse les images agressivement (par exemple Telegram ou WhatsApp), préférez la sortie PDF (`fileFormat: "pdf"`).
