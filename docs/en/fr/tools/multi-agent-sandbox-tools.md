...
`setupCommand` se trouve sous `sandbox.docker` (global ou par agent) et s'exécute une seule fois
...
Pour savoir comment le sandboxing se comporte à l'exécution, voir [Sandboxing](/gateway/sandboxing).
...
### Exemple 3 : Modes Sandbox Différents par Agent
    ...
    "list": [
      {
        ...
        "workspace": "~/.openclaw/workspace",
        "sandbox": {
          "mode": "off" // Remplace : main jamais sandboxé
        }
      },
      {
        "id": "public",
        ...
        "sandbox": {
          "mode": "all", // Remplace : public toujours sandboxé
          "scope": "agent"
        },
        "tools": {
          "allow": ["read"],
...
- `agents.list[].sandbox.{docker,browser,prune}.*` remplace `agents.defaults.sandbox.{docker,browser,prune}.*` pour cet agent (ignoré quand la portée du sandbox se résout en `"shared"`).
...
8. **Politique d'outil de sous-agent** (`tools.subagents.tools`, le cas échéant)

Chaque niveau peut restreindre davantage les outils, mais ne peut pas accorder les outils refusés par les niveaux antérieurs.
...
Les politiques d'outils (global, agent, sandbox) supportent les entrées `group:*` qui se développent en plusieurs outils concrets :
...
- Désactiver le mode élevé globalement (`tools.elevated.enabled: false`) si vous ne voulez que l'exécution en sandbox
...
- Vérifier l'ordre de filtrage des outils : global → agent → sandbox → sous-agent
- Chaque niveau ne peut que restreindre davantage, pas accorder à nouveau
- Vérifier avec les journaux : `[tools] filtering tools for agent:${agentId}`

### Conteneur non isolé par agent

- Définir `scope: "agent"` dans la configuration sandbox spécifique à l'agent
