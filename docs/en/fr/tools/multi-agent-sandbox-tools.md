...
Pour savoir comment le bac à sable se comporte à l'exécution, voir [Sandboxing](/gateway/sandboxing).
Pour déboguer « pourquoi ceci est-il bloqué ? », voir [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) et `openclaw sandbox explain`.
...
Quand les configurations globales (`agents.defaults.*`) et spécifiques à l'agent (`agents.list[].*`) existent :
