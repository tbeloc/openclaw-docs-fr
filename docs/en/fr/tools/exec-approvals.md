...
Si l'interface utilisateur de l'application compagne n'est **pas disponible**, toute demande nécessitant une invite
...
sont traités comme étant dans la liste blanche sur les nœuds (nœud macOS ou hôte nœud sans interface). Cela utilise
`skills.bins` sur le RPC Gateway pour récupérer la liste des bins de compétences. Désactivez ceci si vous voulez des listes blanches strictement manuelles.
...
- Si vous exigez une confiance strictement explicite, gardez `autoAllowSkills: false` et utilisez uniquement les entrées de liste blanche de chemin manuel.
...
le binaire shell lui-même est dans la liste blanche.
...
rejetée donc les opérandes de fichier ne peuvent pas être contrebandés comme des positionnels ambigus.
...
Pour `host=node`, les demandes d'approbation incluent une charge utile `systemRunPlan` canonique. La gateway utilise ce plan comme commande/cwd/contexte de session faisant autorité lors du transfert des demandes `system.run` approuvées.
...
Le cycle de vie Exec est présenté comme des messages système :
