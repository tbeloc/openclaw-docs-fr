...
Les approbations exec sont le **garde-fou de l'application compagnon / hôte nœud** permettant à un agent en sandbox d'exécuter
...
Les approbations exec s'ajoutent **en plus** à la politique d'outils et au contrôle d'élévation (sauf si elevated est défini sur `full`, ce qui ignore les approbations).
...
Si l'interface utilisateur de l'application compagnon n'est **pas disponible**, toute demande nécessitant une invite
est résolue par le **fallback ask** (par défaut : refuser).
...
- Les exécutions de nœud approuvées lient le contexte d'exécution canonique : répertoire de travail canonique, argv exact, liaison env
  le cas échéant, et chemin d'exécutable épinglé le cas échéant.
...
sont traités comme en liste blanche sur les nœuds (nœud macOS ou hôte nœud sans interface). Cela utilise
`skills.bins` sur l'appel RPC Gateway pour récupérer la liste des bins de compétences. Désactivez ceci si vous voulez des listes blanches strictement manuelles.
...
Les bins sûrs doivent également se résoudre à partir de répertoires binaires de confiance (valeurs par défaut du système plus
...
(y compris les bins sûrs ou l'auto-allow de compétence). Les redirections restent non supportées en mode liste blanche.
...
Sur les approbations de l'application compagnon macOS, le texte shell brut contenant le contrôle shell ou la syntaxe d'expansion
...
(`env`, `nice`, `nohup`, `stdbuf`, `timeout`) persistent les chemins d'exécutable internes au lieu des chemins wrapper.
...
multiplexeur ne peut pas être déplie en toute sécurité, aucune entrée de liste blanche n'est persistée automatiquement.
...
| Type de correspondance       | Nom d'exécutable + politique argv safe-bin                 | Motif glob de chemin d'exécutable résolu                        |
...
CLI : `openclaw approvals` prend en charge l'édition de gateway ou de nœud (voir [Approvals CLI](/fr/cli/approvals)).
...
- Challenge/response (nonce + jeton HMAC + hash de demande) + TTL court.
