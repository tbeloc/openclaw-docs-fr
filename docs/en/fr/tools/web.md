...
- `web_search` — Recherchez le web en utilisant l'API Brave Search, Gemini avec ancrage Google Search, Grok, Kimi ou l'API Perplexity Search.
- `web_fetch` — Récupération HTTP + extraction lisible (HTML → markdown/texte).

Ce ne sont **pas** des automatisations de navigateur. Pour les sites lourds en JS ou les connexions, utilisez l'
...
- En mode détection automatique, OpenClaw résout uniquement la clé du fournisseur sélectionné. Les SecretRefs des fournisseurs non sélectionnés restent inactifs jusqu'à sélection.
...
Les modèles Gemini supportent l'[ancrage Google Search](https://ai.google.dev/gemini-api/docs/grounding) intégré,
...
**Alternative environnement :** définissez `GEMINI_API_KEY` dans l'environnement Gateway.
Pour une installation de passerelle, mettez-le dans `~/.openclaw/.env`.

### Notes

- Les URL de citation de l'ancrage Gemini sont automatiquement résolues à partir des URL de redirection de Google vers des URL directes.
...
- La résolution de redirection utilise les paramètres par défaut stricts SSRF, donc les redirections vers des cibles privées/internes sont bloquées.
- Le modèle par défaut (`gemini-2.5-flash`) est rapide et rentable.
  N'importe quel modèle Gemini qui supporte l'ancrage peut être utilisé.
...
`date_before` ne sont pas supportés. Utilisez le mode Brave `web` pour ces filtres.
