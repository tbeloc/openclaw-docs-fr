...
[outil Browser](/fr/tools/browser).
...
Consultez [Configuration Brave Search](/fr/brave-search) et [Configuration Perplexity Search](/fr/perplexity) pour les détails spécifiques au fournisseur.
...
- En mode détection automatique, OpenClaw résout uniquement la clé du fournisseur sélectionné. Les SecretRefs des fournisseurs non sélectionnés restent inactifs jusqu'à leur sélection.
...
Pour une installation de passerelle, mettez-les dans `~/.openclaw/.env` (ou votre environnement de service). Consultez [Variables d'environnement](/fr/help/faq#how-does-openclaw-load-environment-variables).
...
- Les URL de citation du grounding Gemini sont automatiquement résolues à partir des URL de redirection de Google vers des URL directes.
...
Recherchez sur le web en utilisant votre fournisseur configuré.
...
- Voir [Firecrawl](/fr/tools/firecrawl) pour la configuration des clés et les détails du service.
