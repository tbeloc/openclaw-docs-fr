...
Si vous devez utiliser snap Chromium, configurez OpenClaw pour qu'il se connecte à un navigateur démarré manuellement :
...
### Problème : « Chrome extension relay est en cours d'exécution, mais aucun onglet n'est connecté »

Vous utilisez le profil `chrome-relay` (extension relay). Il s'attend à ce que l'extension du navigateur OpenClaw soit attachée à un onglet actif.
