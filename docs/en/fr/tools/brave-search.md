...
3. Stockez la clé dans la configuration ou définissez `BRAVE_API_KEY` dans l'environnement de la passerelle.
...
Les paramètres de recherche Brave spécifiques au fournisseur se trouvent maintenant sous `plugins.entries.brave.config.webSearch.*`.
L'ancienne clé `tools.web.search.apiKey` se charge toujours via le shim de compatibilité, mais ce n'est plus le chemin de configuration canonique.
...
- OpenClaw utilise le plan Brave **Search**. Si vous avez un abonnement hérité (par ex. le plan gratuit original avec 2 000 requêtes/mois), il reste valide mais n'inclut pas les fonctionnalités plus récentes comme LLM Context ou des limites de débit plus élevées.
...
- Le plan Search inclut le point de terminaison LLM Context et les droits d'inférence IA. Le stockage des résultats pour entraîner ou affiner les modèles nécessite un plan avec des droits de stockage explicites. Consultez les [Conditions d'utilisation](https://api-dashboard.search.brave.com/terms-of-service) de Brave.
