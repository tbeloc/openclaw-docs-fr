...
Ces caches réduisent le démarrage en rafales et la surcharge de commandes répétées. Ils sont sûrs à considérer comme des caches de performance de courte durée, pas de persistance.
...
- Les plugins nouveaux et migrés groupés doivent utiliser les sous-chemins spécifiques au canal ou à l'extension ; utilisez `core` pour les surfaces génériques et `compat` uniquement quand des assistants runtime partagés plus larges sont requis.
...
- Les chemins candidats sont vérifiés pour la sécurité avant l'admission de découverte. OpenClaw bloque les candidats quand :
  - l'entrée d'extension se résout en dehors de la racine du plugin (y compris les échappements de symlink/traversée de chemin),
...
Dans le cœur actuel, les ids intégrés activés par défaut incluent les aides locales/fournisseur telles que
...
après résolution de symlink. Les entrées qui s'échappent du répertoire du paquet sont
rejetées.
...
les indices d'installation via `openclaw.install`. Cela garde les données du catalogue libres du cœur.
...
Les plugins chargés ne mutent pas directement les globals aléatoires du cœur. Ils s'enregistrent dans un
...
- exécution du cœur -> consommation de registre

Cette séparation est importante pour la maintenabilité. Cela signifie que la plupart des surfaces du cœur n'ont besoin que
...
Utilisez ceci quand votre plugin a besoin de remplacer ou d'étendre le pipeline de contexte par défaut
plutôt que juste d'ajouter la recherche de mémoire ou des hooks.
...
- les liaisons `methodId` invalides sont réduites à des avertissements et reviennent aux méthodes d'authentification restantes du fournisseur
...
Ils peuvent toujours devenir des plugins, mais le principal avantage de modularité provient de l'extraction des fournisseurs riches en comportement en premier.
