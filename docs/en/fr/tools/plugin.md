...
Les plugins OpenClaw sont des **modules TypeScript** chargés à l'exécution via jiti. **La validation de la configuration n'exécute pas le code du plugin** ; elle utilise à la place le manifeste du plugin et JSON Schema. Voir [Manifeste du plugin](/plugins/manifest).
...
Le module runtime est la partie plan de données. Il enregistre le comportement réel tel que les hooks, les outils, les commandes ou les flux de fournisseur.
...
- les registres des plugins chargés
...
- `auth` : requis. Utilisez `"gateway"` pour exiger l'authentification gateway normale, ou `"plugin"` pour l'authentification gérée par le plugin/vérification du webhook.
...
- `openclaw/plugin-sdk/core` pour les APIs de plugin génériques, les types d'authentification du fournisseur et les assistants partagés.
...
- Inclure les champs de source/statut des identifiants lorsqu'ils sont pertinents, tels que :
  - `tokenSource`, `tokenStatus`
  - `botTokenSource`, `botTokenStatus`
  - `appTokenSource`, `appTokenStatus`
  - `signingSecretSource`, `signingSecretStatus`
- Vous n'avez pas besoin de retourner les valeurs brutes des jetons juste pour signaler la
...
au lieu de planter ou de mal signaler le compte comme non configuré.

Note de performance :

- La découverte de plugins et les métadonnées de manifeste utilisent des caches courts en mémoire pour réduire
...
Exceptions de plugins intégrés activés par défaut :
...
ou les ajoutez à une liste blanche. C'est intentionnel : un dépôt extrait ne devrait pas
devenir silencieusement du code de passerelle de production.

Notes de durcissement :

- Si `plugins.allow` est vide et que des plugins non intégrés sont découvrables, OpenClaw enregistre un avertissement au démarrage avec les ids et sources des plugins.
- Les chemins candidats sont vérifiés pour la sécurité avant l'admission de découverte. OpenClaw bloque les candidats lorsque :
...
- Les plugins non intégrés chargés sans provenance d'installation/chemin de chargement émettent un avertissement pour que vous puissiez épingler la confiance (`plugins.allow`) ou l'installation de suivi (`plugins.installs`).
...
Si plusieurs plugins se résolvent au même id, la première correspondance dans l'ordre ci-dessus
...
- `plugins.allow: ["foo"]` autorise le plugin `foo` actif par id, même lorsque
...
- les listes blanches restreignent l'ensemble actif lorsque `plugins.allow` est non vide
...
Dans le noyau actuel, les ids intégrés activés par défaut incluent les aides locales/fournisseur telles que
...
les indices d'installation via `openclaw.install`. Cela garde les données du catalogue sans données.
...
OpenClaw peut également fusionner les **catalogues de canaux externes** (par exemple, une exportation de registre MPM).
...
Les plugins chargés ne mutent pas directement les globales aléatoires du noyau. Ils s'enregistrent dans un
...
Les fonctionnalités principales lisent ensuite ce registre au lieu de parler directement aux modules de plugin.
Cela garde le chargement unidirectionnel :

- module de plugin -> enregistrement de registre
- exécution du noyau -> consommation de registre

Cette séparation est importante pour la maintenabilité. Cela signifie que la plupart des surfaces du noyau n'ont besoin que
...
- `contextEngine` : plugin de moteur de contexte actif (`"legacy"` est le défaut intégré)
...
plutôt que simplement ajouter une recherche de mémoire ou des hooks.
...
- Les valeurs `before_prompt_build` sont appliquées avant les valeurs de secours hérités `before_agent_start`.
...
1. Choisissez un id + forme de configuration
...
Consultez [Voice Call](/plugins/voice-call) et `extensions/voice-call/README.md` pour la configuration et l'utilisation.
