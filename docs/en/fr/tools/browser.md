...
- `color` + `color` par profil teignent l'interface utilisateur du navigateur pour que vous puissiez voir quel profil est actif.
...
- la Gateway ou le nœud hôte peut lancer `npx chrome-devtools-mcp@latest --autoConnect`
...
- OpenClaw utilise le flux officiel Chrome DevTools MCP `--autoConnect` ici, pas
...
- Laissez le relais loopback uniquement par défaut. Si le relais doit être accessible à partir d'un espace de noms réseau différent (par exemple Gateway dans WSL2, Chrome sur Windows), définissez `browser.relayBindHost` sur une adresse de liaison explicite comme `0.0.0.0` tout en gardant le réseau environnant privé et authentifié.
...
`OPENCLAW_HOME_VOLUME` ou un montage de liaison. Voir [Docker](/install/docker).
...
- `upload` et `dialog` sont des appels **d'armement** ; exécutez-les avant le clic/appui
  qui déclenche le sélecteur/dialogue.
...
- `snapshot` :
  ...
  - Défaut de configuration (outil/CLI uniquement) : définissez `browser.snapshotDefaults.mode: "efficient"` pour utiliser des snapshots efficaces quand l'appelant ne passe pas de mode (voir [Configuration de la passerelle](/gateway/configuration#browser-openclaw-managed-browser)).
...
Quand une action échoue (par exemple « non visible », « violation du mode strict », « couvert ») :
...
- Pour les connexions et les notes anti-bot (X/Twitter, etc.), voir [Connexion au navigateur + publication X/Twitter](/tools/browser-login).
...
[Dépannage WSL2 + Windows + CDP Chrome distant](/tools/browser-wsl2-windows-remote-cdp-troubleshooting).
...
- `browser snapshot` retourne un arbre d'interface utilisateur stable (IA ou ARIA).
...
- `browser` accepte :
  ...
  - Si un nœud capable de navigateur est connecté, l'outil peut se rediriger automatiquement vers lui sauf si vous épinglez `target="host"` ou `target="node"`.
