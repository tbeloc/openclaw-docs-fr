...
Vous pouvez les gater via `metadata.openclaw.requires.config` sur l'entrée de configuration du plugin. Voir [Plugins](/fr/tools/plugin) pour la découverte/configuration et [Outils](/fr/tools) pour la
...
- Préférez les exécutions en sandbox pour les entrées non approuvées et les outils risqués. Voir [Sandboxing](/fr/gateway/sandboxing).
...
- Les installations Node honorent `skills.install.nodeManager` dans `openclaw.json` (par défaut : npm ; options : npm/pnpm/yarn/bun).
  Cela n'affecte que les **installations de compétences** ; le runtime de la passerelle doit toujours être Node
...
- Téléchargements : `url` (requis), `archive` (`tar.gz` | `tar.bz2` | `zip`), `extract` (par défaut : auto quand archive détectée), `stripComponents`, `targetDir` (par défaut : `~/.openclaw/tools/<skillKey>`).
...
- `apiKey` : commodité pour les compétences qui déclarent `metadata.openclaw.primaryEnv`.
  Supporte la chaîne en texte brut ou l'objet SecretRef (`{ source, provider, id }`).
