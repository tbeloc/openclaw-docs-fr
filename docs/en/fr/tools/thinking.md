...
4. Secours : `adaptive` pour les modèles Anthropic Claude 4.6, `low` pour les autres modèles compatibles avec le raisonnement, `off` sinon.
...
- Cela persiste pour la session actuelle (par expéditeur par défaut) ; effacé par `/think:off` ou réinitialisation d'inactivité de session.
