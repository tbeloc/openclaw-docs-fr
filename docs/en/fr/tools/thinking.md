...
- Cela reste pour la session actuelle (par défaut par expéditeur) ; effacé par `/think:off` ou réinitialisation d'inactivité de session.
...
- Pour `openai-codex/*`, le mode rapide applique le même profil à faible latence sur les réponses Codex. OpenClaw maintient un seul basculement `/fast` partagé entre les deux chemins d'authentification.
...
## Directives verboses (/verbose ou /v)

- Niveaux : `on` (minimal) | `full` | `off` (défaut).
- Un message contenant uniquement la directive bascule la verbosité de session et répond `Verbose logging enabled.` / `Verbose logging disabled.` ; les niveaux invalides retournent un indice sans changer l'état.
...
- Quand la verbosité est activée, les agents qui émettent des résultats d'outils structurés (Pi, autres agents JSON) renvoient chaque appel d'outil comme son propre message contenant uniquement des métadonnées, préfixé avec `<emoji> <tool-name>: <arg>` quand disponible (chemin/commande). Ces résumés d'outils sont envoyés dès que chaque outil démarre (bulles séparées), pas comme des deltas de streaming.
- Les résumés d'échec d'outil restent visibles en mode normal, mais les suffixes de détail d'erreur bruts sont masqués sauf si la verbosité est `on` ou `full`.
- Quand la verbosité est `full`, les sorties d'outils sont également transmises après la fin (bulle séparée, tronquée à une longueur sûre). Si vous basculez `/verbose on|full|off` pendant qu'une exécution est en cours, les bulles d'outils suivantes respectent le nouveau paramètre.
