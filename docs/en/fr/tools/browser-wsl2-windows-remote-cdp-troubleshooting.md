...
read_when:
  ...
  - Décision entre CDP distant brut et le relais d'extension Chrome dans les configurations multi-hôtes
...
Si vous utilisez le relais d'extension sur les espaces de noms, `browser.relayBindHost` est le paramètre important introduit dans [Browser](/fr/tools/browser) et [Extension Chrome](/fr/tools/chrome-extension).
...
- le relais d'extension est toujours loopback uniquement alors que vous avez réellement besoin d'un accès multi-espaces de noms
...
Ne passez pas par défaut à une adresse IP LAN pour l'interface de contrôle. HTTP brut sur une adresse LAN ou tailnet peut déclencher un comportement d'authentification d'appareil/origine non sécurisée sans rapport avec CDP lui-même. Voir [Interface de contrôle](/fr/web/control-ui).
...
Pour CDP distant brut, pointez OpenClaw vers l'adresse accessible depuis WSL2 :
