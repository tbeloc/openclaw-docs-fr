...
Ce guide couvre la configuration split-host courante où :
...
Utilisez le profil `chrome` intégré plus l'extension OpenClaw Chrome.
...
- le relais d'extension est toujours loopback uniquement quand vous avez réellement besoin d'un accès cross-namespace
...
Ne pas utiliser par défaut une adresse IP LAN pour l'interface de contrôle. HTTP brut sur une adresse LAN ou tailnet peut déclencher un comportement d'authentification d'appareil/origine non sécurisée qui n'est pas lié à CDP lui-même. Voir [Interface de contrôle](/web/control-ui).
...
- le comportement par défaut est plus sûr car le relais reste loopback uniquement
