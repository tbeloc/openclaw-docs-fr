```markdown
---
summary: "Dépanner la passerelle WSL2 + Windows Chrome CDP distant et les configurations de relais d'extension en couches"
read_when:
  - Exécution d'OpenClaw Gateway dans WSL2 tandis que Chrome se trouve sur Windows
  - Affichage d'erreurs de navigateur/interface de contrôle qui se chevauchent sur WSL2 et Windows
  - Décision entre le CDP distant brut et le relais d'extension Chrome dans les configurations d'hôtes séparés
title: "Dépannage WSL2 + Windows + CDP Chrome distant"
---

# Dépannage WSL2 + Windows + CDP Chrome distant

Ce guide couvre la configuration d'hôtes séparés courante où :
...
Il couvre également le modèle de défaillance en couches du [problème #39369](https://github.com/openclaw/openclaw/issues/39369) : plusieurs problèmes indépendants peuvent apparaître à la fois, ce qui fait que la mauvaise couche semble cassée en premier.
...
- vous préférez le contrôle basé sur l'extension au lieu du `--remote-debugging-port` brut
- le relais lui-même doit être accessible à travers la limite WSL2/Windows

Si vous utilisez le relais d'extension à travers les espaces de noms, `browser.relayBindHost` est le paramètre important introduit dans [Browser](/tools/browser) et [Extension Chrome](/tools/chrome-extension).

## Architecture fonctionnelle

Forme de référence :

- WSL2 exécute la passerelle sur `127.0.0.1:18789`
...
- le relais d'extension est toujours en boucle locale alors que vous avez réellement besoin d'un accès entre espaces de noms
...
Ne pas utiliser par défaut une adresse IP LAN pour l'interface de contrôle. HTTP brut sur une adresse LAN ou tailnet peut déclencher un comportement d'authentification d'appareil/origine non sécurisée qui n'est pas lié au CDP lui-même. Voir [Interface de contrôle](/web/control-ui).
...
Pour le CDP distant brut, pointez OpenClaw vers l'adresse qui est accessible depuis WSL2 :
...
- utilisez l'adresse accessible par WSL2, pas ce qui ne fonctionne que sur Windows
...
Si la machine du navigateur et la passerelle sont séparées par une limite d'espace de noms, le relais peut avoir besoin d'une adresse de liaison non-loopback.
...
- le comportement par défaut est plus sûr car le relais reste en boucle locale
- `0.0.0.0` élargit la surface d'exposition
- gardez l'authentification de la passerelle, l'appairage des nœuds et le réseau environnant privés
...
- `control-ui-insecure-auth`
  - Problème d'origine de l'interface utilisateur / contexte sécurisé, pas un problème de transport CDP
- `token_missing`
...
3. Configuration OpenClaw : `browser.profiles.<name>.cdpUrl` utilise-t-il cette adresse exacte accessible par WSL2 ?
