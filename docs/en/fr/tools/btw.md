...
que cette question ne devienne un historique de conversation normal.

Elle est modélisée d'après le comportement `/btw` de Claude Code, mais adaptée à
...
5. **n'écrit pas** la question BTW ou la réponse dans l'historique de session,
...
Si la tâche principale est actuellement active, OpenClaw crée un instantané de l'état du message
actuel et inclut l'invite principale en cours comme contexte d'arrière-plan, tout en
disant explicitement au modèle :
...
Cela maintient BTW isolée de la tâche principale tout en la rendant consciente du sujet
de la session.
...
réponse unique clairement étiquetée car ces surfaces n'ont pas de concept
de superposition éphémère locale.
...
rendre BTW en direct dans le navigateur. Jusqu'à ce que ce support côté client soit implémenté, BTW est une
