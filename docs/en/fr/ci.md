...
| `changed-scope`   | Détecter les zones modifiées (node/macos/android/windows) | Changements non-doc                |
| `check`           | Types TypeScript, lint, format                          | Non-docs, changements node         |
| `check-docs`      | Lint Markdown + vérification des liens cassés           | Docs modifiées                     |
| `secrets`         | Détecter les secrets divulgués                          | Toujours                           |
| `build-artifacts` | Construire dist une fois, partager avec `release-check` | Pushes vers `main`, changements node |
| `release-check`   | Valider le contenu du pack npm                          | Pushes vers `main` après la construction |
| `checks`          | Tests Node + vérification du protocole sur les PRs ; compatibilité Bun sur push | Non-docs, changements node |
| `compat-node22`   | Compatibilité du runtime Node minimum supporté          | Pushes vers `main`, changements node |
| `checks-windows`  | Tests spécifiques à Windows                             | Non-docs, changements pertinents pour windows |
