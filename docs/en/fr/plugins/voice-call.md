      ...
      "voice-call": {
        ...
        config: {
          ...
          telnyx: {
            apiKey: "...",
            connectionId: "...",
            // Clé publique du webhook Telnyx depuis le portail Telnyx Mission Control
...
- Les valeurs par défaut de sécurité du streaming :
  - `streaming.preStartTimeoutMs` ferme les sockets qui n'envoient jamais une trame `start` valide.
  - `streaming.maxPendingConnections` limite les sockets pré-démarrage non authentifiés totaux.
  - `streaming.maxPendingConnectionsPerIp` limite les sockets pré-démarrage non authentifiés par IP source.
  - `streaming.maxConnections` limite les sockets de flux média ouverts totaux (en attente + actifs).
...
Quand un proxy ou un tunnel se trouve devant la Gateway, le plugin reconstruit l'
URL publique pour la vérification de signature. Ces options contrôlent quels en-têtes transférés
sont de confiance.
...
`webhookSecurity.trustedProxyIPs` ne fait confiance aux en-têtes transférés que quand l'IP
...
- **Edge TTS est ignoré pour les appels vocaux** (l'audio téléphonique a besoin de PCM ; la sortie Edge n'est pas fiable).
...
Remplacer par ElevenLabs juste pour les appels (garder la valeur par défaut du cœur ailleurs) :
