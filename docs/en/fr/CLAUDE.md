...
- La documentation en langue étrangère n'est pas maintenue dans ce repo. La sortie de publication générée se trouve dans le repo séparé `openclaw/docs` (souvent cloné localement sous `../openclaw-docs`).
- N'ajoutez pas ou ne modifiez pas la documentation localisée sous `docs/<locale>/**` ici.
...
- `pnpm docs:check-i18n-glossary` est la protection pour les titres de documents en anglais modifiés et les étiquettes de documents internes courtes.
