...
- `/context detail` → décomposition plus approfondie : par fichier, tailles de schémas d'outils, tailles d'entrées de compétences et taille de l'invite système.
...
- « Wrappers » du fournisseur ou en-têtes cachés (non visibles, toujours comptabilisés).
...
Décomposition complète : [Invite système](/concepts/system-prompt).
...
`/context detail` décompose les plus grands schémas d'outils pour que vous puissiez voir ce qui domine.
...
- **Directives** : `/think`, `/verbose`, `/reasoning`, `/elevated`, `/model`, `/queue` sont supprimées avant que le modèle ne voie le message.
  - Les messages contenant uniquement des directives persistent les paramètres de session.
...
- **Historique normal** persiste dans la transcription de session jusqu'à ce qu'il soit compacté/élagué par politique.
