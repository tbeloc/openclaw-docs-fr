...
- **Block streaming (canaux) :** émet les **blocs** complétés au fur et à mesure que l'assistant écrit. Ce sont des messages de canal normaux (pas des deltas de tokens).
- **Preview streaming (Telegram/Discord/Slack) :** met à jour un **message d'aperçu** temporaire pendant la génération.

Il n'existe **pas de vrai streaming de delta de tokens** vers les messages de canal aujourd'hui. Le preview streaming est basé sur les messages (envoi + éditions/ajouts).

## Block streaming (messages de canal)

Le block streaming envoie la sortie de l'assistant en chunks grossiers au fur et à mesure qu'ils deviennent disponibles.
...
- `channel send` : messages sortants réels (block replies).
...
- `text_end` : stream les blocs dès que le chunker les émet ; flush à chaque `text_end`.
- `message_end` : attendre que le message de l'assistant se termine, puis flush la sortie en buffer.

`message_end` utilise toujours le chunker si le texte en buffer dépasse `maxChars`, il peut donc émettre plusieurs chunks à la fin.

## Algorithme de chunking (limites basse/haute)

Le block chunking est implémenté par `EmbeddedBlockChunker` :

- **Limite basse :** ne pas émettre tant que buffer < `minChars` (sauf si forcé).
- **Limite haute :** préférer les divisions avant `maxChars` ; si forcé, diviser à `maxChars`.
- **Préférence de rupture :** `paragraph` → `newline` → `sentence` → `whitespace` → hard break.
- **Code fences :** ne jamais diviser à l'intérieur des fences ; quand forcé à `maxChars`, fermer + rouvrir la fence pour garder le Markdown valide.
...
- Le coalescing attend les **lacunes d'inactivité** (`idleMs`) avant de flush.
...
- Le joiner est dérivé de `blockStreamingChunk.breakPreference`
  (`paragraph` → `\n\n`, `newline` → `\n`, `sentence` → espace).
- Les surcharges de canal sont disponibles via `*.blockStreamingCoalesce` (y compris les configs par compte).
- Le `minChars` de coalesce par défaut est augmenté à 1500 pour Signal/Slack/Discord sauf surcharge.

## Rythme naturel entre les blocs

Quand le block streaming est activé, vous pouvez ajouter une **pause aléatoire** entre
les block replies (après le premier bloc). Cela rend les réponses multi-bulles
plus naturelles.
...
- S'applique uniquement aux **block replies**, pas aux réponses finales ou résumés d'outils.

## "Stream chunks ou tout"

Cela correspond à :

- **Stream chunks :** `blockStreamingDefault: "on"` + `blockStreamingBreak: "text_end"` (émettre au fur et à mesure). Les canaux non-Telegram ont aussi besoin de `*.blockStreaming: true`.
- **Stream tout à la fin :** `blockStreamingBreak: "message_end"` (flush une fois, possiblement plusieurs chunks si très long).
- **Pas de block streaming :** `blockStreamingDefault: "off"` (seulement réponse finale).
...
(`channels.<channel>.streaming`) sans block replies.

Rappel de localisation config : les défauts `blockStreaming*` se trouvent sous
...
- `block` : aperçu mis à jour en étapes chunked/ajoutées.
...
- Utilise `sendMessage` + `editMessageText` pour les mises à jour d'aperçu dans les DMs et groupes/topics.
...
- Utilise send + edit pour les messages d'aperçu.
...
- `block` utilise des aperçus de brouillon de style append.
