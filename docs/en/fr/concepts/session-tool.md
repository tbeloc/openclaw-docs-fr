...
- Lorsqu'on lui donne un `sessionId`, OpenClaw le résout à la clé de session correspondante (les ids manquants génèrent une erreur).
...
- Après la fin de l'exécution principale, OpenClaw exécute une **boucle de réponse** :
  ...
  - Répondre exactement `REPLY_SKIP` pour arrêter le ping-pong.
...
- Une fois la boucle terminée, OpenClaw exécute l'**étape d'annonce agent-à-agent** (agent cible uniquement) :
  - Répondre exactement `ANNOUNCE_SKIP` pour rester silencieux.
...
- Les sous-agents ne sont pas autorisés à appeler `sessions_spawn` (pas de génération de sous-agent → sous-agent).
...
- Après la fin, OpenClaw exécute une **étape d'annonce de sous-agent** et publie le résultat au canal de chat du demandeur.
  - Si la réponse finale de l'assistant est vide, le dernier `toolResult` de l'historique du sous-agent est inclus comme `Result`.
- Répondre exactement `ANNOUNCE_SKIP` pendant l'étape d'annonce pour rester silencieux.
