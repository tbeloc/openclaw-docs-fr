...
Recommandé : utilisez `openclaw setup` pour créer `~/.openclaw/openclaw.json` s'il manque et initialiser les fichiers d'espace de travail.

Disposition complète de l'espace de travail + guide de sauvegarde : [Agent workspace](/fr/concepts/agent-workspace)

Si `agents.defaults.sandbox` est activé, les sessions non-principales peuvent remplacer cela par des espaces de travail par session sous `agents.defaults.sandbox.workspaceRoot` (voir [Gateway configuration](/fr/gateway/configuration)).
...
`BOOTSTRAP.md` n'est créé que pour un **nouvel espace de travail** (aucun autre fichier d'amorçage présent). Si vous le supprimez après avoir complété le rituel, il ne devrait pas être recréé lors des redémarrages ultérieurs.
...
Les compétences peuvent être contrôlées par la configuration/env (voir `skills` dans [Gateway configuration](/fr/gateway/configuration)).
