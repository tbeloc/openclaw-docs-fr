```markdown
---
summary: "Aperçu des fournisseurs de modèles avec exemples de configs + flux CLI"
read_when:
...
- Les modèles à clé API directe supportent le commutateur `/fast` partagé et `params.fastMode` ; OpenClaw mappe cela à Anthropic `service_tier` (`auto` vs `standard_only`)
...
- Gemini CLI OAuth est livré en tant que plugin fourni (`google-gemini-cli-auth`, désactivé par défaut).
  ...
  - Note : vous ne collez **pas** un identifiant client ou un secret dans `openclaw.json`. Le flux de connexion CLI stocke les jetons dans les profils d'authentification sur l'hôte de passerelle.
...
Consultez [/providers/qwen](/providers/qwen) pour les détails de configuration et les notes.
...
Consultez [/providers/minimax](/providers/minimax) pour les détails de configuration, les options de modèles et les extraits de configuration.
...
`openclaw onboard` et au sélecteur de modèles. Consultez [/providers/ollama](/providers/ollama)
...
Consultez [/providers/vllm](/providers/vllm) pour plus de détails.
...
Consultez [/providers/sglang](/providers/sglang) pour plus de détails.
