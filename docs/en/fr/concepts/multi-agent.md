...
**Note sur l'espace de travail :** l'espace de travail de chaque agent est le **répertoire de travail par défaut**, pas un
bac à sable strict. Les chemins relatifs se résolvent à l'intérieur de l'espace de travail, mais les chemins absolus peuvent
...
Vous pouvez acheminer **différents DM WhatsApp** vers différents agents tout en restant sur **un compte WhatsApp**. Faites correspondre l'expéditeur E.164 (comme `+15551234567`) avec `peer.kind: "direct"`. Les réponses proviennent toujours du même numéro WhatsApp (pas d'identité d'expéditeur par agent).
...
- `binding` : achemine les messages entrants vers un `agentId` par `(channel, accountId, peer)` et optionnellement les IDs de guild/team.
...
Chaque compte de bot Discord correspond à un `accountId` unique. Liez chaque compte à un agent et maintenez des listes d'autorisation par bot.
...
- Les listes d'autorisation/refus d'outils sont des **outils**, pas des compétences. Si une compétence doit exécuter un binaire, assurez-vous que `exec` est autorisé et que le binaire existe dans le sandbox.
...
À partir de v2026.1.6, chaque agent peut avoir ses propres restrictions de sandbox et d'outils :
    ...
    list: [
      ...
      },
      {
        ...
        sandbox: {
          ...
          docker: {
            // Configuration optionnelle unique après la création du conteneur
            setupCommand: "apt-get update && apt-get install -y git curl",
          },
        },
        tools: {
          allow: ["read"],                    // Outil de lecture uniquement
...
- **Contrôle des ressources** : Sandbox des agents spécifiques tout en gardant d'autres sur l'hôte
...
Pour le ciblage de groupe, utilisez `agents.list[].groupChat.mentionPatterns` afin que les @mentions se mappent correctement à l'agent prévu.
