...
Les fournisseurs OAuth émettent couramment un **nouveau jeton d'actualisation** lors des flux de connexion/actualisation. Certains fournisseurs (ou clients OAuth) peuvent invalider les jetons d'actualisation plus anciens quand un nouveau est émis pour le même utilisateur/application.
...
- Fichier de compatibilité hérité : `~/.openclaw/agents/<agentId>/agent/auth.json`
  (les entrées statiques `api_key` sont supprimées quand elles sont détectées)
...
Tous les éléments ci-dessus respectent également `$OPENCLAW_STATE_DIR` (remplacement du répertoire d'état). Référence complète : [/gateway/configuration](/gateway/configuration#auth-storage-oauth--api-keys)

Pour les références de secrets statiques et le comportement d'activation de snapshot d'exécution, voir [Gestion des secrets](/gateway/secrets).

## Jeton de configuration Anthropic (authentification par abonnement)

<Warning>
Le support du jeton de configuration Anthropic est une compatibilité technique, pas une garantie de politique.
...
3. stockez comme profil d'authentification par jeton (pas d'actualisation)
...
4. si le rappel ne peut pas se lier (ou vous êtes à distance/sans interface), collez l'URL de redirection/code
...
- s'il a expiré → actualisez (sous un verrou de fichier) et remplacez les identifiants stockés
...
- [/concepts/model-failover](/concepts/model-failover) (règles de rotation + refroidissement)
