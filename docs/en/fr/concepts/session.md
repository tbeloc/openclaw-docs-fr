...
- Vous pouvez vérifier vos paramètres DM avec `openclaw security audit` (voir [security](/fr/cli/security)).
...
### Avertissement de performance pour les grands magasins
...
- Envoyez `/status` comme message autonome dans le chat pour voir si l'agent est accessible, combien du contexte de session est utilisé, les bascules de réflexion/rapide/verbeux actuelles, et quand vos identifiants WhatsApp Web ont été actualisés pour la dernière fois (aide à détecter les besoins de reliaison).
...
- Envoyez `/compact` (instructions optionnelles) comme message autonome pour résumer le contexte plus ancien et libérer de l'espace de fenêtre. Voir [/concepts/compaction](/fr/concepts/compaction).
