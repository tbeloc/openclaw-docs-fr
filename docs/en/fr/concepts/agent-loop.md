...
- **`agent:bootstrap`** : s'exécute lors de la construction des fichiers de bootstrap avant que le prompt système ne soit finalisé.
...
Voir [Plugins](/fr/tools/plugin#plugin-hooks) pour l'API de hook et les détails d'enregistrement.
...
- Voir [Streaming](/fr/concepts/streaming) pour le comportement de chunking et de réponse de bloc.
...
- Si aucune charge utile renderable ne reste et qu'un outil a erreur, une réponse d'erreur d'outil de secours est émise
