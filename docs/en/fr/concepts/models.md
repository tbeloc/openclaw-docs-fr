...
les délais d'attente et la façon dont cela interagit avec les fallbacks.
...
Si vous ne voulez pas modifier manuellement la configuration, exécutez l'assistant d'intégration :
...
Affiche le modèle principal résolu, les fallbacks, le modèle d'image et un aperçu d'authentification
...
- Nécessite une clé API OpenRouter à partir des profils d'authentification ou `OPENROUTER_API_KEY` (voir [/environment](/fr/help/environment))
...
- Les valeurs `apiKey` du fournisseur gérées par SecretRef sont actualisées à partir des marqueurs de source (`ENV_VAR_NAME` pour les références env, `secretref-managed` pour les références fichier/exec) au lieu de persister les secrets résolus.
- Les valeurs d'en-tête du fournisseur gérées par SecretRef sont actualisées à partir des marqueurs de source (`secretref-env:ENV_VAR_NAME` pour les références env, `secretref-managed` pour les références fichier/exec).
- `apiKey`/`baseUrl` vides ou manquants de l'agent reviennent à la configuration `models.providers`.
