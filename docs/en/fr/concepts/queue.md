...
Nous sérialisons les exécutions de réponse automatique entrantes (tous les canaux) via une minuscule file d'attente en mémoire pour éviter que plusieurs exécutions d'agent ne se heurtent, tout en permettant un parallélisme sûr entre les sessions.
...
- Une file d'attente FIFO consciente des voies draine chaque voie avec un plafond de concurrence configurable (1 par défaut pour les voies non configurées ; main par défaut à 4, subagent à 8).
...
- `steer`: injecter immédiatement dans l'exécution actuelle (annule les appels d'outils en attente après la prochaine limite d'outil). Si non diffusé, revient à followup.
- `followup`: mettre en file d'attente pour le prochain tour d'agent après la fin de l'exécution actuelle.
- `collect`: fusionner tous les messages en file d'attente en un **seul** tour de suivi (par défaut). Si les messages ciblent différents canaux/threads, ils se drainent individuellement pour préserver le routage.
...
Valeurs par défaut (lorsque non définies dans la configuration) :
...
- `debounceMs`: attendre le silence avant de commencer un tour de suivi (prévient "continue, continue").
...
- Si les commandes semblent bloquées, activez les journaux détaillés et recherchez les lignes "queued for …ms" pour confirmer que la file d'attente se draine.
