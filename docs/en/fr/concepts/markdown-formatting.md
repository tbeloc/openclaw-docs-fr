...
- **Adaptation au canal :** mapper la même IR à Slack mrkdwn, Telegram HTML et Signal style ranges sans réanalyser le Markdown.
...
Si vous avez besoin de plus d'informations sur le comportement du chunking entre les canaux, consultez [Streaming + chunking](/fr/concepts/streaming).
...
1. **Analyser une fois :** utilisez l'assistant partagé `markdownToIR(...)` avec les options appropriées au canal (autolink, style de titre, préfixe de blockquote).
...
- Préservez les nouvelles lignes de fin pour les blocs de code clôturés afin que les marqueurs de fermeture se retrouvent sur leur propre ligne.
