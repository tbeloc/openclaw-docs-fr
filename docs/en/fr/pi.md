...
- Personnalisation de l'invite système par canal/contexte
...
| `pi-tui`          | Composants d'interface utilisateur terminal (utilisés dans le mode TUI local d'OpenClaw)                                             |
...
│   ├── sandbox-info.ts            # Informations sandbox pour l'invite système
...
│   ├── system-prompt.ts           # Générateur d'invite système
...
├── model-selection.ts             # Résolution de modèle par défaut
...
├── system-prompt-params.ts        # Résolution des paramètres d'invite système
├── system-prompt-report.ts        # Génération de rapport de débogage
├── tool-summaries.ts              # Résumés de description d'outil
├── tool-policy.ts                 # Résolution de politique d'outil
├── transcript-policy.ts           # Politique de validation de transcription
├── skills.ts                      # Construction d'instantané/invite de compétence
...
6. **Normalisation du schéma** : Schémas nettoyés pour les particularités Gemini/OpenAI
...
## Construction de la requête système

La requête système est construite dans `buildAgentSystemPrompt()` (`system-prompt.ts`). Elle assemble une requête complète avec des sections incluant Outils, Style d'appel d'outils, Garde-fous de sécurité, Référence CLI OpenClaw, Compétences, Documentation, Espace de travail, Sandbox, Messagerie, Balises de réponse, Voix, Réponses silencieuses, Pulsations cardiaques, Métadonnées d'exécution, ainsi que Mémoire et Réactions lorsqu'ils sont activés, et contenu de fichier de contexte optionnel et requête système supplémentaire. Les sections sont réduites pour le mode requête minimale utilisé par les sous-agents.

La requête est appliquée après la création de la session via `applySystemPromptOverrideToSession()`:
...
`limitHistoryTurns()` réduit l'historique de conversation en fonction du type de canal (MP vs groupe).

### Compaction

La compaction automatique se déclenche lors du débordement de contexte. `compactEmbeddedPiSessionDirect()` gère la compaction manuelle:
...
`src/agents/pi-extensions/compaction-safeguard.ts` ajoute des garde-fous à la compaction, incluant un budget de jetons adaptatif plus des résumés d'échec d'outils et d'opérations de fichiers:
...
`src/agents/pi-extensions/context-pruning.ts` implémente l'élagage de contexte basé sur le TTL du cache:
...
### Fallback du niveau de réflexion

Si un niveau de réflexion n'est pas supporté, il bascule:
...
| Requête système | AGENTS.md + prompts     | Dynamique par canal/contexte                                                                   |
...
| Gestion d'événements | Rendu TUI           | Basée sur les rappels (onBlockReply, etc.)                                                     |
...
4. **Complexité du gestionnaire de streaming**: `subscribeEmbeddedPiSession` s'est agrandie
5. **Particularités du fournisseur**: De nombreux chemins de code spécifiques au fournisseur que pi pourrait potentiellement gérer
...
Pour les commandes d'exécution actuelles, voir [Flux de travail de développement Pi](/fr/pi-dev).
