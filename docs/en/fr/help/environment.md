...
## Ordre de précédence (le plus élevé → le plus bas)
...
- `OPENCLAW_SHELL=acp` : défini pour les générations de processus du backend d'exécution ACP (par exemple `acpx`).
...
OpenClaw supporte deux modèles pilotés par l'environnement :
...
Les deux se résolvent à partir de l'environnement du processus au moment de l'activation. Les détails de SecretRef sont documentés dans [Gestion des secrets](/fr/gateway/secrets).
...
| Variable             | Objectif                                                                                                                                                                                      |
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OPENCLAW_LOG_LEVEL` | Remplacer le niveau de journalisation pour le fichier et la console (par exemple `debug`, `trace`). Prend la précédence sur `logging.level` et `logging.consoleLevel` dans la configuration. Les valeurs invalides sont ignorées avec un avertissement. |

### `OPENCLAW_HOME`

Lorsqu'il est défini, `OPENCLAW_HOME` remplace le répertoire personnel du système (`$HOME` / `os.homedir()`) pour toute la résolution de chemin interne. Cela permet une isolation complète du système de fichiers pour les comptes de service sans interface.

**Ordre de précédence :** `OPENCLAW_HOME` > `$HOME` > `USERPROFILE` > `os.homedir()`
