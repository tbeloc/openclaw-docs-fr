...
- Détails complets de configuration Android : [Android App](/platforms/android)
...
- Pourquoi cela existe :
  ...
  - Contient de petites régressions isolées (exemple : replay de réponses OpenAI/Codex + flux d'appel d'outils)
...
Incluez au moins un modèle capable d'images dans `OPENCLAW_LIVE_GATEWAY_MODELS` (variantes Claude/Gemini/OpenAI avec vision, etc.) pour exercer la sonde d'image.
...
## Vérification de santé des docs

Exécutez les vérifications de docs après les modifications de doc : `pnpm docs:list`.
...
- Garde-fou de traversée SecretRef :
  - `src/secrets/exec-secret-ref-id-parity.test.ts` dérive une cible échantillonnée par classe SecretRef à partir des métadonnées du registre (`listSecretTargetRegistryEntries()`), puis affirme que les identifiants exec de segment de traversée sont rejetés.
