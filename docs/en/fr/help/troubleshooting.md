...
## L'installation du plugin échoue avec les extensions openclaw manquantes
...
flowchart TD
  ...
  B --> H[Le nœud est appairé mais l'exécution de l'écran de canevas de caméra de l'outil échoue]
...
<AccordionGroup>
  <Accordion title="Pas de réponses">
    ...
    - `drop guild message (mention required` → la mention gating a bloqué le message dans Discord.
  ...
  <Accordion title="Le canal se connecte mais les messages ne circulent pas">
    ...
    - Les vérifications d'appairage/liste blanche réussissent.
    - Les mentions sont détectées où nécessaire.

    Signatures de journal courantes :

    - `mention required` → la mention de groupe gating a bloqué le traitement.
  ...
  <Accordion title="Le nœud est appairé mais l'exécution de l'écran de canevas de caméra de l'outil échoue">
    ```bash
    openclaw status
    openclaw gateway status
    ...
    - `SYSTEM_RUN_DENIED: allowlist miss` → commande pas sur la liste blanche d'exécution.
