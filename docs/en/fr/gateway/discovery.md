...
1. **Contrôle à distance de l'opérateur** : l'application de la barre de menu macOS contrôlant une passerelle s'exécutant ailleurs.
...
- **Pont TCP hérité (déprécié/supprimé)** : transport de nœud plus ancien (voir [Protocole Bridge](/fr/gateway/bridge-protocol)) ; n'est plus annoncé pour la découverte.
...
Quand il n'y a pas de route directe (ou que direct est désactivé), les clients peuvent toujours se connecter via SSH en transférant le port de passerelle de loopback.
...
- Les demandes d'appairage sont créées/approuvées/rejetées dans la passerelle (voir [Appairage de passerelle](/fr/gateway/pairing)).
- La passerelle applique :
  - l'authentification (jeton / paire de clés)
  - les portées/ACL (la passerelle n'est pas un proxy brut pour chaque méthode)
