...
Pour un aperçu destiné aux utilisateurs (CLI + Control UI + config), voir [/logging](/logging).
...
- Pour capturer les détails réservés au mode verbose dans les journaux de fichiers, définissez `logging.level` sur `debug` ou
  `trace`.
...
Les résumés d'outils détaillés (par ex. `🛠️ Exec: ...`) peuvent masquer les jetons sensibles avant qu'ils ne frappent le
flux de console. Ceci est **réservé aux outils** et ne modifie pas les journaux de fichiers.
