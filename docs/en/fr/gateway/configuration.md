...
<AccordionGroup>
  ...
  <Accordion title="Choisir et configurer les modèles">
    ...
    - `agents.defaults.imageMaxDimensionPx` contrôle la réduction d'échelle des images de transcription/outil (par défaut `1200`) ; les valeurs plus basses réduisent généralement l'utilisation des jetons de vision lors des exécutions avec beaucoup de captures d'écran.
  ...
  <Accordion title="Configurer les tâches cron">
    ...
    - `sessionRetention` : élaguer les sessions d'exécution isolées complétées de `sessions.json` (par défaut `24h` ; définissez `false` pour désactiver).
    - `runLog` : élaguer `cron/runs/<jobId>.jsonl` par taille et lignes conservées.
