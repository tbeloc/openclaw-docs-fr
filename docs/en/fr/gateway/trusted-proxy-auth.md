...
3. OpenClaw vérifie que la demande provient d'une **adresse IP de proxy de confiance** (configurée dans `gateway.trustedProxies`)
...
- L'appairage n'est plus la porte principale d'accès à l'interface de contrôle dans ce mode.
- Votre politique d'authentification du proxy inverse et `allowUsers` deviennent le contrôle d'accès effectif.
- Gardez l'entrée de la passerelle verrouillée aux adresses IP du proxy de confiance uniquement (`gateway.trustedProxies` + pare-feu).
...
```json5
{
  gateway: {
    ...
    // CRITIQUE : Ajoutez uniquement les adresses IP de votre proxy ici
...
| ------------------------------------------- | -------- | --------------------------------------------------------------------------- |
| `gateway.trustedProxies`                    | Oui      | Tableau des adresses IP du proxy à faire confiance. Les demandes d'autres adresses IP sont rejetées. |
...
- Augmentez à des valeurs longue durée (par exemple `max-age=31536000`) seulement après une grande confiance.
- Ajoutez `includeSubDomains` seulement si chaque sous-domaine est prêt pour HTTPS.
- Utilisez preload seulement si vous répondez intentionnellement aux exigences de preload pour votre ensemble de domaines.
...
- [ ] **trustedProxies est minimal** : Seulement vos adresses IP de proxy réelles, pas des sous-réseaux entiers
- [ ] **Le proxy supprime les en-têtes** : Votre proxy réécrit (ne pas ajoute) les en-têtes `x-forwarded-*` des clients
...
- [Security](/gateway/security) — guide de sécurité complet
...
- [Tailscale](/gateway/tailscale) — alternative plus simple pour l'accès tailnet uniquement
