...
OpenClaw peut auto-configurer Tailscale **Serve** (tailnet) ou **Funnel** (public) pour le
...
- `tailscale.mode: "funnel"` refuse de démarrer à moins que le mode auth soit `password` pour éviter une exposition publique.
...
- Serve/Funnel exposent uniquement l'**interface Control UI + WS**. Les nœuds se connectent via
  le même point de terminaison Gateway WS, donc Serve peut fonctionner pour l'accès aux nœuds.
