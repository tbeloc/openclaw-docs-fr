...
- Les slugs de guilde sont en minuscules avec des espaces remplacés par `-` ; les clés de canal utilisent le nom slugifié (pas de `#`). Préférez les ID de guilde.
...
- `params` fusionner la précédence (config) : `agents.defaults.models["provider/model"].params` est la base, puis `agents.list[].params` (correspondant à l'id de l'agent) remplace par clé.
...
- `maxConcurrent` : nombre maximum d'exécutions d'agents parallèles entre les sessions (chaque session toujours sérialisée). Valeur par défaut : 1.
...
- `suppressToolErrorWarnings` : lorsque true, supprime les charges utiles d'avertissement d'erreur d'outil lors des exécutions de battement cardiaque.
...
- `lightContext` : lorsque true, les exécutions de battement cardiaque utilisent un contexte d'amorçage léger et conservent uniquement `HEARTBEAT.md` des fichiers d'amorçage de l'espace de travail.
- `isolatedSession` : lorsque true, chaque battement cardiaque s'exécute dans une session nouvelle sans historique de conversation antérieur. Même modèle d'isolation que cron `sessionTarget: "isolated"`. Réduit le coût des jetons par battement cardiaque d'environ 100K à 2-5K jetons.
- Par agent : définissez `agents.list[].heartbeat`. Lorsqu'un agent définit `heartbeat`, **seuls ces agents** exécutent les battements cardiaques.
...
- `model` : la forme chaîne remplace uniquement `primary` ; la forme objet `{ primary, fallbacks }` remplace les deux (`[]` désactive les secours globaux). Les travaux cron qui remplacent uniquement `primary` héritent toujours des secours par défaut sauf si vous définissez `fallbacks: []`.
...
Les configurations locales par défaut des nouveaux paramètres locaux à `tools.profile: "coding"` lorsqu'ils ne sont pas définis (les profils explicites existants sont conservés).
...
- `gateway.auth.rateLimit` : limiteur optionnel d'authentification échouée. S'applique par IP client et par portée d'authentification (secret partagé et jeton d'appareil sont suivis indépendamment). Les tentatives bloquées retournent `429` + `Retry-After`.
  - `gateway.auth.rateLimit.exemptLoopback` est par défaut à `true` ; définissez à `false` quand vous voulez intentionnellement que le trafic localhost soit aussi limité en débit (pour les configurations de test ou les déploiements de proxy stricts).
...
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1` : remplacement de secours côté client qui permet le texte brut `ws://` aux IPs de réseau privé de confiance ; le défaut reste loopback uniquement pour le texte brut.
- `gateway.remote.token` / `.password` sont des champs de credentials de client distant. Ils ne configurent pas l'authentification de passerelle par eux-mêmes.
...
- `gateway.push.apns.relay.timeoutMs` : délai d'expiration d'envoi passerelle-vers-relais en millisecondes. Par défaut à `10000`.
...
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true` : trappe d'échappement de développement uniquement pour les URLs de relais HTTP loopback. Les URLs de relais de production doivent rester sur HTTPS.
...
- Matrice canonique : [Surface de credential SecretRef](/reference/secretref-credential-surface)
...
- `cli.banner.taglineMode` contrôle le style de la ligne d'accroche de la bannière :
  ...
  - `"off"` : pas de texte de ligne d'accroche (titre/version de la bannière toujours affiché).
