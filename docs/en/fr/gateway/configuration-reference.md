...
Chaque champ disponible dans `~/.openclaw/openclaw.json`. Pour un aperçu orienté tâches, voir [Configuration](/fr/gateway/configuration).
...
- Politique de nouvelle tentative : voir [Retry policy](/fr/concepts/retry).
...
- OpenClaw tente en outre la récupération de réception vocale en quittant/rejoignant une session vocale après des défaillances de déchiffrement répétées.
- `channels.discord.streaming` est la clé du mode de flux canonique. Les valeurs `streamMode` héritées et booléennes `streaming` sont auto-migrées.
...
- `channels.slack.streaming` est la clé du mode de flux canonique. Les valeurs `streamMode` héritées et booléennes `streaming` sont auto-migrées.
...
Taille maximale en pixels pour le côté le plus long d'une image dans les blocs d'image de transcription/outil avant les appels au fournisseur.
...
- Les backends CLI sont d'abord texte ; les outils sont toujours désactivés.
...
- `isolatedSession` : lorsque true, chaque battement de cœur s'exécute dans une session fraîche sans historique de conversation antérieur. Même modèle d'isolation que cron `sessionTarget: "isolated"`. Réduit le coût des jetons par battement de cœur d'environ 100K à 2-5K jetons.
...
- `mode` : `default` ou `safeguard` (résumé par chunks pour les longs historiques). Voir [Compaction](/fr/concepts/compaction).
- `identifierPolicy` : `strict` (par défaut), `off` ou `custom`. `strict` ajoute des conseils de rétention d'identifiants opaques intégrés lors de la résumé de compaction.
- `identifierInstructions` : texte optionnel de préservation d'identifiants personnalisés utilisé lorsque `identifierPolicy=custom`.
...
- `model` : remplacement optionnel `provider/model-id` pour la résumé de compaction uniquement. Utilisez ceci lorsque la session principale doit conserver un modèle mais les résumés de compaction doivent s'exécuter sur un autre ; lorsque non défini, la compaction utilise le modèle principal de la session.
...
Élague les **anciens résultats d'outils** du contexte en mémoire avant d'envoyer au LLM. Ne modifie **pas** l'historique de session sur disque.
...
Voir [Streaming](/fr/concepts/streaming) pour les détails du comportement + chunking.
...
- `sandbox.browser.binds` monte des répertoires d'hôte supplémentaires dans le conteneur du navigateur sandboxé uniquement. Lorsqu'il est défini (y compris `[]`), il remplace `docker.binds` pour le conteneur du navigateur.
...
- `params` : paramètres de flux par agent fusionnés sur l'entrée de modèle sélectionnée dans `agents.defaults.models`. Utilisez ceci pour les remplacements spécifiques à l'agent comme `cacheRetention`, `temperature` ou `maxTokens` sans dupliquer le catalogue de modèles entier.
...
- `models.providers.*.models.*.compat.supportsDeveloperRole` : indice de compatibilité optionnel. Pour `api: "openai-completions"` avec un `baseUrl` non vide et non natif (hôte pas `api.openai.com`), OpenClaw force ceci à `false` au runtime. Un `baseUrl` vide/omis conserve le comportement OpenAI par défaut.
...
Voir [Local Models](/fr/gateway/local-models). TL;DR : exécutez MiniMax M2.5 via l'API LM Studio Responses sur du matériel sérieux ; conservez les modèles hébergés fusionnés pour la secours.
...
- `plugins.entries.<id>.env` : carte de variable env au niveau du plugin.
- `plugins.entries.<id>.hooks.allowPromptInjection` : lorsque `false`, le noyau bloque `before_prompt_build` et ignore les champs mutants de prompt de `before_agent_start` hérité, tout en préservant `modelOverride` et `providerOverride` hérités.
...
- Définissez `ssrfPolicy.dangerouslyAllowPrivateNetwork: false` pour une navigation navigateur stricte en public uniquement.
...
- `relayBindHost` change l'adresse d'écoute du relais d'extension Chrome. Laissez non défini pour un accès loopback uniquement ; définissez une adresse de liaison explicite non-loopback comme `0.0.0.0` uniquement quand le relais doit traverser une limite d'espace de noms (par exemple WSL2) et le réseau hôte est déjà de confiance.
...
- `seamColor` : couleur d'accent pour la chrome UI de l'application native (teinte de bulle Talk Mode, etc.).
- `assistant` : remplacement de l'identité UI. Revient à l'identité de l'agent actif.
...
- `gateway.auth.mode: "trusted-proxy"` : déléguer l'authentification à un proxy inverse conscient de l'identité et faire confiance aux en-têtes d'identité de `gateway.trustedProxies` (voir [Authentification par proxy de confiance](/fr/gateway/trusted-proxy-auth)).
...
- `controlUi.allowedOrigins` : liste d'autorisation explicite d'origine navigateur pour les connexions WebSocket de la passerelle. Requis quand des clients navigateur sont attendus d'origines non-loopback.
...
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1` : remplacement de secours côté client qui permet le `ws://` en texte clair vers les IPs de réseau privé de confiance ; le défaut reste loopback uniquement pour le texte clair.
...
- Les enregistrements soutenus par relais sont délégués à une identité de passerelle spécifique. L'app iOS appairée récupère `gateway.identity.get`, inclut cette identité dans l'enregistrement du relais, et transfère une subvention d'envoi à portée d'enregistrement à la passerelle. Une autre passerelle ne peut pas réutiliser cet enregistrement stocké.
...
- `allowRealIpFallback` : quand `true`, la passerelle accepte `X-Real-IP` si `X-Forwarded-For` est manquant. Par défaut `false` pour un comportement de fermeture en cas d'échec.
...
- En-tête de durcissement de réponse optionnel :
  - `gateway.http.securityHeaders.strictTransportSecurity` (définissez uniquement pour les origines HTTPS que vous contrôlez ; voir [Authentification par proxy de confiance](/fr/gateway/trusted-proxy-auth#tls-termination-and-hsts))
...
- `allowedAgentIds` : restreint le routage explicite (`*` ou omis = autoriser tous, `[]` = refuser tous).
...
- Locale uniquement : conservez `gateway.bind: "loopback"` (par défaut).
...
- `shellEnv` : importe les clés attendues manquantes à partir de votre profil de shell de connexion.
...
- Matrice canonique : [Surface de credential SecretRef](/fr/reference/secretref-credential-surface)
- `secrets apply` cible les chemins de credential `openclaw.json` supportés.
...
- Les références de secret sont résolues au moment de l'activation dans un snapshot en mémoire, puis les chemins de requête lisent uniquement le snapshot.
...
- `cli.banner.taglineMode` contrôle le style de la ligne d'accroche de la bannière :
  - `"random"` (par défaut) : lignes d'accroche amusantes/saisonnières rotatives.
