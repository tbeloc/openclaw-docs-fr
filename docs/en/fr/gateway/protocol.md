...
OpenClaw. Tous les clients (CLI, interface web, application macOS, nœuds iOS/Android, nœuds headless) se connectent via WebSocket et déclarent leur **rôle** + **portée** au
moment du handshake.
...
- Après l'appairage, la Gateway émet un **token de périphérique** limité au rôle de connexion + portées. Il est retourné dans `hello-ok.auth.deviceToken` et doit être
...
- Comportement du client pour `AUTH_TOKEN_MISMATCH` :
  - Les clients de confiance peuvent tenter une tentative bornée avec un token par périphérique en cache.
  - Si cette tentative échoue, les clients doivent arrêter les boucles de reconnexion automatique et afficher les conseils d'action de l'opérateur.
...
| `device signature expired`  | `DEVICE_AUTH_SIGNATURE_EXPIRED`  | `device-signature-stale` | L'horodatage signé est en dehors du skew autorisé. |
