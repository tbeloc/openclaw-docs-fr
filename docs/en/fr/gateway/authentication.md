...
Consultez [/concepts/oauth](/fr/concepts/oauth) pour le flux OAuth complet et la disposition du stockage.
Pour l'authentification basée sur SecretRef (`env`/`file`/`exec` providers), consultez [Gestion des secrets](/fr/gateway/secrets).
Pour les règles de codes de raison/d'éligibilité des identifiants d'authentification utilisées par `models status --probe`, consultez [Sémantique des identifiants d'authentification](/fr/auth-credential-semantics).
...
Consultez [Aide](/fr/help) pour plus de détails sur l'héritage d'env (`env.shellEnv`,
...
Certains fournisseurs supportent la nouvelle tentative d'une requête avec des clés alternatives lorsqu'un appel API atteint une limite de débit du fournisseur.
