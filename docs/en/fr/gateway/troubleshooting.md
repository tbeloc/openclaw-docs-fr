...
| `AUTH_DEVICE_TOKEN_MISMATCH` | Le jeton d'appareil en cache est obsolète ou révoqué.             | Faites pivoter/réapprouvez le jeton d'appareil en utilisant [CLI des appareils](/fr/cli/devices), puis reconnectez-vous.                                                                                    |
...
## Si vous avez effectué une mise à jour et quelque chose s'est soudainement cassé

La plupart des dysfonctionnements après une mise à jour sont dus à une dérive de configuration ou à des valeurs par défaut plus strictes qui sont maintenant appliquées.

### 1) Le comportement de substitution d'authentification et d'URL a changé
