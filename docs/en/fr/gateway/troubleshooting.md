...
- `unauthorized` répété après cette nouvelle tentative → dérive du jeton partagé/jeton d'appareil ; actualisez la configuration du jeton et réapprobez/faites pivoter le jeton d'appareil si nécessaire.
...
| `AUTH_DEVICE_TOKEN_MISMATCH` | Le jeton d'appareil en cache par appareil est obsolète ou révoqué.             | Faites pivoter/réapprobez le jeton d'appareil en utilisant [CLI des appareils](/cli/devices), puis reconnectez-vous.                                                                                    |
...
- Historique d'exécution du travail (`ok`, `skipped`, `error`).
- Raisons d'omission du heartbeat (`quiet-hours`, `requests-in-flight`, `alerts-disabled`).
...
La plupart des pannes post-mise à niveau sont dues à une dérive de configuration ou à des valeurs par défaut plus strictes maintenant appliquées.
...
- Si `gateway.mode=remote`, les appels CLI peuvent cibler le distant tandis que votre service local va bien.
