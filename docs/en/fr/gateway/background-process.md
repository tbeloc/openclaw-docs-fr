...
- Lorsqu'elles sont mises en arrière-plan (explicitement ou par délai), l'outil retourne `status: "running"` + `sessionId` et une courte queue.
...
- `tools.exec.notifyOnExitEmptySuccess` (par défaut false) : lorsque true, met également en file d'attente les événements de fin pour les exécutions en arrière-plan réussies qui n'ont produit aucune sortie.
