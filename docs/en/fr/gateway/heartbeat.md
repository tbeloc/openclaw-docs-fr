...
> **Heartbeat vs Cron?** Voir [Cron vs Heartbeat](/fr/automation/cron-vs-heartbeat) pour des conseils sur quand utiliser chacun.
...
- `accountId`: ID de compte optionnel pour les canaux multi-comptes. Quand `target: "last"`, l'ID de compte s'applique au dernier canal résolu s'il supporte les comptes; sinon il est ignoré. Si l'ID de compte ne correspond pas à un compte configuré pour le canal résolu, la livraison est ignorée.
