```markdown
---
summary: "Runbook pour le service Gateway, cycle de vie et opérations"
read_when:
...
<CardGroup cols={2}>
  ...
  <Card title="Contrat du plan des secrets" icon="shield-check" href="/gateway/secrets-plan-contract">
    Règles exactes de cible/chemin `secrets apply` et comportement auth-profile en référence uniquement.
  </Card>
</CardGroup>
...
- Port multiplexé unique pour :
  ...
  - APIs HTTP (compatibles OpenAI, Responses, tools invoke)
...
| `restart`             | Redémarrer sur les changements nécessitant un rechargement |
| `hybrid` (défaut)     | Application à chaud si sûr, redémarrage si nécessaire |
...
Pour la persistance après la déconnexion, activez lingering :
...
Voir la documentation complète du protocole : [Gateway Protocol](/gateway/protocol).
...
| `unauthorized` during connect                                  | Authentification incompatible entre client et gateway |

Pour les échelles de diagnostic complètes, utilisez [Gateway Troubleshooting](/gateway/troubleshooting).

## Garanties de sécurité

- Les clients du protocole Gateway échouent rapidement lorsque Gateway est indisponible (pas de secours de canal direct implicite).
