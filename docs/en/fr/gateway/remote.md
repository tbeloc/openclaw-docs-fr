...
### 2) Le desktop domestique exécute la Gateway, le laptop est une télécommande distante

Le laptop **n'exécute pas** l'agent. Il se connecte à distance :

- Utilisez le mode **Remote over SSH** de l'app macOS (Paramètres → Général → « OpenClaw runs »).
...
L'app de barre de menu macOS peut piloter la même configuration de bout en bout (vérifications de statut distantes, WebChat et transfert de Voice Wake).
