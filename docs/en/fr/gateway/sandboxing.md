...
- Navigateur isolé optionnel (`agents.defaults.sandbox.browser`).
  ...
  - L'accès observateur noVNC est protégé par mot de passe par défaut ; OpenClaw émet une URL de jeton de courte durée qui sert une page d'amorçage locale et ouvre noVNC avec le mot de passe dans le fragment d'URL (pas les journaux de requête/en-tête).
...
- Tout outil explicitement autorisé à s'exécuter sur l'hôte (par exemple `tools.elevated`).
  ...
  - Si l'isolation est désactivée, `tools.elevated` ne change pas l'exécution (déjà sur l'hôte). Voir [Mode Élevé](/fr/tools/elevated).
...
- `"off"` : pas d'isolation.
- `"non-main"` : isoler uniquement les sessions **non-main** (par défaut si vous voulez des chats normaux sur l'hôte).
- `"all"` : chaque session s'exécute dans un bac à sable.
  ...
  Les sessions de groupe/canal utilisent leurs propres clés, elles sont donc comptées comme non-main et seront isolées.
...
- Voir [Isolation vs Politique d'Outil vs Élevé](/fr/gateway/sandbox-vs-tool-policy-vs-elevated) pour savoir comment les liaisons interagissent avec la politique d'outil et l'exécution élevée.
...
`exec`, utilisez la politique d'outil deny (voir [Isolation vs Politique d'Outil vs Élevé](/fr/gateway/sandbox-vs-tool-policy-vs-elevated)).
...
- Voir [Isolation vs Politique d'Outil vs Élevé](/fr/gateway/sandbox-vs-tool-policy-vs-elevated) pour le modèle mental "pourquoi c'est bloqué ?".
...
Voir [Isolation Multi-Agent & Outils](/fr/tools/multi-agent-sandbox-tools) pour la précédence.
...
- [Configuration de l'Isolation](/fr/gateway/configuration#agentsdefaults-sandbox)
- [Isolation Multi-Agent & Outils](/fr/tools/multi-agent-sandbox-tools)
