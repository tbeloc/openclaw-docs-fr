---
summary: "Backends CLI : secours texte uniquement via les CLI d'IA locaux"
read_when:
...
OpenClaw peut exécuter des **CLI d'IA locaux** comme **secours texte uniquement** quand les fournisseurs d'API sont indisponibles,
...
- **Les sessions sont supportées** (donc les tours suivants restent cohérents).
...
3. **Exécute le CLI** avec un identifiant de session (s'il est supporté) pour que l'historique reste cohérent.
...
- Si le CLI supporte les sessions, définissez `sessionArg` (par ex. `--session-id`) ou
...
chemins de fichiers à l'invite (injection de chemin), ce qui est suffisant pour les CLI qui chargent automatiquement
...
- **Les sessions Codex CLI** reprennent via sortie texte (pas JSONL), ce qui est moins
  structuré que l'exécution `--json` initiale. Les sessions OpenClaw fonctionnent toujours
  normalement.

## Dépannage

- **CLI non trouvé** : définissez `command` sur un chemin complet.
