...
- Chemin de réutilisation de démarrage rapide : lorsque `gateway.auth.token` est déjà une SecretRef, l'intégration la résout avant l'amorçage de la sonde/tableau de bord (pour les références `env`, `file` et `exec`) en utilisant la même porte de défaillance rapide.
...
- [Contrat du plan Secrets Apply](/fr/gateway/secrets-plan-contract)
...
- Détails du contrat du plan : [Contrat du plan Secrets Apply](/fr/gateway/secrets-plan-contract)
- Surface d'identifiants : [Surface d'identifiants SecretRef](/fr/reference/secretref-credential-surface)
- Configuration d'authentification : [Authentification](/fr/gateway/authentication)
- Posture de sécurité : [Sécurité](/fr/gateway/security)
- Précédence des variables d'environnement : [Variables d'environnement](/fr/help/environment)
