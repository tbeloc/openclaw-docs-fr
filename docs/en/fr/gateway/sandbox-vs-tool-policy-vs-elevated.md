...
summary: "Pourquoi un outil est bloqué : runtime sandbox, politique d'autorisation/refus d'outil, et portes d'exécution élevées"
read_when: "Vous avez rencontré une 'sandbox jail' ou un refus d'outil/exécution élevé et vous voulez la clé de configuration exacte à modifier."
status: active
---
...
3. **Elevated** (`tools.elevated.*`, `agents.list[].tools.elevated.*`) est une **échappatoire d'exécution uniquement** pour s'exécuter sur l'hôte quand vous êtes en sandbox.
...
- les portes élevées et les chemins de clé de correction
