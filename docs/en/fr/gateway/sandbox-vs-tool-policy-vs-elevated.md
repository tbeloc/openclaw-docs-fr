...
read_when: "Vous avez rencontré une 'sandbox jail' ou un refus d'outil/elevated et vous voulez la clé de configuration exacte à modifier."
status: active
---
...
- l'autorisation/refus d'outil sandbox effectif (et s'il provient d'agent/global/default)
...
- `/exec` change uniquement les valeurs par défaut de session pour les expéditeurs autorisés ; il n'accorde pas l'accès aux outils.
...
## Elevated : "exécuter sur l'hôte" exec uniquement
...
- Si vous exécutez déjà directement, elevated est effectivement un no-op (toujours contrôlé).
- Elevated n'est **pas** limité à la compétence et ne contourne **pas** l'autorisation/refus d'outil.
- `/exec` est séparé de elevated. Il ajuste uniquement les valeurs par défaut exec par session pour les expéditeurs autorisés.
...
Voir [Elevated Mode](/tools/elevated).
