...
3. Configurez le **split DNS** de Tailscale pour que votre domaine choisi se résout via ce
...
- Ajoutez un split DNS pour que votre domaine de découverte utilise ce serveur de noms.
...
- `canvasPort=<port>` (uniquement quand l'hôte canvas est activé ; actuellement identique à `gatewayPort`)
...
- Les UIs doivent décoder pour l'affichage (iOS utilise `BonjourEscapes.decode`).
...
- Appairage de nœud + approbations : [Appairage Gateway](/fr/gateway/pairing)
