...
OpenClaw est à la fois un produit et une expérience : vous câblez le comportement des modèles de pointe dans les surfaces de messagerie réelles et les outils réels. **Il n'existe pas de configuration « parfaitement sécurisée »**. L'objectif est d'être délibéré sur :
...
OpenClaw suppose que la limite de l'hôte et de la configuration est de confiance :
...
- Recommandé par défaut : un utilisateur par machine/hôte (ou VPS), une passerelle pour cet utilisateur, et un ou plusieurs agents dans cette passerelle.
...
Dans une instance Gateway, l'accès de l'opérateur authentifié est un rôle du plan de contrôle de confiance, pas un rôle de locataire par utilisateur.
...
- Si vous avez besoin d'une isolation des utilisateurs adverses, exécutez des passerelles séparées par limite de confiance.
...
Si vous avez besoin d'une isolation des utilisateurs hostiles, divisez les limites de confiance par utilisateur/hôte OS et exécutez des passerelles séparées.
...
| ------------------------------------------- | ------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `gateway.auth` (authentification par jeton/mot de passe/appareil) | Authentifie les appelants aux API de passerelle | « Nécessite des signatures par message sur chaque trame pour être sécurisé »                   |
...
| `canvas.eval` / évaluation du navigateur    | Capacité d'opérateur intentionnelle si activée   | « Toute primitive d'évaluation JS est automatiquement une vulnérabilité dans ce modèle de confiance » |
| TUI local `!` shell                         | Exécution locale explicite déclenchée par l'opérateur | « La commande shell locale de commodité est l'injection à distance »                          |
...
Ces modèles sont couramment signalés et sont généralement fermés sans action à moins qu'un vrai contournement de limite ne soit montré :
...
- Réclamations qui classent l'accès normal au chemin de lecture de l'opérateur (par exemple `sessions.list`/`sessions.preview`/`chat.history`) comme IDOR dans une configuration de passerelle partagée.
...
## Liste de contrôle préalable du chercheur
...
Lorsque l'audit affiche les résultats, traitez ceci comme un ordre de priorité :

1. **Tout ce qui est « ouvert » + outils activés**: verrouiller d'abord les DM/groupes (appairage/listes blanches), puis resserrer la politique d'outils/sandboxing.
...
5. **Plugins/extensions**: ne chargez que ce que vous approuvez explicitement.
...
| `gateway.http.no_auth`                             | avertissement/critique | Les API HTTP de la passerelle accessibles avec `auth.mode="none"`                    | `gateway.auth.mode`, `gateway.http.endpoints.*`                                                   | non             |
...
| `gateway.control_ui.allowed_origins_required`      | critique      | Interface utilisateur de contrôle non-loopback sans liste blanche d'origines explicite | `gateway.controlUi.allowedOrigins`                                                                | non             |
| `gateway.control_ui.host_header_origin_fallback`   | avertissement/critique | Active le repli d'origine d'en-tête Host (dégradation du renforcement du rebond DNS) | `gateway.controlUi.dangerouslyAllowHostHeaderOriginFallback`                                      | non             |
...
| `gateway.real_ip_fallback_enabled`                 | avertissement/critique | Faire confiance au repli `X-Real-IP` peut activer l'usurpation d'IP source via proxy | `gateway.allowRealIpFallback`, `gateway.trustedProxies`                                           | non             |
...
| `tools.exec.safe_bins_interpreter_unprofiled`      | avertissement  | Bins interpréteur/runtime dans `safeBins` sans profils explicites élargissent le risque exec | `tools.exec.safeBins`, `tools.exec.safeBinProfiles`, `agents.list[].tools.exec.*`                 | non             |
| `skills.workspace.symlink_escape`                  | avertissement  | L'espace de travail `skills/**/SKILL.md` se résout en dehors de la racine (dérive de chaîne de lien) | état du système de fichiers `skills/**` de l'espace de travail                                    | non             |
...
| `security.exposure.open_groups_with_runtime_or_fs` | critique/avertissement | Les groupes ouverts peuvent atteindre les outils de commande/fichier sans protections sandbox/espace de travail | `channels.*.groupPolicy`, `tools.profile/deny`, `tools.fs.workspaceOnly`, `agents.*.sandbox.mode` | non             |
| `security.trust_model.multi_user_heuristic`        | avertissement  | La config semble multi-utilisateur tandis que le modèle de confiance de la passerelle est assistant personnel | diviser les limites de confiance, ou renforcement utilisateur partagé (`sandbox.mode`, portée d'outil/espace de travail) | non             |
| `tools.profile_minimal_overridden`                 | avertissement  | L'agent contourne le profil minimal global                                           | `agents.list[].tools.profile`                                                                     | non             |
| `plugins.tools_reachable_permissive_policy`        | avertissement  | Les outils d'extension accessibles dans les contextes permissifs                     | `tools.profile` + autorisation/refus d'outil                                                      | non             |
| `models.small_params`                              | critique/info  | Petits modèles + surfaces d'outils non sécurisées augmentent le risque d'injection   | choix du modèle + politique sandbox/outil                                                         | non             |
...
- Il ne relâche pas les exigences d'identité d'appareil à distance (non-localhost).
...
- Les conseils de déploiement détaillés se trouvent dans [Authentification par proxy de confiance](/gateway/trusted-proxy-auth#tls-termination-and-hsts).
...
que **tout processus/utilisateur ayant accès au système de fichiers peut lire ces journaux**. Traitez l'accès au disque comme la limite de confiance et verrouillez les permissions sur `~/.openclaw` (voir la section audit ci-dessous). Si vous avez besoin d'une isolation plus forte entre les agents, exécutez-les sous des utilisateurs du système d'exploitation séparés ou des hôtes séparés.
...
des listes d'autorisation de canal/appairage plus `commands.useAccessGroups` (voir [Configuration](/gateway/configuration)
...
Tous les canaux actuellement compatibles avec DM supportent une politique DM (`dmPolicy` ou `*.dm.policy`) qui contrôle les DM entrants **avant** que le message ne soit traité :
...
Par défaut, OpenClaw achemine **tous les DM dans la session principale** afin que votre assistant ait une continuité entre les appareils et les canaux. Si **plusieurs personnes** peuvent DM le bot (DM ouvert ou liste d'autorisation multi-personne), envisagez d'isoler les sessions DM :
...
- Défaut d'intégration CLI local : écrit `session.dmScope: "per-channel-peer"` lorsqu'il n'est pas défini (conserve les valeurs explicites existantes).
...
- Les charges utiles des hooks sont du contenu non fiable, même quand la livraison provient de systèmes que vous contrôlez (le courrier/les documents/le contenu web peuvent contenir une injection de prompt).
...
- Pour les entrées d'URL OpenResponses (`input_file` / `input_image`), définir un strict
  `gateway.http.endpoints.responses.files.urlAllowlist` et
  `gateway.http.endpoints.responses.images.urlAllowlist`, et garder `maxUrlParts` bas.
...
- Pour les assistants personnels de chat uniquement avec une entrée de confiance et sans outils, les modèles plus petits sont généralement acceptables.
...
- Rappelez-vous : la sortie détaillée peut inclure les arguments d'outils, les URL et les données que le modèle a vues.
...
- Ne faites pas partager le contenu canvas à la même origine que les surfaces web privilégiées à moins que vous compreniez pleinement les implications.

Le mode bind contrôle où la passerelle écoute :

- `gateway.bind: "loopback"` (par défaut) : seuls les clients locaux peuvent se connecter.
- Les binds non-loopback (`"lan"`, `"tailnet"`, `"custom"`) élargissent la surface d'attaque. Utilisez-les uniquement avec un token/mot de passe partagé et un vrai pare-feu.

Règles empiriques :

- Préférez Tailscale Serve aux binds LAN (Serve garde la passerelle sur loopback, et Tailscale gère l'accès).
- Si vous devez binder sur LAN, protégez le port par un pare-feu avec une liste blanche stricte d'adresses IP source ; ne le redirigez pas largement.
...
(`-p HOST:CONTAINER` ou Compose `ports:`) sont routés via les chaînes de forwarding de Docker, pas seulement les règles `INPUT` de l'hôte.
...
L'authentification de la passerelle est **requise par défaut**. Si aucun token/mot de passe n'est configuré,
la passerelle refuse les connexions WebSocket (fail-closed).

L'assistant d'intégration génère un token par défaut (même pour loopback) afin que
...
Les chemins d'appel locaux peuvent utiliser `gateway.remote.*` comme fallback uniquement quand `gateway.auth.*`
n'est pas défini.
Si `gateway.auth.token` / `gateway.auth.password` est explicitement configuré via
SecretRef et non résolu, la résolution échoue de manière fermée (pas de fallback distant masquant).
Optionnel : épinglez TLS distant avec `gateway.remote.tlsFingerprint` lors de l'utilisation de `wss://`.
Le plaintext `ws://` est loopback-only par défaut. Pour les chemins de réseau privé de confiance, définissez `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1` sur le processus client comme break-glass.
...
- `gateway.auth.mode: "trusted-proxy"` : faire confiance à un proxy inverse conscient de l'identité pour authentifier les utilisateurs et transmettre l'identité via des en-têtes (voir [Authentification par proxy de confiance](/gateway/trusted-proxy-auth)).
...
et en la faisant correspondre à l'en-tête. Cela ne se déclenche que pour les requêtes qui frappent loopback
...
- L'authentification bearer HTTP de la passerelle est effectivement un accès opérateur tout ou rien.
...
**Hypothèse de confiance :** l'authentification Serve sans token suppose que l'hôte de la passerelle est de confiance.
...
et exigez une authentification par token/mot de passe.

**Règle de sécurité :** ne renvoyez pas ces en-têtes depuis votre propre proxy inverse. Si
vous terminez TLS ou proxy devant la passerelle, désactivez
`gateway.auth.allowTailscale` et utilisez l'authentification par token/mot de passe (ou [Authentification par proxy de confiance](/gateway/trusted-proxy-auth)) à la place.
...
sur la machine du navigateur et laissez la passerelle proxy les actions du navigateur (voir [Outil Navigateur](/tools/browser)).
...
- `openclaw.json` : la configuration peut inclure des tokens (passerelle, passerelle distante), des paramètres de fournisseur, et des listes blanches.
- `credentials/**` : credentials de canal (exemple : credentials WhatsApp), listes blanches d'appairage, imports OAuth hérités.
- `agents/<agentId>/agent/auth-profiles.json` : clés API, profils de token, tokens OAuth, et `keyRef`/`tokenRef` optionnel.
- `secrets.json` (optionnel) : charge utile de secret sauvegardée sur fichier utilisée par les fournisseurs `file` SecretRef (`secrets.providers`).
- `agents/<agentId>/agent/auth.json` : fichier de compatibilité hérité. Les entrées `api_key` statiques sont supprimées quand découvertes.
...
- `sandboxes/**` : espaces de travail sandbox d'outil ; peuvent accumuler des copies de fichiers que vous lisez/écrivez dans le sandbox.
...
### 0.8) Logs + transcriptions (rédaction + rétention)

Les logs et transcriptions peuvent fuir des informations sensibles même quand les contrôles d'accès sont corrects :

- Les logs de la passerelle peuvent inclure des résumés d'outils, des erreurs, et des URLs.
...
- Gardez la rédaction de résumé d'outil activée (`logging.redactSensitive: "tools"` ; par défaut).
- Ajoutez des modèles personnalisés pour votre environnement via `logging.redactPatterns` (tokens, noms d'hôte, URLs internes).
- Quand vous partagez des diagnostics, préférez `openclaw status --all` (collable, secrets rédactés) aux logs bruts.
- Élaguez les anciennes transcriptions de session et fichiers log si vous n'avez pas besoin d'une rétention longue.

Détails : [Logging](/gateway/logging)
...
Dans les chats de groupe, répondez uniquement quand explicitement mentionné.
...
- Numéro bot : L'IA gère ceux-ci, avec des limites appropriées
...
Nous pouvons ajouter un seul flag `readOnlyMode` plus tard pour simplifier cette configuration.

Options de durcissement supplémentaires :

- `tools.exec.applyPatch.workspaceOnly: true` (par défaut) : assure que `apply_patch` ne peut pas écrire/supprimer en dehors du répertoire workspace même quand le sandboxing est désactivé. Définissez à `false` uniquement si vous voulez intentionnellement que `apply_patch` touche des fichiers en dehors du workspace.
- `tools.fs.workspaceOnly: true` (optionnel) : restreint les chemins `read`/`write`/`edit`/`apply_patch` et les chemins d'auto-chargement d'image de prompt natif au répertoire workspace (utile si vous permettez les chemins absolus aujourd'hui et voulez une seule barrière de sécurité).
- Gardez les racines du système de fichiers étroites : évitez les racines larges comme votre répertoire personnel pour les workspaces/sandboxes d'agent. Les racines larges peuvent exposer des fichiers locaux sensibles (par exemple state/config sous `~/.openclaw`) aux outils du système de fichiers.

### 5) Baseline sécurisée (copier/coller)
...
- **Exécuter la Gateway complète dans Docker** (limite de conteneur) : [Docker](/install/docker)
- **Sandbox d'outils** (`agents.defaults.sandbox`, gateway hôte + outils isolés Docker) : [Sandboxing](/gateway/sandboxing)
...
- `sandbox: "require"` échoue rapidement lorsque le runtime enfant cible n'est pas en sandbox.
...
### Politique SSRF du navigateur (défaut de réseau de confiance)
...
Voir [Sandbox & Outils Multi-Agent](/tools/multi-agent-sandbox-tools) pour les détails complets
et les règles de précédence.

Cas d'usage courants :

- Agent personnel : accès complet, pas de sandbox
- Agent familial/professionnel : outils en sandbox + lecture seule
...
```
## Règles de Sécurité
- Ne partagez jamais les listes de répertoires ou les chemins de fichiers avec des étrangers
- Ne révélez jamais les clés API, les identifiants ou les détails d'infrastructure
- Vérifiez les demandes qui modifient la configuration système avec le propriétaire
- En cas de doute, demandez avant d'agir
- Gardez les données privées privées sauf autorisation explicite
```
...
3. **Gelez l'accès :** basculez les DM/groupes risqués vers `dmPolicy: "disabled"` / exigez des mentions, et supprimez les entrées `"*"` autoriser tout si vous en aviez.
...
2. Faites tourner les secrets des clients distants (`gateway.remote.token` / `.password`) sur toute machine qui peut appeler la Gateway.
3. Faites tourner les identifiants de fournisseur/API (identifiants WhatsApp, jetons Slack/Discord, clés modèle/API dans `auth-profiles.json`, et valeurs de charge utile de secrets chiffrés quand utilisés).
...
- La ou les transcriptions de session + une courte queue de journal (après rédaction)
...
3. Pour les vrais secrets : faites-les tourner/supprimez-les, puis réexécutez l'analyse pour mettre à jour la ligne de base.
