...
Utilisez **Raspberry Pi OS Lite (64-bit)** — pas de bureau nécessaire pour un serveur sans écran.
...
sudo timedatectl set-timezone America/Chicago  # Changez selon votre fuseau horaire
```
...
Le swap prévient les plantages dus à manque de mémoire :
...
### Option B : Installation hackable (Pour bricoler)
...
L'installation hackable vous donne un accès direct aux journaux et au code — utile pour déboguer les problèmes spécifiques à ARM.
...
l'URL peut être un lien simple `http://127.0.0.1:18789/` ou un lien qui inclut `#token=...`.
...
Ouvrez ensuite l'URL du tableau de bord imprimée dans votre navigateur local.

Si l'interface demande une authentification, collez le token de `gateway.auth.token`
...
Les cartes SD sont lentes et s'usent. Un SSD USB améliore dramatiquement les performances :
...
Sur les hôtes Pi moins puissants, activez le cache de compilation de modules Node pour que les exécutions CLI répétées soient plus rapides :
...
### Tuning du démarrage systemd (optionnel)

Si ce Pi exécute principalement OpenClaw, ajoutez un drop-in de service pour réduire
les tremblements de redémarrage et maintenir l'environnement de démarrage stable :
...
Si possible, gardez l'état/cache OpenClaw sur un stockage soutenu par SSD pour éviter
...
**Utilisez toujours l'OS 64-bit.** Node.js et de nombreux outils modernes l'exigent. Vérifiez avec :
...
cd ~/openclaw  # si vous utilisez l'installation hackable
npm run build
sudo systemctl restart openclaw
```
...
1. Vérifiez si le binaire a une build ARM64
...
| **Pi 5 (8GB)**     | ~80 $       | 0 $          | Excessif mais futuriste   |
...
- [Nodes](/fr/nodes) — associez votre ordinateur portable/téléphone à la passerelle Pi
