...
- Alimentation électrique (PSU Pi officiel recommandé)
...
2. **Authentification :** Les clés API sont recommandées (OAuth peut être capricieux sur Pi sans écran)
...
La commande imprime `Dashboard URL:`. Selon la configuration de `gateway.auth.token`,
...
Pour un accès à distance toujours actif, voir [Tailscale](/gateway/tailscale).
...
```bash
# Vérifier si le démarrage se fait à partir d'USB
lsblk
```
...
- `OPENCLAW_NO_RESPAWN=1` évite le coût de démarrage supplémentaire du respawn automatique du CLI.
...
```bash
uname -m
# Devrait afficher : aarch64 (64-bit) et non armv7l (32-bit)
```
...
- Vérifier l'étranglement du CPU : `vcgencmd get_throttled` (devrait retourner `0x0`)
...
- [Nodes](/nodes) — associer votre ordinateur portable/téléphone à la passerelle Pi
```
