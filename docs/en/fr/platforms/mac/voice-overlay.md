...
- Si la superposition est déjà visible à partir du mot de réveil et que l'utilisateur appuie sur la touche de raccourci, la session de touche de raccourci _adopte_ le texte existant au lieu de le réinitialiser. La superposition reste visible tant que la touche de raccourci est enfoncée. Lorsque l'utilisateur relâche : envoyer s'il y a du texte coupé, sinon fermer.
...
- La transmission par appui adopte tout texte de superposition visible comme préfixe (donc appuyer sur la touche de raccourci alors que la superposition de réveil est active conserve le texte et ajoute la nouvelle parole). Elle attend jusqu'à 1,5 s une transcription finale avant de revenir au texte actuel.
...
- Vérifiez qu'un seul jeton de session actif ; les rappels obsolètes doivent être supprimés par le coordinateur.
