...
Cette application est désormais livrée avec les mises à jour automatiques Sparkle. Les builds de publication doivent être signés avec Developer ID, compressés et publiés avec une entrée appcast signée.
...
- Identifiants Notary (profil keychain ou clé API) pour `xcrun notarytool` si vous souhaitez une distribution DMG/zip sécurisée par Gatekeeper.
...
- Si `APP_BUILD` est omis, `scripts/package-mac-app.sh` dérive une valeur par défaut sûre pour Sparkle à partir de `APP_VERSION` (`YYYYMMDDNN` : les versions stables par défaut à `90`, les préversions utilisent une voie dérivée du suffixe) et utilise la valeur la plus élevée entre cette valeur et le nombre de commits git.
...
- Pour `BUILD_CONFIG=release`, `scripts/package-mac-app.sh` utilise désormais par défaut universal (`arm64 x86_64`) automatiquement. Vous pouvez toujours remplacer avec `BUILD_ARCHS=arm64` ou `BUILD_ARCHS=x86_64`. Pour les builds locaux/dev (`BUILD_CONFIG=debug`), il utilise par défaut l'architecture actuelle (`$(uname -m)`).
...
- Téléchargez `OpenClaw-2026.3.13.zip` (et `OpenClaw-2026.3.13.dSYM.zip`) vers la version GitHub pour la balise `v2026.3.13`.
...
- Vérifications de cohérence :
  ...
  - Sur une build publique précédente, exécutez « Vérifier les mises à jour… » à partir de l'onglet À propos et vérifiez que Sparkle installe la nouvelle build correctement.

Définition de terminé : l'application signée + appcast sont publiés, le flux de mise à jour fonctionne à partir d'une version installée plus ancienne, et les artefacts de publication sont attachés à la version GitHub.
