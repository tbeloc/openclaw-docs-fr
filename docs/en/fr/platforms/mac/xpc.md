...
**Modèle actuel :** une socket Unix locale connecte le **service hôte nœud** à l'**app macOS** pour les approbations d'exécution + `system.run`. Une CLI de débogage `openclaw-mac` existe pour les vérifications de découverte/connexion ; les actions d'agent transitent toujours par la WebSocket Gateway et `node.invoke`. L'automatisation UI utilise PeekabooBridge.
...
- Une petite surface pour l'automatisation : commandes Gateway + nœud, plus PeekabooBridge pour l'automatisation UI.
...
- Les requêtes `system.run` sont transférées à l'app macOS via une socket Unix locale.
...
- Sécurité : les hôtes bridge nécessitent un TeamID autorisé ; l'échappatoire DEBUG-only même-UID est gardée par `PEEKABOO_ALLOW_UNSIGNED_SOCKET_CLIENTS=1` (convention Peekaboo).
...
- Toute communication reste locale uniquement ; aucune socket réseau n'est exposée.
