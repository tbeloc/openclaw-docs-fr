...
read_when:
  - Débogage de la vue WebChat macOS ou du port de bouclage
title: "WebChat"
---
...
- Plan de données : méthodes WebSocket de la passerelle `chat.history`, `chat.send`, `chat.abort`, `chat.inject` et événements `chat`, `agent`, `presence`, `tick`, `health`.
- Session : utilise par défaut la session principale (`main`, ou `global` quand la portée est globale). L'interface utilisateur peut basculer entre les sessions.
