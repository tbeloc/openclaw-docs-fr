...
- **En cours de traitement (agent actif) :** `AppState.isWorking=true` déclenche un micro-mouvement de « queue/jambe qui s'agite » : tremblement des jambes plus rapide et léger décalage pendant le traitement. Actuellement basculé autour des exécutions de l'agent WebChat ; ajoutez le même basculement autour d'autres tâches longues quand vous les connecterez.
...
- Gardez les TTL courts (<10s) pour que l'icône revienne à l'état de base rapidement si une tâche se bloque.
