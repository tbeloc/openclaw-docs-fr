...
- **Déclenchement vocal (grandes oreilles) :** Le détecteur de réveil vocal appelle `AppState.triggerVoiceEars(ttl: nil)` quand le mot de réveil est entendu, en gardant `earBoostActive=true` pendant que l'énoncé est capturé. Les oreilles s'agrandissent (1,9x), obtiennent des trous d'oreille circulaires pour la lisibilité, puis disparaissent via `stopVoiceEars()` après 1s de silence. Déclenché uniquement à partir du pipeline vocal intégré à l'application.
- **En cours de traitement (agent en cours d'exécution) :** `AppState.isWorking=true` déclenche un micro-mouvement de « queue/jambe qui s'agite » : tremblement de jambe plus rapide et léger décalage pendant que le travail est en cours. Actuellement basculé autour des exécutions de l'agent WebChat ; ajoutez le même basculement autour d'autres tâches longues quand vous les connecterez.
...
- L'agitation utilise le tremblement des jambes jusqu'à ~1,0 avec un léger jiggle horizontal ; c'est additif à tout tremblement inactif existant.
...
- Gardez les TTL courts (&lt;10s) pour que l'icône revienne à la ligne de base rapidement si un travail se bloque.
