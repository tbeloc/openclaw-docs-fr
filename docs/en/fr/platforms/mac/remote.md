...
- L'hôte distant a besoin des mêmes approbations TCC que le local (Automation, Accessibility, Screen Recording, Microphone, Speech Recognition, Notifications). Exécutez l'intégration sur cette machine pour les accorder une fois.
...
- Réexécutez la connexion sur cet hôte si l'authentification expire. La vérification de santé surfacera les problèmes de lien.
