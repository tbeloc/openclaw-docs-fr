...
Le contrôle système (launchd/systemd) se trouve sur l'hôte de la passerelle. Voir [Passerelle](/gateway).

## Runbook de connexion

Nœud Android ⇄ (mDNS/NSD + WebSocket) ⇄ **Passerelle**

Android se connecte directement au WebSocket de la passerelle (par défaut `ws://<host>:18789`) et utilise l'appairage d'appareil (`role: node`).
...
- Vous pouvez exécuter la CLI (`openclaw`) sur la machine de passerelle (ou via SSH).
...
Pour les configurations tailnet uniquement (recommandé pour Vienna ⇄ London), liez la passerelle à l'IP tailnet :
...
À partir de la machine de passerelle :
...
#### Découverte Tailnet (Vienna ⇄ London) via unicast DNS-SD

La découverte NSD/mDNS Android ne traversera pas les réseaux. Si votre nœud Android et la passerelle se trouvent sur des réseaux différents mais sont connectés via Tailscale, utilisez plutôt Wide-Area Bonjour / unicast DNS-SD :
...
- L'application maintient sa connexion de passerelle active via un **service de premier plan** (notification persistante).
...
Commandes caméra (premier plan uniquement ; contrôlées par permission) :
