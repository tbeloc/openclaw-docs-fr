...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `heartbeat skipped` with `reason=no-route` → the default `target: "last"` has no conversation route yet; message the bot once or configure an explicit heartbeat target.
