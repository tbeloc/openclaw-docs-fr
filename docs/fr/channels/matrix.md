...
mais il nécessite l'activation du chiffrement E2EE.

État : supporté via plugin (@vector-im/matrix-bot-sdk). Messages directs, salons, threads, médias, réactions,
...
Support multi-compte : utilisez `channels.matrix.accounts` avec des identifiants par compte et un `name` optionnel. Voir [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) pour le modèle partagé.
...
- N'utilisez pas les noms d'affichage ou les parties locales nues (exemple : `"Alice"` ou `"alice"`). Ils sont ambigus et sont ignorés pour la correspondance de liste blanche. Utilisez les ID complets `@user:server`.
...
- Salons de liste blanche avec `channels.matrix.groups` (ID de salon ou alias ; les noms sont résolus en ID lorsque la recherche d'annuaire trouve une correspondance exacte unique) :
...
- Les listes blanches `users` par salon peuvent restreindre davantage les expéditeurs dans un salon spécifique (utilisez les ID utilisateur Matrix complets).
- L'assistant de configuration demande les listes blanches de salon (ID de salon, alias ou noms) et résout les noms uniquement sur une correspondance exacte et unique.
- Au démarrage, OpenClaw résout les noms de salon/utilisateur dans les listes blanches en ID et enregistre le mappage ; les entrées non résolues sont ignorées pour la correspondance de liste blanche.
...
- Pour autoriser **aucun salon**, définissez `channels.matrix.groupPolicy: "disabled"` (ou gardez une liste blanche vide).
...
- Connecté mais messages de salon ignorés : salon bloqué par `groupPolicy` ou liste blanche de salon.
- Messages directs ignorés : expéditeur en attente d'approbation lorsque `channels.matrix.dm.policy="pairing"`.
- Salons chiffrés échouent : support crypto ou décalage des paramètres de chiffrement.
