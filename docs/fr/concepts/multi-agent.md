...
read_when: "Vous voulez plusieurs agents isolés (espaces de travail + auth) dans un seul processus Gateway."
status: active
---

# Routage Multi-Agent

Objectif : plusieurs agents _isolés_ (espace de travail séparé + `agentDir` + sessions), plus plusieurs comptes de canal (par exemple deux WhatsApps) dans un Gateway en cours d'exécution. Les messages entrants sont acheminés vers un agent via des liaisons.
...
atteindre d'autres emplacements hôtes sauf si le bac à sable est activé. Voir
...
- Utilisez `accountId: "*"` pour un retour au niveau du canal sur tous les comptes.
...
Si vous voulez un compte par défaut au niveau du canal quand `accountId` est omis, définissez
