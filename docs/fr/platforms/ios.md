...
- Expose les capacités du nœud : Canvas, Capture d'écran, Capture caméra, Localisation, Mode conversation, Activation vocale.
...
2. Dans l'application iOS, ouvrez Paramètres et sélectionnez une gateway découverte (ou activez Host manuel et entrez host/port).
...
Les builds iOS officiels distribués utilisent le relais de push externe au lieu de publier le token APNs brut à la gateway.
...
4. L'application publie `push.apns.register` automatiquement après avoir un token APNs, la session opérateur connectée et l'enregistrement du relais réussi.
5. Après cela, `push.test`, les réveils de reconnexion et les nudges de réveil peuvent utiliser l'enregistrement relayé stocké.

Note de compatibilité :

- `OPENCLAW_APNS_RELAY_BASE_URL` fonctionne toujours comme un remplacement env temporaire pour la gateway.
...
5. `Relais -> APNs`
   ...
   - La gateway ne stocke jamais le token APNs brut pour les builds officiels soutenus par relais.
...
- Pour éviter de stocker les tokens APNs bruts des builds officiels sur la gateway.
...
Dans Paramètres, activez **Host manuel** et entrez le host + port de la gateway (par défaut `18789`).
...
- iOS peut suspendre l'audio en arrière-plan ; traitez les fonctionnalités vocales comme étant au mieux quand l'application n'est pas active.

## Erreurs courantes

- `NODE_BACKGROUND_UNAVAILABLE` : mettez l'application iOS au premier plan (les commandes canvas/caméra/écran l'exigent).
- `A2UI_HOST_NOT_CONFIGURED` : la Gateway n'a pas annoncé d'URL d'hôte canvas ; vérifiez `canvasHost` dans [Configuration de la Gateway](/gateway/configuration).
- L'invite d'appairage n'apparaît jamais : exécutez `openclaw devices list` et approuvez manuellement.
- La reconnexion échoue après la réinstallation : le token d'appairage Keychain a été effacé ; réappairez le nœud.
