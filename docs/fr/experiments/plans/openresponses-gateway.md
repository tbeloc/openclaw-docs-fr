...
pour les flux de travail d'agents et utilise des entrées basées sur des éléments ainsi que des événements de streaming sémantique. La spécification OpenResponses définit `/v1/responses`, et non `/v1/chat/completions`.
...
- Parité complète des fonctionnalités OpenResponses à la première étape (images, fichiers, outils hébergés).
...
- Retourner un seul message assistant avec le contenu `output_text`.
- Retourner `usage` avec des valeurs mises à zéro jusqu'à ce que la comptabilité des jetons soit connectée.
