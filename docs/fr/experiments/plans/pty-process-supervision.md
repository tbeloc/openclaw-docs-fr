---
summary: "Plan de production pour la supervision fiable des processus interactifs (PTY + non-PTY) avec propriété explicite, cycle de vie unifié et nettoyage déterministe"
read_when:
...
- les exécutions au premier plan `exec`
- les exécutions en arrière-plan `exec`
...
- L'exécution maintenant passe `ptyCommand` directement dans `src/agents/bash-tools.exec-runtime.ts`.
...
- `process kill/remove` utilise maintenant la terminaison de secours de l'arborescence des processus en cas d'absence de recherche du superviseur.
...
6. Tests du chemin du superviseur direct ajoutés
...
- `src/process/supervisor/adapters/child.ts` utilise maintenant la sémantique de terminaison de l'arborescence des processus pour les chemins d'annulation/délai d'expiration par défaut.
...
- Ajout de `src/process/supervisor/supervisor.pty-command.test.ts` pour le transfert verbatim de la commande PTY et le rejet de commande vide.
- Ajout de `src/process/supervisor/adapters/child.test.ts` pour le comportement de suppression de l'arborescence des processus dans l'annulation de l'adaptateur enfant.
...
- `process kill/remove` a maintenant un secours de terminaison du système d'exploitation réel en cas d'absence de recherche du superviseur.
- l'annulation/délai d'expiration enfant utilise maintenant la sémantique de suppression de l'arborescence des processus pour le chemin de suppression par défaut.
- Des tests de régression ont été ajoutés pour les deux comportements.

### Durabilité et réconciliation au démarrage

Le comportement de redémarrage est maintenant explicitement défini comme cycle de vie en mémoire uniquement.
...
La passe d'implémentation pour les éléments de contrat et de fiabilité requis est complète.

Complété :

- secours de terminaison réelle `process kill/remove`
- annulation de l'arborescence des processus pour le chemin de suppression par défaut de l'adaptateur enfant
- tests de régression pour le secours de suppression et le chemin de suppression de l'adaptateur enfant
...
- `src/agents/bash-tools.exec-runtime.ts` mis à jour pour passer la commande PTY explicitement et garder le chemin de secours.
- `src/agents/bash-tools.process.ts` mis à jour pour annuler via le superviseur avec terminaison de secours de l'arborescence des processus réelle.
- `src/agents/bash-tools.shared.ts` chemin d'aide de suppression directe supprimé.
...
- L'adaptateur PTY garantit toujours le règlement de l'attente lors de la suppression forcée et de la suppression de l'écouteur.

## 9. Définition de terminé

1. Le superviseur est propriétaire du cycle de vie pour les exécutions gérées.
2. La génération PTY utilise un contrat de commande explicite sans reconstruction argv.
3. La couche processus n'a aucune dépendance de type sur la couche agent pour les contrats stdin du superviseur.
4. Les valeurs par défaut du chien de garde sont à source unique.
...
- annulation de l'arborescence des processus pour les chemins de suppression par défaut des exécutions enfants
- valeurs par défaut du chien de garde unifiées
