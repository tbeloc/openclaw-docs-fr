...
- Assurez-vous que votre CLI se termine par 0 et imprime du texte brut ; JSON doit être massé via `jq -r .text`.
...
- Gardez les délais d'attente raisonnables (`timeoutSeconds`, 60 s par défaut) pour éviter de bloquer la file d'attente de réponse.
