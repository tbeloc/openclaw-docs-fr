...
pour la session d'onglet de navigateur actuelle et l'URL de passerelle sélectionnée, et les supprime de l'URL après le chargement.
...
- Si l'interface vous demande une authentification, collez le jeton de `gateway.auth.token` (ou `OPENCLAW_GATEWAY_TOKEN`) dans les paramètres de Control UI.

## Principes de base des jetons (local vs distant)

- **Localhost** : ouvrez `http://127.0.0.1:18789/`.
- **Source du jeton** : `gateway.auth.token` (ou `OPENCLAW_GATEWAY_TOKEN`) ; `openclaw dashboard` peut le transmettre via le fragment d'URL pour un amorçage unique, et le Control UI le conserve dans sessionStorage pour la session d'onglet de navigateur actuelle et l'URL de passerelle sélectionnée au lieu de localStorage.
