...
- Fournir une **politique par agent**, liste blanche, mode demande et liaison de nœud.
- Supporter les **modes demande** qui fonctionnent _avec_ ou _sans_ listes blanches.
- Multi-plateforme : socket Unix + authentification par token (parité macOS/Linux/Windows).

## Non-objectifs

- Pas de migration de liste blanche héritée ou support de schéma hérité.
...
- **Authentification du socket :** socket Unix + token (multi-plateforme) ; scinder plus tard si nécessaire.
...
- `on-miss` : demander uniquement quand la liste blanche ne correspond pas.
- `always` : demander à chaque fois.

La demande est **indépendante** de la liste blanche ; la liste blanche peut être utilisée avec `always` ou `on-miss`.
...
- Politique locale + listes blanches pour l'**hôte d'exécution** (gateway ou runner de nœud).
...
- Pas de formats de liste blanche hérités.
...
- JSON des approbations est local à l'hôte d'exécution.
...
2. Le service de nœud se connecte au socket local et envoie la demande/requête exec.
...
- Modèle de liaison :
  ...
  - Si non défini, l'agent peut choisir n'importe quel nœud (la politique applique toujours les défauts).
...
- `Exec finished (node=<id>, id=<runId>, code=<code>)` + sortie optionnelle
...
- La gateway `handleBridgeEvent` les mappe dans `enqueueSystemEvent`.
...
- Limiter stdout+stderr combinés à **200k** ; garder **tail 20k** pour les événements.
...
- Appliquer la liste blanche + modes de demande pour l'hôte `gateway`.
- Ajouter les limites de sortie.

### Phase 3 : application du runner de nœud

- Mettre à jour le runner de nœud pour appliquer la liste blanche + demande.
- Ajouter le pont de demande du socket Unix à l'UI de l'app macOS.
- Câbler `askFallback`.
...
- App Mac : éditeur de liste blanche, sélecteur par agent, UI de politique de demande.
...
- Tests d'événement Bridge : routage événement de nœud → événement système.
