...
Si vous liez votre WhatsApp personnel à OpenClaw, chaque message que vous recevez devient « entrée d'agent ». C'est rarement ce que vous voulez.
...
OpenClaw lit les instructions d'exploitation et la « mémoire » à partir du répertoire de l'espace de travail.
...
- Par défaut, la livraison du heartbeat aux cibles de style DM `user:<id>` est autorisée. Définissez `agents.defaults.heartbeat.directPolicy: "block"` pour supprimer la livraison directe tout en gardant les exécutions de heartbeat actives.
