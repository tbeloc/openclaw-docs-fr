...
<Tabs>
  <Tab title="Démarrage rapide (paramètres par défaut)">
    ...
    - Isolation DM par défaut : l'intégration CLI locale écrit `session.dmScope: "per-channel-peer"` si non défini. Détails : [Référence d'intégration CLI](/start/wizard-cli-reference#outputs-and-internals)
    - Exposition Tailscale **Désactivée**
    - Les DM Telegram + WhatsApp utilisent par défaut une **liste blanche** (vous serez invité à entrer votre numéro de téléphone)
  </Tab>
...
1. **Modèle/Authentification** — choisissez n'importe quel fournisseur/flux d'authentification supporté (clé API, OAuth ou jeton de configuration), y compris le fournisseur personnalisé
...
En mode `ref` non-interactif, la variable d'environnement du fournisseur doit être définie ; passer des drapeaux de clé inline sans cette variable d'environnement échoue rapidement.
Dans les exécutions interactives, choisir le mode de référence secrète vous permet de pointer vers une variable d'environnement ou une référence de fournisseur configurée (`file` ou `exec`), avec une validation rapide avant l'enregistrement.
2. **Espace de travail** — Emplacement des fichiers d'agent (par défaut `~/.openclaw/workspace`). Amorce les fichiers de bootstrap.
3. **Passerelle** — Port, adresse de liaison, mode d'authentification, exposition Tailscale.
En mode token interactif, choisissez le stockage de token en texte brut par défaut ou optez pour SecretRef.
Chemin SecretRef token non-interactif : `--gateway-token-ref-env <ENV_VAR>`.
...
Si l'authentification par token nécessite un token et que `gateway.auth.token` est géré par SecretRef, l'installation du démon le valide mais ne persiste pas le token résolu dans les métadonnées d'environnement du service superviseur.
Si l'authentification par token nécessite un token et que le SecretRef token configuré n'est pas résolu, l'installation du démon est bloquée avec des conseils actionnables.
Si à la fois `gateway.auth.token` et `gateway.auth.password` sont configurés et que `gateway.auth.mode` n'est pas défini, l'installation du démon est bloquée jusqu'à ce que le mode soit défini explicitement.
...
Il **n'installe** ou ne change **rien** sur l'hôte distant.
...
Pour des détails étape par étape et les résultats de configuration, consultez
[Référence d'intégration CLI](/start/wizard-cli-reference).
Pour des exemples non-interactifs, consultez [Automatisation CLI](/start/wizard-cli-automation).
...
- Aperçu de l'intégration : [Aperçu de l'intégration](/start/onboarding-overview)
- Intégration de l'application macOS : [Intégration](/start/onboarding)
- Rituel de première exécution de l'agent : [Amorçage de l'agent](/start/bootstrapping)
