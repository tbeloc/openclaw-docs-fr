...
Générez un code QR d'appairage iOS et un code de configuration à partir de votre configuration de passerelle actuelle.
...
- Le code de configuration lui-même porte désormais un opaque `bootstrapToken` de courte durée, et non le jeton/mot de passe de passerelle partagé.
...
- Remarque sur l'asymétrie de version de passerelle : ce chemin de commande nécessite une passerelle qui prend en charge `secrets.resolve` ; les anciennes passerelles retournent une erreur de méthode inconnue.
