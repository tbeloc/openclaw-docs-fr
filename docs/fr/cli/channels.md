...
read_when:
  ...
  - Vous souhaitez vérifier le statut du canal ou consulter les journaux du canal
title: "channels"
---

# `openclaw channels`

Gérez les comptes de canal de chat et leur statut d'exécution sur la passerelle.
...
Conseil : `openclaw channels add --help` affiche les drapeaux spécifiques au canal (token, app token, chemins signal-cli, etc).
...
- Les liaisons existantes limitées au canal (pas de `accountId`) continuent de correspondre au compte par défaut.
...
- `openclaw channels status` revient à des résumés limités à la configuration lorsque la passerelle est inaccessible. Si une credential de canal prise en charge est configurée via SecretRef mais indisponible dans le chemin de commande actuel, elle signale ce compte comme configuré avec des notes dégradées au lieu de l'afficher comme non configuré.
