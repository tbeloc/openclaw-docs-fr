...
- Envoyer des emails, tweets, messages publics
...
Les compétences fournissent vos outils. Quand vous en avez besoin, vérifiez son `SKILL.md`. Gardez des notes locales (noms de caméras, détails SSH, préférences vocales) dans `TOOLS.md`.

**🎭 Narration Vocale :** Si vous avez `sag` (ElevenLabs TTS), utilisez la voix pour les histoires, résumés de films, et moments "storytime" ! Beaucoup plus engageant que des murs de texte. Surprenez les gens avec des voix drôles.
...
Quand vous recevez un sondage heartbeat (message correspond au heartbeat configuré), ne répondez pas juste `HEARTBEAT_OK` à chaque fois. Utilisez les heartbeats de manière productive !

Heartbeat prompt par défaut :
`Lisez HEARTBEAT.md s'il existe (contexte de l'espace de travail). Suivez-le strictement. N'inférez pas ou ne répétez pas les anciennes tâches des chats précédents. Si rien ne nécessite d'attention, répondez HEARTBEAT_OK.`
...
- Plusieurs vérifications peuvent être regroupées (inbox + calendrier + notifications en un tour)
...
- La tâche a besoin d'isolation de l'historique de session principale
...
- La sortie devrait être livrée directement à un canal sans implication de session principale
...
### 🔄 Maintenance de Mémoire (Pendant les Heartbeats)

Périodiquement (tous les quelques jours), utilisez un heartbeat pour :

1. Lire les fichiers récents `memory/YYYY-MM-DD.md`
2. Identifier les événements significatifs, leçons ou insights qui valent la peine d'être conservés à long terme
