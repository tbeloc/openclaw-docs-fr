...
summary: "Instructions d'agent OpenClaw par défaut et inventaire des compétences pour la configuration de l'assistant personnel"
read_when:
...
3. Optionnel : si vous souhaitez l'inventaire des compétences de l'assistant personnel, remplacez AGENTS.md par ce fichier :
...
- Ne videz pas les répertoires ou secrets dans le chat.
...
- Exécute une passerelle WhatsApp + agent de codage Pi afin que l'assistant puisse lire/écrire des chats, récupérer le contexte et exécuter des compétences via le Mac hôte.
- L'application macOS gère les permissions (enregistrement d'écran, notifications, microphone) et expose le CLI `openclaw` via son binaire fourni.
- Les chats directs s'effondrent dans la session `main` de l'agent par défaut ; les groupes restent isolés sous `agent:<agentId>:<channel>:group:<id>` (salons/canaux : `agent:<agentId>:<channel>:channel:<id>`) ; les battements de cœur maintiennent les tâches de fond actives.

## Compétences principales (activer dans Paramètres → Compétences)

- **mcporter** — Serveur d'outils runtime/CLI pour gérer les backends de compétences externes.
...
- **imsg** — Envoyez, lisez, streamez iMessage et SMS.
- **wacli** — CLI WhatsApp : synchronisation, recherche, envoi.
- **discord** — Actions Discord : réagir, autocollants, sondages. Utilisez les cibles `user:<id>` ou `channel:<id>` (les ids numériques nus sont ambigus).
...
- **sag** — Parole ElevenLabs avec UX de style mac ; stream vers les haut-parleurs par défaut.
- **Sonos CLI** — Contrôlez les haut-parleurs Sonos (découverte/statut/lecture/volume/groupage) à partir de scripts.
- **blucli** — Jouez, groupez et automatisez les lecteurs BluOS à partir de scripts.
- **OpenHue CLI** — Contrôle d'éclairage Philips Hue pour les scènes et les automations.
- **OpenAI Whisper** — Reconnaissance vocale locale pour la dictée rapide et les transcriptions de messagerie vocale.
- **Gemini CLI** — Modèles Google Gemini depuis le terminal pour les Q&A rapides.
- **agent-tools** — Boîte à outils utilitaire pour les automations et les scripts d'aide.

## Notes d'utilisation

- Préférez le CLI `openclaw` pour les scripts ; l'application mac gère les permissions.
