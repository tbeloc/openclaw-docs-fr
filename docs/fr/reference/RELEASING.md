...
summary: "Liste de contrôle de publication étape par étape pour npm + application macOS"
read_when:
...
- Le workflow de publication applique les formats de balise stable/bêta actuels et rejette les versions dont la date CalVer est à plus de 2 jours calendaires UTC de la date de publication.
...
- [ ] Poussez la balise git correspondante pour déclencher `.github/workflows/openclaw-npm-release.yml`.
  ...
  - Le workflow rejette les balises qui ne correspondent pas à `package.json`, ne sont pas sur `main`, ou dont la date CalVer est à plus de 2 jours calendaires UTC de la date de publication.
...
- [ ] Balisez et poussez : `git tag vX.Y.Z && git push origin vX.Y.Z` (ou `git push --tags`).
  - Pousser la balise déclenche également le workflow de publication npm.
