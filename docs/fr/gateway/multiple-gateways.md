...
La plupart des configurations doivent utiliser une seule passerelle car une passerelle unique peut gérer plusieurs connexions de messagerie et agents. Si vous avez besoin d'une isolation plus forte ou d'une redondance (par exemple, un bot de secours), exécutez des passerelles séparées avec des profils/ports isolés.
...
Espacement des ports : laissez au moins 20 ports entre les ports de base pour que les ports dérivés du navigateur/canvas/CDP ne se heurtent jamais.
...
# - le reste de l'intégration est le même qu'à la normale
...
Si vous remplacez l'un de ces éléments dans la configuration ou l'environnement, vous devez les maintenir uniques par instance.
