...
Sous le capot, les requêtes sont exécutées comme une exécution normale d'agent Gateway (même chemin de code que `openclaw agent`), donc le routage/les permissions/la configuration correspondent à votre Gateway.
...
- Il n'y a pas de limite de tool séparé non-propriétaire/par-utilisateur sur ce point de terminaison ; une fois qu'un appelant passe l'authentification Gateway ici, OpenClaw traite cet appelant comme un opérateur de confiance pour cette gateway.
