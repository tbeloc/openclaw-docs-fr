...
Si le même nom de compétence existe à plusieurs endroits, la précédence habituelle
...
ClawHub est le registre public des compétences pour OpenClaw. Parcourez
...
- Si un agent est en sandbox, le binaire doit également exister **à l'intérieur du conteneur**.
  ...
  Les installations de paquets nécessitent également une sortie réseau, un système de fichiers inscriptible et un utilisateur root dans le sandbox.
  Exemple : la compétence `summarize` (`skills/summarize/SKILL.md`) a besoin de la CLI `summarize`
...
- `config` : sac optionnel pour les champs personnalisés par compétence ; les clés personnalisées doivent se trouver ici.
...
OpenClaw prend un snapshot des compétences éligibles **quand une session démarre** et réutilise cette liste pour les tours suivants dans la même session. Les modifications apportées aux compétences ou à la configuration prennent effet à la prochaine nouvelle session.
...
Cela dépend du nœud signalant son support de commande et d'une sonde bin via `system.run
