---
summary: "Tâches LLM JSON uniquement pour les workflows (outil plugin optionnel)"
read_when:
...
`llm-task` est un **outil plugin optionnel** qui exécute une tâche LLM JSON uniquement et
...
- Placez les approbations avant toute étape ayant des effets secondaires (envoi, publication, exécution).
