...
Créez un fichier `SKILL.md` dans ce répertoire. Ce fichier utilise le frontmatter YAML pour les métadonnées et Markdown pour les instructions.
...
Vous pouvez définir des outils personnalisés dans le frontmatter ou instruire l'agent pour utiliser des outils système existants (comme `bash` ou `browser`).
