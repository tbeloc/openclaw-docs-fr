...
- Sachez comment vous avez installé : **globale** (npm/pnpm) vs **à partir de la source** (git clone).
...
- Nécessite un arborescence propre.
...
L'interface utilisateur de contrôle a **Mise à jour et redémarrage** (RPC : `update.run`). Elle :
