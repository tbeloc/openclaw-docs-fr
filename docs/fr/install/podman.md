---
summary: "Exécuter OpenClaw dans un conteneur Podman sans privilèges"
read_when:
...
Exécutez la passerelle OpenClaw dans un conteneur Podman **sans privilèges**. Utilise la même image que Docker (construite à partir du [Dockerfile](https://github.com/openclaw/openclaw/blob/main/Dockerfile) du dépôt).
...
**1. Configuration initiale** (depuis la racine du dépôt ; crée un utilisateur, construit l'image, installe le script de lancement) :
...
- `OPENCLAW_EXTENSIONS` — pré-installer les dépendances d'extension (noms d'extension séparés par des espaces, par ex. `diagnostics-otel matrix`)
...
**3. Assistant d'intégration** (par ex. pour ajouter des canaux ou des fournisseurs) :
...
- **Accueil :** par ex. `/home/openclaw` — contient `~/.openclaw` (config, espace de travail) et le script de lancement `run-openclaw-podman.sh`.
- **Podman sans privilèges :** L'utilisateur doit avoir une plage **subuid** et **subgid**. De nombreuses distributions les attribuent automatiquement lors de la création de l'utilisateur. Si setup affiche un avertissement, ajoutez des lignes à `/etc/subuid` et `/etc/subgid` :
...
- **Config :** Seuls `openclaw` et root peuvent accéder à `/home/openclaw/.openclaw`. Pour modifier la config : utilisez l'interface de contrôle une fois la passerelle en cours d'exécution, ou `sudo -u openclaw $EDITOR /home/openclaw/.openclaw/openclaw.json`.
...
- **Optionnel :** Dans ce `.env`, vous pouvez définir les clés de fournisseur (par ex. `GROQ_API_KEY`, `OLLAMA_API_KEY`) et d'autres variables d'environnement OpenClaw.
...
- **Chemins :** La config et l'espace de travail hôte par défaut sont `~openclaw/.openclaw` et `~openclaw/.openclaw/workspace`. Remplacez les chemins hôte utilisés par le script de lancement avec `OPENCLAW_CONFIG_DIR` et `OPENCLAW_WORKSPACE_DIR`.
...
`setup-podman.sh` met maintenant en scène le tar d'image dans un répertoire temporaire privé et imprime le répertoire de base choisi lors de la configuration. Pour les exécutions sans root, il n'accepte `TMPDIR` que lorsque cette base est sûre à utiliser ; sinon, il revient à `/var/tmp`, puis `/tmp`. Le tar sauvegardé reste propriétaire uniquement et est diffusé dans le `podman load` de l'utilisateur cible, de sorte que les répertoires temporaires privés de l'appelant ne bloquent pas la configuration.
...
- **Permission refusée (EACCES) sur la config ou les profils d'authentification :** Le conteneur utilise par défaut `--userns=keep-id` et s'exécute avec le même uid/gid que l'utilisateur hôte exécutant le script. Assurez-vous que votre `OPENCLAW_CONFIG_DIR` et `OPENCLAW_WORKSPACE_DIR` hôte sont possédés par cet utilisateur.
- **Démarrage de la passerelle bloqué (manquant `gateway.mode=local`) :** Assurez-vous que `~openclaw/.openclaw/openclaw.json` existe et définit `gateway.mode="local"`. `setup-podman.sh` crée ce fichier s'il est manquant.
- **Podman sans privilèges échoue pour l'utilisateur openclaw :** Vérifiez que `/etc/subuid` et `/etc/subgid` contiennent une ligne pour `openclaw` (par ex. `openclaw:100000:65536`). Ajoutez-la si elle est manquante et redémarrez.
- **Nom de conteneur en cours d'utilisation :** Le script de lancement utilise `podman run --replace`, de sorte que le conteneur existant est remplacé au redémarrage. Pour nettoyer manuellement : `podman rm -f openclaw`.
- **Script non trouvé lors de l'exécution en tant qu'openclaw :** Assurez-vous que `setup-podman.sh` a été exécuté pour que `run-openclaw-podman.sh` soit copié dans le répertoire personnel d'openclaw (par ex. `/home/openclaw/run-openclaw-podman.sh`).
- **Service Quadlet non trouvé ou échec du démarrage :** Exécutez `sudo systemctl --machine openclaw@ --user daemon-reload` après avoir modifié le fichier `.container`. Quadlet nécessite cgroups v2 : `podman info --format '{{.Host.CgroupsVersion}}'` doit afficher `2`.
