...
OpenClaw est un conteneur unique avec quelques fichiers de configuration. La personnalisation intéressante se trouve dans le contenu des agents (fichiers markdown, compétences, surcharges de configuration), pas dans la création de modèles d'infrastructure. Kustomize gère les overlays sans la surcharge d'un chart Helm. Si votre déploiement devient plus complexe, un chart Helm peut être superposé à ces manifestes.
...
Récupérez le jeton de la passerelle et collez-le dans l'interface utilisateur de contrôle :
...
- Sécurité : `readOnlyRootFilesystem`, `drop: ALL` capabilities, utilisateur non-root (UID 1000)
...
- Les secrets sont générés dans un répertoire temporaire et appliqués directement au cluster — aucun matériel secret n'est écrit dans le checkout du repo
